...
/// Bidirectional hash map from `i64` keys to `i64` values.
///
...
/// maps in sync.
pub const I64I64HashBiMap = struct {
    forward: OpenHashMap(i64, i64),
    reverse: OpenHashMap(i64, i64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I64I64HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I64I64HashBiMap {
        return .{
            .forward = OpenHashMap(i64, i64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i64, i64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I64I64HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I64I64HashBiMap, key: i64, value: i64) ?i64 {
    ...
    pub fn removeValue(self: *I64I64HashBiMap, value: i64) ?i64 {
    ...
    pub fn containsKey(self: *const I64I64HashBiMap, key: i64) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I64I64HashBiMap, value: i64) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I64I64HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I64I64HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I64I64HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I64I64HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I64I64HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I64I64HashBiMap, f: *const fn (i64, i64) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I64I64HashBiMap, f: *const fn (i64) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I64I64HashBiMap, f: *const fn (i64) void) void {
    ...
    pub fn inverse(self: *const I64I64HashBiMap) I64I64HashBiMap {
        var result = I64I64HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I64I64HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I64I64HashBiMap, other: *const I64I64HashBiMap) bool {
...
test "I64I64HashBiMap: put and get and getKey" {
    var m = I64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    // forward lookup
...
test "I64I64HashBiMap: put overwrite updates reverse" {
    var m = I64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I64I64HashBiMap: removeValue" {
    var m = I64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I64I64HashBiMap: containsKey and containsValue" {
    var m = I64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    try std.testing.expect(m.containsKey(1));
...
test "I64I64HashBiMap: display format" {
    var m = I64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I64I64HashBiMap: eql" {
    var m1 = I64I64HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);
    var m2 = I64I64HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I64I64HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64I64HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I64I64HashBiMap.init(failing.allocator());
    defer m.deinit();
