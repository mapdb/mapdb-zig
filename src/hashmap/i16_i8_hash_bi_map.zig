...
const I8I16HashBiMap = @import("i8_i16_hash_bi_map.zig").I8I16HashBiMap;

/// Bidirectional hash map from `i16` keys to `i8` values.
///
...
/// maps in sync.
pub const I16I8HashBiMap = struct {
    forward: OpenHashMap(i16, i8),
    reverse: OpenHashMap(i8, i16),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I16I8HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I16I8HashBiMap {
        return .{
            .forward = OpenHashMap(i16, i8).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i8, i16).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I16I8HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I16I8HashBiMap, key: i16, value: i8) ?i8 {
    ...
    pub fn removeValue(self: *I16I8HashBiMap, value: i8) ?i16 {
    ...
    pub fn containsKey(self: *const I16I8HashBiMap, key: i16) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I16I8HashBiMap, value: i8) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I16I8HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I16I8HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I16I8HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I16I8HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16I8HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16I8HashBiMap, f: *const fn (i16, i8) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I16I8HashBiMap, f: *const fn (i16) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I16I8HashBiMap, f: *const fn (i8) void) void {
    ...
    pub fn inverse(self: *const I16I8HashBiMap) I8I16HashBiMap {
        var result = I8I16HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I16I8HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I16I8HashBiMap, other: *const I16I8HashBiMap) bool {
...
test "I16I8HashBiMap: put and get and getKey" {
    var m = I16I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    // forward lookup
...
test "I16I8HashBiMap: put overwrite updates reverse" {
    var m = I16I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I16I8HashBiMap: removeValue" {
    var m = I16I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I16I8HashBiMap: containsKey and containsValue" {
    var m = I16I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    try std.testing.expect(m.containsKey(1));
...
test "I16I8HashBiMap: display format" {
    var m = I16I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I16I8HashBiMap: eql" {
    var m1 = I16I8HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);
    var m2 = I16I8HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I16I8HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I16I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16I8HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I16I8HashBiMap.init(failing.allocator());
    defer m.deinit();
