...
const I32CharHashBiMap = @import("i32_char_hash_bi_map.zig").I32CharHashBiMap;

/// Bidirectional hash map from `u21` keys to `i32` values.
///
...
/// maps in sync.
pub const CharI32HashBiMap = struct {
    forward: OpenHashMap(u21, i32),
    reverse: OpenHashMap(i32, u21),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharI32HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) CharI32HashBiMap {
        return .{
            .forward = OpenHashMap(u21, i32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i32, u21).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *CharI32HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *CharI32HashBiMap, key: u21, value: i32) ?i32 {
    ...
    pub fn removeValue(self: *CharI32HashBiMap, value: i32) ?u21 {
    ...
    pub fn containsKey(self: *const CharI32HashBiMap, key: u21) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const CharI32HashBiMap, value: i32) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const CharI32HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const CharI32HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *CharI32HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *CharI32HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI32HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI32HashBiMap, f: *const fn (u21, i32) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const CharI32HashBiMap, f: *const fn (u21) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const CharI32HashBiMap, f: *const fn (i32) void) void {
    ...
    pub fn inverse(self: *const CharI32HashBiMap) I32CharHashBiMap {
        var result = I32CharHashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const CharI32HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const CharI32HashBiMap, other: *const CharI32HashBiMap) bool {
...
test "CharI32HashBiMap: put and get and getKey" {
    var m = CharI32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
    // forward lookup
    try std.testing.expectEqual(@as(?i32, 1), m.get('a'));
    try std.testing.expectEqual(@as(?i32, 2), m.get('b'));
    // reverse lookup
...
test "CharI32HashBiMap: put overwrite updates reverse" {
    var m = CharI32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
...
test "CharI32HashBiMap: removeValue" {
    var m = CharI32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI32HashBiMap: containsKey and containsValue" {
    var m = CharI32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    try std.testing.expect(m.containsKey('a'));
...
test "CharI32HashBiMap: display format" {
    var m = CharI32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
...
test "CharI32HashBiMap: eql" {
    var m1 = CharI32HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1);
    _ = m1.put('b', 2);
    var m2 = CharI32HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2);
    _ = m2.put('a', 1);
...
test "CharI32HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = CharI32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI32HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = CharI32HashBiMap.init(failing.allocator());
    defer m.deinit();
