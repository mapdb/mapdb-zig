...
const ImmutableI8BoolHashMap = @import("../immutable/immutable_i8_bool_hash_map.zig").ImmutableI8BoolHashMap;

/// Hash map from `i8` keys to `bool` values.
///
...
pub const I8BoolHashMap = struct {
    inner: OpenHashMap(i8, bool),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I8BoolHashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) I8BoolHashMap {
        return .{
            .inner = OpenHashMap(i8, bool).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I8BoolHashMap {
        return .{
            .inner = OpenHashMap(i8, bool).init(
    ...
    pub fn deinit(self: *I8BoolHashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *I8BoolHashMap, key: i8, value: bool) ?bool {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const I8BoolHashMap, key: i8) ?bool {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const I8BoolHashMap, key: i8, default_value: bool) bool {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *I8BoolHashMap, key: i8) ?bool {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const I8BoolHashMap, key: i8) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const I8BoolHashMap, value: bool) bool {
    ...
    pub fn getEntry(self: *I8BoolHashMap, key: i8) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const I8BoolHashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const I8BoolHashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const I8BoolHashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *I8BoolHashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I8BoolHashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8BoolHashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8BoolHashMap, f: *const fn (i8, bool) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const I8BoolHashMap, f: *const fn (i8) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const I8BoolHashMap, f: *const fn (bool) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I8BoolHashMap, predicate: *const fn (i8, bool) bool) I8BoolHashMap {
    ...
    pub fn reject(self: *const I8BoolHashMap, predicate: *const fn (i8, bool) bool) I8BoolHashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const I8BoolHashMap, predicate: *const fn (i8, bool) bool) ?struct { key: i8, value: bool } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const I8BoolHashMap, predicate: *const fn (i8, bool) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const I8BoolHashMap, predicate: *const fn (i8, bool) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const I8BoolHashMap, predicate: *const fn (i8, bool) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I8BoolHashMap, predicate: *const fn (i8, bool) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const I8BoolHashMap, initial: bool, f: *const fn (bool, i8, bool) bool) bool {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const I8BoolHashMap, allocator: Allocator) []i8 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const I8BoolHashMap, allocator: Allocator) []bool {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Conversion ----

    pub fn toImmutable(self: *const I8BoolHashMap) ImmutableI8BoolHashMap {
        return ImmutableI8BoolHashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *I8BoolHashMap, key: i8, value: bool) *I8BoolHashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I8BoolHashMap, key: i8) *I8BoolHashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *I8BoolHashMap, keys: []const i8) *I8BoolHashMap {
    ...
    pub fn updateValue(self: *I8BoolHashMap, key: i8, initial: bool, f: *const fn (bool) bool) bool {
    ...
    pub fn format(self: *const I8BoolHashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const I8BoolHashMap, other: *const I8BoolHashMap) bool {
...
test "I8BoolHashMap: put and get" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
    _ = m.put(3, true);
...
test "I8BoolHashMap: put overwrite" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
...
test "I8BoolHashMap: containsKey and containsValue" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    try std.testing.expect(m.containsKey(1));
...
test "I8BoolHashMap: getOrDefault" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
...
test "I8BoolHashMap: keysToSlice and valuesToSlice" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
...
test "I8BoolHashMap: toImmutable" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
...
test "I8BoolHashMap: fluent withKeyValue/withoutKey" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolHashMap: eql" {
    var m1 = I8BoolHashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, true);
    _ = m1.put(2, false);
    var m2 = I8BoolHashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, false);
    _ = m2.put(1, true);
...
test "I8BoolHashMap: resize" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(i8, @intCast(i)), i % 3 == 0);
    }
    try std.testing.expect(m.len() > 0);
}

test "I8BoolHashMap: entry orInsert" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolHashMap: entry andModify orInsert" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolHashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolHashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = I8BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolHashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I8BoolHashMap.init(failing.allocator());
    defer m.deinit();
