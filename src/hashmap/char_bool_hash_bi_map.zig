...
const BoolCharHashBiMap = @import("bool_char_hash_bi_map.zig").BoolCharHashBiMap;

/// Bidirectional hash map from `u21` keys to `bool` values.
///
...
/// maps in sync.
pub const CharBoolHashBiMap = struct {
    forward: OpenHashMap(u21, bool),
    reverse: OpenHashMap(bool, u21),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharBoolHashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) CharBoolHashBiMap {
        return .{
            .forward = OpenHashMap(u21, bool).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(bool, u21).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *CharBoolHashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *CharBoolHashBiMap, key: u21, value: bool) ?bool {
    ...
    pub fn removeValue(self: *CharBoolHashBiMap, value: bool) ?u21 {
    ...
    pub fn containsKey(self: *const CharBoolHashBiMap, key: u21) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const CharBoolHashBiMap, value: bool) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const CharBoolHashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const CharBoolHashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *CharBoolHashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *CharBoolHashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharBoolHashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharBoolHashBiMap, f: *const fn (u21, bool) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const CharBoolHashBiMap, f: *const fn (u21) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const CharBoolHashBiMap, f: *const fn (bool) void) void {
    ...
    pub fn inverse(self: *const CharBoolHashBiMap) BoolCharHashBiMap {
        var result = BoolCharHashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const CharBoolHashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const CharBoolHashBiMap, other: *const CharBoolHashBiMap) bool {
...
test "CharBoolHashBiMap: put and get and getKey" {
    var m = CharBoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', true);
    _ = m.put('b', false);
    // forward lookup
    try std.testing.expectEqual(@as(?bool, true), m.get('a'));
    try std.testing.expectEqual(@as(?bool, false), m.get('b'));
    // reverse lookup
    try std.testing.expectEqual(@as(?u21, 'a'), m.getKey(true));
    try std.testing.expectEqual(@as(?u21, 'b'), m.getKey(false));
    // missing key
...
test "CharBoolHashBiMap: put overwrite updates reverse" {
    var m = CharBoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', true);
    ...
    try std.testing.expectEqual(@as(?u21, null), m.getKey(true));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?u21, 'a'), m.getKey(false));
}

test "CharBoolHashBiMap: remove" {
    var m = CharBoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', true);
    _ = m.put('b', false);
...
test "CharBoolHashBiMap: removeValue" {
    var m = CharBoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', true);
    _ = m.put('b', false);
...
test "CharBoolHashBiMap: containsKey and containsValue" {
    var m = CharBoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', true);
    try std.testing.expect(m.containsKey('a'));
...
test "CharBoolHashBiMap: inverse" {
    var m = CharBoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', true);
    _ = m.put('b', false);
    ...
    try std.testing.expectEqual(@as(?u21, 'a'), inv.get(true));
    try std.testing.expectEqual(@as(?u21, 'b'), inv.get(false));
}

test "CharBoolHashBiMap: display format" {
    var m = CharBoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', true);
...
test "CharBoolHashBiMap: eql" {
    var m1 = CharBoolHashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', true);
    _ = m1.put('b', false);
    var m2 = CharBoolHashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', false);
    _ = m2.put('a', true);
...
test "CharBoolHashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = CharBoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharBoolHashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = CharBoolHashBiMap.init(failing.allocator());
    defer m.deinit();
