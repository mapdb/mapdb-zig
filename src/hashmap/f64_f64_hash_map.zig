...
const ImmutableF64F64HashMap = @import("../immutable/immutable_f64_f64_hash_map.zig").ImmutableF64F64HashMap;

/// Hash map from `f64` keys to `f64` values.
///
...
pub const F64F64HashMap = struct {
    inner: OpenHashMap(f64, f64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64F64HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) F64F64HashMap {
        return .{
            .inner = OpenHashMap(f64, f64).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F64F64HashMap {
        return .{
            .inner = OpenHashMap(f64, f64).init(
    ...
    pub fn deinit(self: *F64F64HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *F64F64HashMap, key: f64, value: f64) ?f64 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const F64F64HashMap, key: f64) ?f64 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const F64F64HashMap, key: f64, default_value: f64) f64 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *F64F64HashMap, key: f64) ?f64 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const F64F64HashMap, key: f64) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const F64F64HashMap, value: f64) bool {
    ...
    pub fn getEntry(self: *F64F64HashMap, key: f64) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const F64F64HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const F64F64HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const F64F64HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *F64F64HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F64F64HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64F64HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64F64HashMap, f: *const fn (f64, f64) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const F64F64HashMap, f: *const fn (f64) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const F64F64HashMap, f: *const fn (f64) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F64F64HashMap, predicate: *const fn (f64, f64) bool) F64F64HashMap {
    ...
    pub fn reject(self: *const F64F64HashMap, predicate: *const fn (f64, f64) bool) F64F64HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const F64F64HashMap, predicate: *const fn (f64, f64) bool) ?struct { key: f64, value: f64 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const F64F64HashMap, predicate: *const fn (f64, f64) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const F64F64HashMap, predicate: *const fn (f64, f64) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const F64F64HashMap, predicate: *const fn (f64, f64) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F64F64HashMap, predicate: *const fn (f64, f64) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const F64F64HashMap, initial: f64, f: *const fn (f64, f64, f64) f64) f64 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const F64F64HashMap, allocator: Allocator) []f64 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const F64F64HashMap, allocator: Allocator) []f64 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const F64F64HashMap) f64 {
        var total: f64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *F64F64HashMap, key: f64, delta: f64) f64 {
    ...
    pub fn toImmutable(self: *const F64F64HashMap) ImmutableF64F64HashMap {
        return ImmutableF64F64HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *F64F64HashMap, key: f64, value: f64) *F64F64HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F64F64HashMap, key: f64) *F64F64HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *F64F64HashMap, keys: []const f64) *F64F64HashMap {
    ...
    pub fn updateValue(self: *F64F64HashMap, key: f64, initial: f64, f: *const fn (f64) f64) f64 {
    ...
    pub fn format(self: *const F64F64HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const F64F64HashMap, other: *const F64F64HashMap) bool {
...
test "F64F64HashMap: put and get" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
    _ = m.put(3.0, 3.0);
...
test "F64F64HashMap: put overwrite" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
...
test "F64F64HashMap: containsKey and containsValue" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    try std.testing.expect(m.containsKey(1.0));
...
test "F64F64HashMap: getOrDefault" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
...
test "F64F64HashMap: select and reject" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F64HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F64HashMap: keysToSlice and valuesToSlice" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F64HashMap: sumOfValues" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F64HashMap: toImmutable" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F64HashMap: fluent withKeyValue/withoutKey" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F64HashMap: eql" {
    var m1 = F64F64HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1.0);
    _ = m1.put(2.0, 2.0);
    var m2 = F64F64HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2.0);
    _ = m2.put(1.0, 1.0);
...
test "F64F64HashMap: resize" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(f64, @floatFromInt(i)), @as(f64, @floatFromInt(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "F64F64HashMap: entry orInsert" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F64HashMap: entry andModify orInsert" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F64HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F64HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F64HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64F64HashMap.init(failing.allocator());
    defer m.deinit();
...
test "F64F64HashMap: NaN key findable" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1.0);
    try std.testing.expect(m.containsKey(nan_key));
    try std.testing.expectEqual(@as(?f64, 1.0), m.get(nan_key));
}

test "F64F64HashMap: NaN key replaces, does not duplicate" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1.0);
    ...
    _ = m.put(nan_key, 3.0);
    try std.testing.expectEqual(@as(usize, 1), m.len());
    try std.testing.expectEqual(@as(?f64, 3.0), m.get(nan_key));
}

test "F64F64HashMap: NaN key remove" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1.0);
...
test "F64F64HashMap: -0.0 distinct from +0.0" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    ...
    try std.testing.expectEqual(@as(usize, 2), m.len());
    try std.testing.expectEqual(@as(?f64, 1.0), m.get(pos_zero));
    try std.testing.expectEqual(@as(?f64, 2.0), m.get(neg_zero));
}

test "F64F64HashMap: +/-Infinity keys" {
    var m = F64F64HashMap.init(std.testing.allocator);
    defer m.deinit();
    ...
    try std.testing.expectEqual(@as(?f64, 1.0), m.get(pos_inf));
    try std.testing.expectEqual(@as(?f64, 2.0), m.get(neg_inf));
}
