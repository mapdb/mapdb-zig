...
const I8BoolHashBiMap = @import("i8_bool_hash_bi_map.zig").I8BoolHashBiMap;

/// Bidirectional hash map from `bool` keys to `i8` values.
///
...
/// maps in sync.
pub const BoolI8HashBiMap = struct {
    forward: OpenHashMap(bool, i8),
    reverse: OpenHashMap(i8, bool),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) BoolI8HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) BoolI8HashBiMap {
        return .{
            .forward = OpenHashMap(bool, i8).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i8, bool).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *BoolI8HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *BoolI8HashBiMap, key: bool, value: i8) ?i8 {
    ...
    pub fn removeValue(self: *BoolI8HashBiMap, value: i8) ?bool {
    ...
    pub fn containsKey(self: *const BoolI8HashBiMap, key: bool) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const BoolI8HashBiMap, value: i8) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const BoolI8HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const BoolI8HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *BoolI8HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *BoolI8HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolI8HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolI8HashBiMap, f: *const fn (bool, i8) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const BoolI8HashBiMap, f: *const fn (bool) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const BoolI8HashBiMap, f: *const fn (i8) void) void {
    ...
    pub fn inverse(self: *const BoolI8HashBiMap) I8BoolHashBiMap {
        var result = I8BoolHashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const BoolI8HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const BoolI8HashBiMap, other: *const BoolI8HashBiMap) bool {
...
test "BoolI8HashBiMap: put and get and getKey" {
    var m = BoolI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
    // forward lookup
    try std.testing.expectEqual(@as(?i8, 1), m.get(true));
    try std.testing.expectEqual(@as(?i8, 2), m.get(false));
    // reverse lookup
...
test "BoolI8HashBiMap: put overwrite updates reverse" {
    var m = BoolI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
...
test "BoolI8HashBiMap: removeValue" {
    var m = BoolI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI8HashBiMap: containsKey and containsValue" {
    var m = BoolI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    try std.testing.expect(m.containsKey(true));
...
test "BoolI8HashBiMap: display format" {
    var m = BoolI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
...
test "BoolI8HashBiMap: eql" {
    var m1 = BoolI8HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(true, 1);
    _ = m1.put(false, 2);
    var m2 = BoolI8HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(false, 2);
    _ = m2.put(true, 1);
...
test "BoolI8HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = BoolI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = BoolI8HashBiMap.init(failing.allocator());
    defer m.deinit();
