...
const ImmutableI16F32HashMap = @import("../immutable/immutable_i16_f32_hash_map.zig").ImmutableI16F32HashMap;

/// Hash map from `i16` keys to `f32` values.
///
...
pub const I16F32HashMap = struct {
    inner: OpenHashMap(i16, f32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I16F32HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) I16F32HashMap {
        return .{
            .inner = OpenHashMap(i16, f32).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I16F32HashMap {
        return .{
            .inner = OpenHashMap(i16, f32).init(
    ...
    pub fn deinit(self: *I16F32HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *I16F32HashMap, key: i16, value: f32) ?f32 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const I16F32HashMap, key: i16) ?f32 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const I16F32HashMap, key: i16, default_value: f32) f32 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *I16F32HashMap, key: i16) ?f32 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const I16F32HashMap, key: i16) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const I16F32HashMap, value: f32) bool {
    ...
    pub fn getEntry(self: *I16F32HashMap, key: i16) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const I16F32HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const I16F32HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const I16F32HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *I16F32HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I16F32HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16F32HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16F32HashMap, f: *const fn (i16, f32) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const I16F32HashMap, f: *const fn (i16) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const I16F32HashMap, f: *const fn (f32) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I16F32HashMap, predicate: *const fn (i16, f32) bool) I16F32HashMap {
    ...
    pub fn reject(self: *const I16F32HashMap, predicate: *const fn (i16, f32) bool) I16F32HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const I16F32HashMap, predicate: *const fn (i16, f32) bool) ?struct { key: i16, value: f32 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const I16F32HashMap, predicate: *const fn (i16, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const I16F32HashMap, predicate: *const fn (i16, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const I16F32HashMap, predicate: *const fn (i16, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I16F32HashMap, predicate: *const fn (i16, f32) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const I16F32HashMap, initial: f32, f: *const fn (f32, i16, f32) f32) f32 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const I16F32HashMap, allocator: Allocator) []i16 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const I16F32HashMap, allocator: Allocator) []f32 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const I16F32HashMap) f32 {
        var total: f32 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *I16F32HashMap, key: i16, delta: f32) f32 {
    ...
    pub fn toImmutable(self: *const I16F32HashMap) ImmutableI16F32HashMap {
        return ImmutableI16F32HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *I16F32HashMap, key: i16, value: f32) *I16F32HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I16F32HashMap, key: i16) *I16F32HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *I16F32HashMap, keys: []const i16) *I16F32HashMap {
    ...
    pub fn updateValue(self: *I16F32HashMap, key: i16, initial: f32, f: *const fn (f32) f32) f32 {
    ...
    pub fn format(self: *const I16F32HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const I16F32HashMap, other: *const I16F32HashMap) bool {
...
test "I16F32HashMap: put and get" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
    _ = m.put(3, 3.0);
...
test "I16F32HashMap: put overwrite" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
...
test "I16F32HashMap: containsKey and containsValue" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    try std.testing.expect(m.containsKey(1));
...
test "I16F32HashMap: getOrDefault" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
...
test "I16F32HashMap: select and reject" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
...
test "I16F32HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
...
test "I16F32HashMap: keysToSlice and valuesToSlice" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
...
test "I16F32HashMap: sumOfValues" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
...
test "I16F32HashMap: toImmutable" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
...
test "I16F32HashMap: fluent withKeyValue/withoutKey" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F32HashMap: eql" {
    var m1 = I16F32HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1.0);
    _ = m1.put(2, 2.0);
    var m2 = I16F32HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2.0);
    _ = m2.put(1, 1.0);
...
test "I16F32HashMap: resize" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(i16, @intCast(i)), @as(f32, @floatFromInt(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "I16F32HashMap: entry orInsert" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F32HashMap: entry andModify orInsert" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F32HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F32HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = I16F32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F32HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16F32HashMap.init(failing.allocator());
    defer m.deinit();
