...
const ImmutableBoolI16HashMap = @import("../immutable/immutable_bool_i16_hash_map.zig").ImmutableBoolI16HashMap;

/// Hash map from `bool` keys to `i16` values.
///
...
pub const BoolI16HashMap = struct {
    inner: OpenHashMap(bool, i16),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) BoolI16HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) BoolI16HashMap {
        return .{
            .inner = OpenHashMap(bool, i16).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) BoolI16HashMap {
        return .{
            .inner = OpenHashMap(bool, i16).init(
    ...
    pub fn deinit(self: *BoolI16HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *BoolI16HashMap, key: bool, value: i16) ?i16 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const BoolI16HashMap, key: bool) ?i16 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const BoolI16HashMap, key: bool, default_value: i16) i16 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *BoolI16HashMap, key: bool) ?i16 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const BoolI16HashMap, key: bool) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const BoolI16HashMap, value: i16) bool {
    ...
    pub fn getEntry(self: *BoolI16HashMap, key: bool) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const BoolI16HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const BoolI16HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const BoolI16HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *BoolI16HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *BoolI16HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolI16HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolI16HashMap, f: *const fn (bool, i16) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const BoolI16HashMap, f: *const fn (bool) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const BoolI16HashMap, f: *const fn (i16) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const BoolI16HashMap, predicate: *const fn (bool, i16) bool) BoolI16HashMap {
    ...
    pub fn reject(self: *const BoolI16HashMap, predicate: *const fn (bool, i16) bool) BoolI16HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const BoolI16HashMap, predicate: *const fn (bool, i16) bool) ?struct { key: bool, value: i16 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const BoolI16HashMap, predicate: *const fn (bool, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const BoolI16HashMap, predicate: *const fn (bool, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const BoolI16HashMap, predicate: *const fn (bool, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const BoolI16HashMap, predicate: *const fn (bool, i16) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const BoolI16HashMap, initial: i16, f: *const fn (i16, bool, i16) i16) i16 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const BoolI16HashMap, allocator: Allocator) []bool {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const BoolI16HashMap, allocator: Allocator) []i16 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const BoolI16HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *BoolI16HashMap, key: bool, delta: i16) i16 {
    ...
    pub fn toImmutable(self: *const BoolI16HashMap) ImmutableBoolI16HashMap {
        return ImmutableBoolI16HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *BoolI16HashMap, key: bool, value: i16) *BoolI16HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *BoolI16HashMap, key: bool) *BoolI16HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *BoolI16HashMap, keys: []const bool) *BoolI16HashMap {
    ...
    pub fn updateValue(self: *BoolI16HashMap, key: bool, initial: i16, f: *const fn (i16) i16) i16 {
    ...
    pub fn format(self: *const BoolI16HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const BoolI16HashMap, other: *const BoolI16HashMap) bool {
...
test "BoolI16HashMap: put and get" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI16HashMap: put overwrite" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
...
test "BoolI16HashMap: containsKey and containsValue" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    try std.testing.expect(m.containsKey(true));
...
test "BoolI16HashMap: getOrDefault" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
...
test "BoolI16HashMap: keysToSlice and valuesToSlice" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI16HashMap: sumOfValues" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI16HashMap: addToValue" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16HashMap: toImmutable" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI16HashMap: fluent withKeyValue/withoutKey" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16HashMap: eql" {
    var m1 = BoolI16HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(true, 1);
    _ = m1.put(false, 2);
    var m2 = BoolI16HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(false, 2);
    _ = m2.put(true, 1);
...
test "BoolI16HashMap: resize" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(i % 2 == 0, @as(i16, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "BoolI16HashMap: entry orInsert" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16HashMap: entry andModify orInsert" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = BoolI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolI16HashMap.init(failing.allocator());
    defer m.deinit();
