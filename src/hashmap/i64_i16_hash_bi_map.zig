...
const I16I64HashBiMap = @import("i16_i64_hash_bi_map.zig").I16I64HashBiMap;

/// Bidirectional hash map from `i64` keys to `i16` values.
///
...
/// maps in sync.
pub const I64I16HashBiMap = struct {
    forward: OpenHashMap(i64, i16),
    reverse: OpenHashMap(i16, i64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I64I16HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I64I16HashBiMap {
        return .{
            .forward = OpenHashMap(i64, i16).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i16, i64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I64I16HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I64I16HashBiMap, key: i64, value: i16) ?i16 {
    ...
    pub fn removeValue(self: *I64I16HashBiMap, value: i16) ?i64 {
    ...
    pub fn containsKey(self: *const I64I16HashBiMap, key: i64) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I64I16HashBiMap, value: i16) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I64I16HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I64I16HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I64I16HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I64I16HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I64I16HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I64I16HashBiMap, f: *const fn (i64, i16) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I64I16HashBiMap, f: *const fn (i64) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I64I16HashBiMap, f: *const fn (i16) void) void {
    ...
    pub fn inverse(self: *const I64I16HashBiMap) I16I64HashBiMap {
        var result = I16I64HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I64I16HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I64I16HashBiMap, other: *const I64I16HashBiMap) bool {
...
test "I64I16HashBiMap: put and get and getKey" {
    var m = I64I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    // forward lookup
    try std.testing.expectEqual(@as(?i16, 1), m.get(1));
    try std.testing.expectEqual(@as(?i16, 2), m.get(2));
    // reverse lookup
    try std.testing.expectEqual(@as(?i64, 1), m.getKey(1));
    try std.testing.expectEqual(@as(?i64, 2), m.getKey(2));
    // missing key
    try std.testing.expectEqual(@as(?i16, null), m.get(99));
    // missing value
    try std.testing.expectEqual(@as(?i64, null), m.getKey(99));
    try std.testing.expectEqual(@as(usize, 2), m.len());
}

test "I64I16HashBiMap: put overwrite updates reverse" {
    var m = I64I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    ...
    try std.testing.expectEqual(@as(?i64, null), m.getKey(1));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?i64, 1), m.getKey(2));
}

test "I64I16HashBiMap: remove" {
    var m = I64I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I64I16HashBiMap: removeValue" {
    var m = I64I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    const removed_key = m.removeValue(1);
    try std.testing.expectEqual(@as(?i64, 1), removed_key);
    try std.testing.expectEqual(@as(usize, 1), m.len());
    try std.testing.expect(!m.containsValue(1));
    try std.testing.expect(!m.containsKey(1));
}

test "I64I16HashBiMap: containsKey and containsValue" {
    var m = I64I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    try std.testing.expect(m.containsKey(1));
...
test "I64I16HashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?i64, 1), inv.get(1));
    try std.testing.expectEqual(@as(?i64, 2), inv.get(2));
}

test "I64I16HashBiMap: display format" {
    var m = I64I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I64I16HashBiMap: eql" {
    var m1 = I64I16HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);
    var m2 = I64I16HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I64I16HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I64I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64I16HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I64I16HashBiMap.init(failing.allocator());
    defer m.deinit();
