...
const ImmutableF32I32HashMap = @import("../immutable/immutable_f32_i32_hash_map.zig").ImmutableF32I32HashMap;

/// Hash map from `f32` keys to `i32` values.
///
...
pub const F32I32HashMap = struct {
    inner: OpenHashMap(f32, i32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F32I32HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) F32I32HashMap {
        return .{
            .inner = OpenHashMap(f32, i32).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F32I32HashMap {
        return .{
            .inner = OpenHashMap(f32, i32).init(
    ...
    pub fn deinit(self: *F32I32HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *F32I32HashMap, key: f32, value: i32) ?i32 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const F32I32HashMap, key: f32) ?i32 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const F32I32HashMap, key: f32, default_value: i32) i32 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *F32I32HashMap, key: f32) ?i32 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const F32I32HashMap, key: f32) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const F32I32HashMap, value: i32) bool {
    ...
    pub fn getEntry(self: *F32I32HashMap, key: f32) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const F32I32HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const F32I32HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const F32I32HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *F32I32HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F32I32HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F32I32HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32I32HashMap, f: *const fn (f32, i32) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const F32I32HashMap, f: *const fn (f32) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const F32I32HashMap, f: *const fn (i32) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F32I32HashMap, predicate: *const fn (f32, i32) bool) F32I32HashMap {
    ...
    pub fn reject(self: *const F32I32HashMap, predicate: *const fn (f32, i32) bool) F32I32HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const F32I32HashMap, predicate: *const fn (f32, i32) bool) ?struct { key: f32, value: i32 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const F32I32HashMap, predicate: *const fn (f32, i32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const F32I32HashMap, predicate: *const fn (f32, i32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const F32I32HashMap, predicate: *const fn (f32, i32) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F32I32HashMap, predicate: *const fn (f32, i32) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const F32I32HashMap, initial: i32, f: *const fn (i32, f32, i32) i32) i32 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const F32I32HashMap, allocator: Allocator) []f32 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const F32I32HashMap, allocator: Allocator) []i32 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const F32I32HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *F32I32HashMap, key: f32, delta: i32) i32 {
    ...
    pub fn toImmutable(self: *const F32I32HashMap) ImmutableF32I32HashMap {
        return ImmutableF32I32HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *F32I32HashMap, key: f32, value: i32) *F32I32HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F32I32HashMap, key: f32) *F32I32HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *F32I32HashMap, keys: []const f32) *F32I32HashMap {
    ...
    pub fn updateValue(self: *F32I32HashMap, key: f32, initial: i32, f: *const fn (i32) i32) i32 {
    ...
    pub fn format(self: *const F32I32HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const F32I32HashMap, other: *const F32I32HashMap) bool {
...
test "F32I32HashMap: put and get" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
    _ = m.put(3.0, 3);
...
test "F32I32HashMap: put overwrite" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F32I32HashMap: containsKey and containsValue" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    try std.testing.expect(m.containsKey(1.0));
...
test "F32I32HashMap: getOrDefault" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F32I32HashMap: select and reject" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I32HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I32HashMap: keysToSlice and valuesToSlice" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I32HashMap: sumOfValues" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I32HashMap: toImmutable" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I32HashMap: fluent withKeyValue/withoutKey" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I32HashMap: eql" {
    var m1 = F32I32HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1);
    _ = m1.put(2.0, 2);
    var m2 = F32I32HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2);
    _ = m2.put(1.0, 1);
...
test "F32I32HashMap: resize" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(f32, @floatFromInt(i)), @as(i32, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "F32I32HashMap: entry orInsert" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I32HashMap: entry andModify orInsert" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I32HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I32HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I32HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F32I32HashMap.init(failing.allocator());
    defer m.deinit();
...
test "F32I32HashMap: NaN key findable" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f32);
    _ = m.put(nan_key, 1);
    try std.testing.expect(m.containsKey(nan_key));
    try std.testing.expectEqual(@as(?i32, 1), m.get(nan_key));
}

test "F32I32HashMap: NaN key replaces, does not duplicate" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f32);
    _ = m.put(nan_key, 1);
    ...
    _ = m.put(nan_key, 3);
    try std.testing.expectEqual(@as(usize, 1), m.len());
    try std.testing.expectEqual(@as(?i32, 3), m.get(nan_key));
}

test "F32I32HashMap: NaN key remove" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f32);
    _ = m.put(nan_key, 1);
...
test "F32I32HashMap: -0.0 distinct from +0.0" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    ...
    _ = m.put(neg_zero, 2);
    try std.testing.expectEqual(@as(usize, 2), m.len());
    try std.testing.expectEqual(@as(?i32, 1), m.get(pos_zero));
    try std.testing.expectEqual(@as(?i32, 2), m.get(neg_zero));
}

test "F32I32HashMap: +/-Infinity keys" {
    var m = F32I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    ...
    try std.testing.expectEqual(@as(?i32, 1), m.get(pos_inf));
    try std.testing.expectEqual(@as(?i32, 2), m.get(neg_inf));
}
