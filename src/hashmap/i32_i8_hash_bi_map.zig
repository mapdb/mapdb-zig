...
const I8I32HashBiMap = @import("i8_i32_hash_bi_map.zig").I8I32HashBiMap;

/// Bidirectional hash map from `i32` keys to `i8` values.
///
...
/// maps in sync.
pub const I32I8HashBiMap = struct {
    forward: OpenHashMap(i32, i8),
    reverse: OpenHashMap(i8, i32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I32I8HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I32I8HashBiMap {
        return .{
            .forward = OpenHashMap(i32, i8).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i8, i32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I32I8HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I32I8HashBiMap, key: i32, value: i8) ?i8 {
    ...
    pub fn removeValue(self: *I32I8HashBiMap, value: i8) ?i32 {
    ...
    pub fn containsKey(self: *const I32I8HashBiMap, key: i32) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I32I8HashBiMap, value: i8) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I32I8HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I32I8HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I32I8HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I32I8HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32I8HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32I8HashBiMap, f: *const fn (i32, i8) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I32I8HashBiMap, f: *const fn (i32) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I32I8HashBiMap, f: *const fn (i8) void) void {
    ...
    pub fn inverse(self: *const I32I8HashBiMap) I8I32HashBiMap {
        var result = I8I32HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I32I8HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I32I8HashBiMap, other: *const I32I8HashBiMap) bool {
...
test "I32I8HashBiMap: put and get and getKey" {
    var m = I32I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    // forward lookup
...
test "I32I8HashBiMap: put overwrite updates reverse" {
    var m = I32I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I32I8HashBiMap: removeValue" {
    var m = I32I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I32I8HashBiMap: containsKey and containsValue" {
    var m = I32I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    try std.testing.expect(m.containsKey(1));
...
test "I32I8HashBiMap: display format" {
    var m = I32I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I32I8HashBiMap: eql" {
    var m1 = I32I8HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);
    var m2 = I32I8HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I32I8HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I32I8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I8HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I32I8HashBiMap.init(failing.allocator());
    defer m.deinit();
