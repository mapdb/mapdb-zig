...
const BoolI64HashBiMap = @import("bool_i64_hash_bi_map.zig").BoolI64HashBiMap;

/// Bidirectional hash map from `i64` keys to `bool` values.
///
...
/// maps in sync.
pub const I64BoolHashBiMap = struct {
    forward: OpenHashMap(i64, bool),
    reverse: OpenHashMap(bool, i64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I64BoolHashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I64BoolHashBiMap {
        return .{
            .forward = OpenHashMap(i64, bool).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(bool, i64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I64BoolHashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I64BoolHashBiMap, key: i64, value: bool) ?bool {
    ...
    pub fn removeValue(self: *I64BoolHashBiMap, value: bool) ?i64 {
    ...
    pub fn containsKey(self: *const I64BoolHashBiMap, key: i64) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I64BoolHashBiMap, value: bool) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I64BoolHashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I64BoolHashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I64BoolHashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I64BoolHashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I64BoolHashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I64BoolHashBiMap, f: *const fn (i64, bool) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I64BoolHashBiMap, f: *const fn (i64) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I64BoolHashBiMap, f: *const fn (bool) void) void {
    ...
    pub fn inverse(self: *const I64BoolHashBiMap) BoolI64HashBiMap {
        var result = BoolI64HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I64BoolHashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I64BoolHashBiMap, other: *const I64BoolHashBiMap) bool {
...
test "I64BoolHashBiMap: put and get and getKey" {
    var m = I64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
    // forward lookup
    ...
    // reverse lookup
    try std.testing.expectEqual(@as(?i64, 1), m.getKey(true));
    try std.testing.expectEqual(@as(?i64, 2), m.getKey(false));
    // missing key
...
test "I64BoolHashBiMap: put overwrite updates reverse" {
    var m = I64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    ...
    try std.testing.expectEqual(@as(?i64, null), m.getKey(true));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?i64, 1), m.getKey(false));
}

test "I64BoolHashBiMap: remove" {
    var m = I64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
...
test "I64BoolHashBiMap: removeValue" {
    var m = I64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
...
test "I64BoolHashBiMap: containsKey and containsValue" {
    var m = I64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    try std.testing.expect(m.containsKey(1));
...
test "I64BoolHashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?i64, 1), inv.get(true));
    try std.testing.expectEqual(@as(?i64, 2), inv.get(false));
}

test "I64BoolHashBiMap: display format" {
    var m = I64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
...
test "I64BoolHashBiMap: eql" {
    var m1 = I64BoolHashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, true);
    _ = m1.put(2, false);
    var m2 = I64BoolHashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, false);
    _ = m2.put(1, true);
...
test "I64BoolHashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64BoolHashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I64BoolHashBiMap.init(failing.allocator());
    defer m.deinit();
