...
const BoolI8HashBiMap = @import("bool_i8_hash_bi_map.zig").BoolI8HashBiMap;
...
/// maps in sync.
pub const I8BoolHashBiMap = struct {
    ...
    pub fn init(allocator: Allocator) I8BoolHashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I8BoolHashBiMap {
        return .{
            .forward = OpenHashMap(i8, bool).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(bool, i8).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I8BoolHashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I8BoolHashBiMap, key: i8, value: bool) ?bool {
    ...
    pub fn removeValue(self: *I8BoolHashBiMap, value: bool) ?i8 {
    ...
    pub fn containsKey(self: *const I8BoolHashBiMap, key: i8) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I8BoolHashBiMap, value: bool) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I8BoolHashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I8BoolHashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I8BoolHashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I8BoolHashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8BoolHashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8BoolHashBiMap, f: *const fn (i8, bool) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I8BoolHashBiMap, f: *const fn (i8) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I8BoolHashBiMap, f: *const fn (bool) void) void {
    ...
    pub fn inverse(self: *const I8BoolHashBiMap) BoolI8HashBiMap {
        var result = BoolI8HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I8BoolHashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I8BoolHashBiMap, other: *const I8BoolHashBiMap) bool {
...
test "I8BoolHashBiMap: put and get and getKey" {
    var m = I8BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
    // forward lookup
    ...
    // reverse lookup
    try std.testing.expectEqual(@as(?i8, 1), m.getKey(true));
    try std.testing.expectEqual(@as(?i8, 2), m.getKey(false));
    // missing key
...
test "I8BoolHashBiMap: put overwrite updates reverse" {
    var m = I8BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    ...
    try std.testing.expectEqual(@as(?i8, null), m.getKey(true));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?i8, 1), m.getKey(false));
}

test "I8BoolHashBiMap: remove" {
    var m = I8BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
...
test "I8BoolHashBiMap: removeValue" {
    var m = I8BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
...
test "I8BoolHashBiMap: containsKey and containsValue" {
    var m = I8BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    try std.testing.expect(m.containsKey(1));
...
test "I8BoolHashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?i8, 1), inv.get(true));
    try std.testing.expectEqual(@as(?i8, 2), inv.get(false));
}

test "I8BoolHashBiMap: display format" {
    var m = I8BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
...
test "I8BoolHashBiMap: eql" {
    var m1 = I8BoolHashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, true);
    _ = m1.put(2, false);
    var m2 = I8BoolHashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, false);
    _ = m2.put(1, true);
...
test "I8BoolHashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I8BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolHashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I8BoolHashBiMap.init(failing.allocator());
    defer m.deinit();
