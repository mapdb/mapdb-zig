...
const ImmutableCharI8HashMap = @import("../immutable/immutable_char_i8_hash_map.zig").ImmutableCharI8HashMap;

/// Hash map from `u21` keys to `i8` values.
///
...
pub const CharI8HashMap = struct {
    inner: OpenHashMap(u21, i8),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharI8HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) CharI8HashMap {
        return .{
            .inner = OpenHashMap(u21, i8).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) CharI8HashMap {
        return .{
            .inner = OpenHashMap(u21, i8).init(
    ...
    pub fn deinit(self: *CharI8HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *CharI8HashMap, key: u21, value: i8) ?i8 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const CharI8HashMap, key: u21) ?i8 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const CharI8HashMap, key: u21, default_value: i8) i8 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *CharI8HashMap, key: u21) ?i8 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const CharI8HashMap, key: u21) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const CharI8HashMap, value: i8) bool {
    ...
    pub fn getEntry(self: *CharI8HashMap, key: u21) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const CharI8HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const CharI8HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const CharI8HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *CharI8HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *CharI8HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI8HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI8HashMap, f: *const fn (u21, i8) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const CharI8HashMap, f: *const fn (u21) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const CharI8HashMap, f: *const fn (i8) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const CharI8HashMap, predicate: *const fn (u21, i8) bool) CharI8HashMap {
    ...
    pub fn reject(self: *const CharI8HashMap, predicate: *const fn (u21, i8) bool) CharI8HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const CharI8HashMap, predicate: *const fn (u21, i8) bool) ?struct { key: u21, value: i8 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const CharI8HashMap, predicate: *const fn (u21, i8) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const CharI8HashMap, predicate: *const fn (u21, i8) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const CharI8HashMap, predicate: *const fn (u21, i8) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const CharI8HashMap, predicate: *const fn (u21, i8) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const CharI8HashMap, initial: i8, f: *const fn (i8, u21, i8) i8) i8 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const CharI8HashMap, allocator: Allocator) []u21 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const CharI8HashMap, allocator: Allocator) []i8 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const CharI8HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *CharI8HashMap, key: u21, delta: i8) i8 {
    ...
    pub fn toImmutable(self: *const CharI8HashMap) ImmutableCharI8HashMap {
        return ImmutableCharI8HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *CharI8HashMap, key: u21, value: i8) *CharI8HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *CharI8HashMap, key: u21) *CharI8HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *CharI8HashMap, keys: []const u21) *CharI8HashMap {
    ...
    pub fn updateValue(self: *CharI8HashMap, key: u21, initial: i8, f: *const fn (i8) i8) i8 {
    ...
    pub fn format(self: *const CharI8HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const CharI8HashMap, other: *const CharI8HashMap) bool {
...
test "CharI8HashMap: put and get" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
    _ = m.put('c', 3);
    try std.testing.expectEqual(@as(?i8, 1), m.get('a'));
    try std.testing.expectEqual(@as(?i8, null), m.get('z'));
    try std.testing.expectEqual(@as(usize, 3), m.len());
}

test "CharI8HashMap: put overwrite" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    const old = m.put('a', 2);
    try std.testing.expectEqual(@as(?i8, 1), old);
    try std.testing.expectEqual(@as(?i8, 2), m.get('a'));
}

test "CharI8HashMap: remove" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI8HashMap: containsKey and containsValue" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    try std.testing.expect(m.containsKey('a'));
...
test "CharI8HashMap: getOrDefault" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
...
test "CharI8HashMap: select and reject" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI8HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI8HashMap: keysToSlice and valuesToSlice" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI8HashMap: sumOfValues" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI8HashMap: toImmutable" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI8HashMap: fluent withKeyValue/withoutKey" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8HashMap: eql" {
    var m1 = CharI8HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1);
    _ = m1.put('b', 2);
    var m2 = CharI8HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2);
    _ = m2.put('a', 1);
...
test "CharI8HashMap: resize" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(u21, @intCast(i % 26)) + 'a', @as(i8, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "CharI8HashMap: entry orInsert" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8HashMap: entry andModify orInsert" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = CharI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI8HashMap.init(failing.allocator());
    defer m.deinit();
