...
const F32F64HashBiMap = @import("f32_f64_hash_bi_map.zig").F32F64HashBiMap;

/// Bidirectional hash map from `f64` keys to `f32` values.
///
...
/// maps in sync.
pub const F64F32HashBiMap = struct {
    forward: OpenHashMap(f64, f32),
    reverse: OpenHashMap(f32, f64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64F32HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F64F32HashBiMap {
        return .{
            .forward = OpenHashMap(f64, f32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(f32, f64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *F64F32HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *F64F32HashBiMap, key: f64, value: f32) ?f32 {
    ...
    pub fn removeValue(self: *F64F32HashBiMap, value: f32) ?f64 {
    ...
    pub fn containsKey(self: *const F64F32HashBiMap, key: f64) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const F64F32HashBiMap, value: f32) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const F64F32HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const F64F32HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *F64F32HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F64F32HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64F32HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64F32HashBiMap, f: *const fn (f64, f32) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const F64F32HashBiMap, f: *const fn (f64) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const F64F32HashBiMap, f: *const fn (f32) void) void {
    ...
    pub fn inverse(self: *const F64F32HashBiMap) F32F64HashBiMap {
        var result = F32F64HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const F64F32HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const F64F32HashBiMap, other: *const F64F32HashBiMap) bool {
...
test "F64F32HashBiMap: put and get and getKey" {
    var m = F64F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
    // forward lookup
...
test "F64F32HashBiMap: put overwrite updates reverse" {
    var m = F64F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
...
test "F64F32HashBiMap: removeValue" {
    var m = F64F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F32HashBiMap: containsKey and containsValue" {
    var m = F64F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    try std.testing.expect(m.containsKey(1.0));
...
test "F64F32HashBiMap: display format" {
    var m = F64F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
...
test "F64F32HashBiMap: eql" {
    var m1 = F64F32HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1.0);
    _ = m1.put(2.0, 2.0);
    var m2 = F64F32HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2.0);
    _ = m2.put(1.0, 1.0);
...
test "F64F32HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = F64F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F32HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = F64F32HashBiMap.init(failing.allocator());
    defer m.deinit();
