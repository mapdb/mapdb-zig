...
const I16CharHashBiMap = @import("i16_char_hash_bi_map.zig").I16CharHashBiMap;

/// Bidirectional hash map from `u21` keys to `i16` values.
///
...
/// maps in sync.
pub const CharI16HashBiMap = struct {
    forward: OpenHashMap(u21, i16),
    reverse: OpenHashMap(i16, u21),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharI16HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) CharI16HashBiMap {
        return .{
            .forward = OpenHashMap(u21, i16).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i16, u21).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *CharI16HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *CharI16HashBiMap, key: u21, value: i16) ?i16 {
    ...
    pub fn removeValue(self: *CharI16HashBiMap, value: i16) ?u21 {
    ...
    pub fn containsKey(self: *const CharI16HashBiMap, key: u21) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const CharI16HashBiMap, value: i16) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const CharI16HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const CharI16HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *CharI16HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *CharI16HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI16HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI16HashBiMap, f: *const fn (u21, i16) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const CharI16HashBiMap, f: *const fn (u21) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const CharI16HashBiMap, f: *const fn (i16) void) void {
    ...
    pub fn inverse(self: *const CharI16HashBiMap) I16CharHashBiMap {
        var result = I16CharHashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const CharI16HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const CharI16HashBiMap, other: *const CharI16HashBiMap) bool {
...
test "CharI16HashBiMap: put and get and getKey" {
    var m = CharI16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
    // forward lookup
    try std.testing.expectEqual(@as(?i16, 1), m.get('a'));
    try std.testing.expectEqual(@as(?i16, 2), m.get('b'));
    // reverse lookup
...
test "CharI16HashBiMap: put overwrite updates reverse" {
    var m = CharI16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
...
test "CharI16HashBiMap: removeValue" {
    var m = CharI16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI16HashBiMap: containsKey and containsValue" {
    var m = CharI16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    try std.testing.expect(m.containsKey('a'));
...
test "CharI16HashBiMap: display format" {
    var m = CharI16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
...
test "CharI16HashBiMap: eql" {
    var m1 = CharI16HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1);
    _ = m1.put('b', 2);
    var m2 = CharI16HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2);
    _ = m2.put('a', 1);
...
test "CharI16HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = CharI16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI16HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = CharI16HashBiMap.init(failing.allocator());
    defer m.deinit();
