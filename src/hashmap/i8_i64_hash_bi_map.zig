...
const I64I8HashBiMap = @import("i64_i8_hash_bi_map.zig").I64I8HashBiMap;

/// Bidirectional hash map from `i8` keys to `i64` values.
///
...
/// maps in sync.
pub const I8I64HashBiMap = struct {
    forward: OpenHashMap(i8, i64),
    reverse: OpenHashMap(i64, i8),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I8I64HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I8I64HashBiMap {
        return .{
            .forward = OpenHashMap(i8, i64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i64, i8).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I8I64HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I8I64HashBiMap, key: i8, value: i64) ?i64 {
    ...
    pub fn removeValue(self: *I8I64HashBiMap, value: i64) ?i8 {
    ...
    pub fn containsKey(self: *const I8I64HashBiMap, key: i8) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I8I64HashBiMap, value: i64) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I8I64HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I8I64HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I8I64HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I8I64HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8I64HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8I64HashBiMap, f: *const fn (i8, i64) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I8I64HashBiMap, f: *const fn (i8) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I8I64HashBiMap, f: *const fn (i64) void) void {
    ...
    pub fn inverse(self: *const I8I64HashBiMap) I64I8HashBiMap {
        var result = I64I8HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I8I64HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I8I64HashBiMap, other: *const I8I64HashBiMap) bool {
...
test "I8I64HashBiMap: put and get and getKey" {
    var m = I8I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    // forward lookup
...
test "I8I64HashBiMap: put overwrite updates reverse" {
    var m = I8I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I8I64HashBiMap: removeValue" {
    var m = I8I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I8I64HashBiMap: containsKey and containsValue" {
    var m = I8I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    try std.testing.expect(m.containsKey(1));
...
test "I8I64HashBiMap: display format" {
    var m = I8I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I8I64HashBiMap: eql" {
    var m1 = I8I64HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);
    var m2 = I8I64HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I8I64HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I8I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8I64HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I8I64HashBiMap.init(failing.allocator());
    defer m.deinit();
