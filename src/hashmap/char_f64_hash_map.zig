...
const ImmutableCharF64HashMap = @import("../immutable/immutable_char_f64_hash_map.zig").ImmutableCharF64HashMap;

/// Hash map from `u21` keys to `f64` values.
///
...
pub const CharF64HashMap = struct {
    inner: OpenHashMap(u21, f64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharF64HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) CharF64HashMap {
        return .{
            .inner = OpenHashMap(u21, f64).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) CharF64HashMap {
        return .{
            .inner = OpenHashMap(u21, f64).init(
    ...
    pub fn deinit(self: *CharF64HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *CharF64HashMap, key: u21, value: f64) ?f64 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const CharF64HashMap, key: u21) ?f64 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const CharF64HashMap, key: u21, default_value: f64) f64 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *CharF64HashMap, key: u21) ?f64 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const CharF64HashMap, key: u21) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const CharF64HashMap, value: f64) bool {
    ...
    pub fn getEntry(self: *CharF64HashMap, key: u21) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const CharF64HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const CharF64HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const CharF64HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *CharF64HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *CharF64HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharF64HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharF64HashMap, f: *const fn (u21, f64) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const CharF64HashMap, f: *const fn (u21) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const CharF64HashMap, f: *const fn (f64) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const CharF64HashMap, predicate: *const fn (u21, f64) bool) CharF64HashMap {
    ...
    pub fn reject(self: *const CharF64HashMap, predicate: *const fn (u21, f64) bool) CharF64HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const CharF64HashMap, predicate: *const fn (u21, f64) bool) ?struct { key: u21, value: f64 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const CharF64HashMap, predicate: *const fn (u21, f64) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const CharF64HashMap, predicate: *const fn (u21, f64) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const CharF64HashMap, predicate: *const fn (u21, f64) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const CharF64HashMap, predicate: *const fn (u21, f64) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const CharF64HashMap, initial: f64, f: *const fn (f64, u21, f64) f64) f64 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const CharF64HashMap, allocator: Allocator) []u21 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const CharF64HashMap, allocator: Allocator) []f64 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const CharF64HashMap) f64 {
        var total: f64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *CharF64HashMap, key: u21, delta: f64) f64 {
    ...
    pub fn toImmutable(self: *const CharF64HashMap) ImmutableCharF64HashMap {
        return ImmutableCharF64HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *CharF64HashMap, key: u21, value: f64) *CharF64HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *CharF64HashMap, key: u21) *CharF64HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *CharF64HashMap, keys: []const u21) *CharF64HashMap {
    ...
    pub fn updateValue(self: *CharF64HashMap, key: u21, initial: f64, f: *const fn (f64) f64) f64 {
    ...
    pub fn format(self: *const CharF64HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const CharF64HashMap, other: *const CharF64HashMap) bool {
...
test "CharF64HashMap: put and get" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
    _ = m.put('c', 3.0);
    try std.testing.expectEqual(@as(?f64, 1.0), m.get('a'));
    try std.testing.expectEqual(@as(?f64, null), m.get('z'));
    try std.testing.expectEqual(@as(usize, 3), m.len());
}

test "CharF64HashMap: put overwrite" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    const old = m.put('a', 2.0);
    try std.testing.expectEqual(@as(?f64, 1.0), old);
    try std.testing.expectEqual(@as(?f64, 2.0), m.get('a'));
}

test "CharF64HashMap: remove" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
...
test "CharF64HashMap: containsKey and containsValue" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    try std.testing.expect(m.containsKey('a'));
...
test "CharF64HashMap: getOrDefault" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    try std.testing.expectEqual(1.0, m.getOrDefault('a', 3.0));
    try std.testing.expectEqual(3.0, m.getOrDefault('z', 3.0));
}

test "CharF64HashMap: clear" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    m.clear();
...
test "CharF64HashMap: select and reject" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
...
test "CharF64HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
...
test "CharF64HashMap: keysToSlice and valuesToSlice" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
...
test "CharF64HashMap: sumOfValues" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
...
test "CharF64HashMap: addToValue" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.addToValue('a', 1.0);
    _ = m.addToValue('a', 2.0);
    // Value should be sum of the two adds
    try std.testing.expect(m.get('a') != null);
}

test "CharF64HashMap: toImmutable" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
...
test "CharF64HashMap: fluent withKeyValue/withoutKey" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.withKeyValue('a', 1.0).withKeyValue('b', 2.0);
    try std.testing.expectEqual(@as(usize, 2), m.len());
    _ = m.withoutKey('a');
    try std.testing.expectEqual(@as(usize, 1), m.len());
}

test "CharF64HashMap: eql" {
    var m1 = CharF64HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1.0);
    _ = m1.put('b', 2.0);
    var m2 = CharF64HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2.0);
    _ = m2.put('a', 1.0);
...
test "CharF64HashMap: resize" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(u21, @intCast(i % 26)) + 'a', @as(f64, @floatFromInt(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "CharF64HashMap: entry orInsert" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
    // Key absent — inserts default
    const v1 = m.getEntry('a').orInsert(1.0);
    try std.testing.expectEqual(1.0, v1);
    // Key present — returns existing
    _ = m.put('a', 2.0);
    const v2 = m.getEntry('a').orInsert(3.0);
    try std.testing.expectEqual(2.0, v2);
}

test "CharF64HashMap: entry andModify orInsert" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF64HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF64HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = CharF64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF64HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharF64HashMap.init(failing.allocator());
    defer m.deinit();
