...
const ImmutableBoolF32HashMap = @import("../immutable/immutable_bool_f32_hash_map.zig").ImmutableBoolF32HashMap;

/// Hash map from `bool` keys to `f32` values.
///
...
pub const BoolF32HashMap = struct {
    inner: OpenHashMap(bool, f32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) BoolF32HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) BoolF32HashMap {
        return .{
            .inner = OpenHashMap(bool, f32).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) BoolF32HashMap {
        return .{
            .inner = OpenHashMap(bool, f32).init(
    ...
    pub fn deinit(self: *BoolF32HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *BoolF32HashMap, key: bool, value: f32) ?f32 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const BoolF32HashMap, key: bool) ?f32 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const BoolF32HashMap, key: bool, default_value: f32) f32 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *BoolF32HashMap, key: bool) ?f32 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const BoolF32HashMap, key: bool) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const BoolF32HashMap, value: f32) bool {
    ...
    pub fn getEntry(self: *BoolF32HashMap, key: bool) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const BoolF32HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const BoolF32HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const BoolF32HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *BoolF32HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *BoolF32HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolF32HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolF32HashMap, f: *const fn (bool, f32) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const BoolF32HashMap, f: *const fn (bool) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const BoolF32HashMap, f: *const fn (f32) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const BoolF32HashMap, predicate: *const fn (bool, f32) bool) BoolF32HashMap {
    ...
    pub fn reject(self: *const BoolF32HashMap, predicate: *const fn (bool, f32) bool) BoolF32HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const BoolF32HashMap, predicate: *const fn (bool, f32) bool) ?struct { key: bool, value: f32 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const BoolF32HashMap, predicate: *const fn (bool, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const BoolF32HashMap, predicate: *const fn (bool, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const BoolF32HashMap, predicate: *const fn (bool, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const BoolF32HashMap, predicate: *const fn (bool, f32) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const BoolF32HashMap, initial: f32, f: *const fn (f32, bool, f32) f32) f32 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const BoolF32HashMap, allocator: Allocator) []bool {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const BoolF32HashMap, allocator: Allocator) []f32 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const BoolF32HashMap) f32 {
        var total: f32 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *BoolF32HashMap, key: bool, delta: f32) f32 {
    ...
    pub fn toImmutable(self: *const BoolF32HashMap) ImmutableBoolF32HashMap {
        return ImmutableBoolF32HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *BoolF32HashMap, key: bool, value: f32) *BoolF32HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *BoolF32HashMap, key: bool) *BoolF32HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *BoolF32HashMap, keys: []const bool) *BoolF32HashMap {
    ...
    pub fn updateValue(self: *BoolF32HashMap, key: bool, initial: f32, f: *const fn (f32) f32) f32 {
    ...
    pub fn format(self: *const BoolF32HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const BoolF32HashMap, other: *const BoolF32HashMap) bool {
...
test "BoolF32HashMap: put and get" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
    _ = m.put(false, 2.0);
...
test "BoolF32HashMap: put overwrite" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
...
test "BoolF32HashMap: containsKey and containsValue" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
    try std.testing.expect(m.containsKey(true));
...
test "BoolF32HashMap: getOrDefault" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
...
test "BoolF32HashMap: keysToSlice and valuesToSlice" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
    _ = m.put(false, 2.0);
...
test "BoolF32HashMap: sumOfValues" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
    _ = m.put(false, 2.0);
...
test "BoolF32HashMap: addToValue" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32HashMap: toImmutable" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
    _ = m.put(false, 2.0);
...
test "BoolF32HashMap: fluent withKeyValue/withoutKey" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32HashMap: eql" {
    var m1 = BoolF32HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(true, 1.0);
    _ = m1.put(false, 2.0);
    var m2 = BoolF32HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(false, 2.0);
    _ = m2.put(true, 1.0);
...
test "BoolF32HashMap: resize" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(i % 2 == 0, @as(f32, @floatFromInt(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "BoolF32HashMap: entry orInsert" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32HashMap: entry andModify orInsert" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = BoolF32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolF32HashMap.init(failing.allocator());
    defer m.deinit();
