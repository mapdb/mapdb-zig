...
const BoolF32HashBiMap = @import("bool_f32_hash_bi_map.zig").BoolF32HashBiMap;
...
/// maps in sync.
pub const F32BoolHashBiMap = struct {
    ...
    pub fn init(allocator: Allocator) F32BoolHashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F32BoolHashBiMap {
        return .{
            .forward = OpenHashMap(f32, bool).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(bool, f32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *F32BoolHashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *F32BoolHashBiMap, key: f32, value: bool) ?bool {
    ...
    pub fn removeValue(self: *F32BoolHashBiMap, value: bool) ?f32 {
    ...
    pub fn containsKey(self: *const F32BoolHashBiMap, key: f32) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const F32BoolHashBiMap, value: bool) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const F32BoolHashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const F32BoolHashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *F32BoolHashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F32BoolHashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F32BoolHashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32BoolHashBiMap, f: *const fn (f32, bool) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const F32BoolHashBiMap, f: *const fn (f32) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const F32BoolHashBiMap, f: *const fn (bool) void) void {
    ...
    pub fn inverse(self: *const F32BoolHashBiMap) BoolF32HashBiMap {
        var result = BoolF32HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const F32BoolHashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const F32BoolHashBiMap, other: *const F32BoolHashBiMap) bool {
...
test "F32BoolHashBiMap: put and get and getKey" {
    var m = F32BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
    // forward lookup
    ...
    // reverse lookup
    try std.testing.expectEqual(@as(?f32, 1.0), m.getKey(true));
    try std.testing.expectEqual(@as(?f32, 2.0), m.getKey(false));
    // missing key
...
test "F32BoolHashBiMap: put overwrite updates reverse" {
    var m = F32BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    ...
    try std.testing.expectEqual(@as(?f32, null), m.getKey(true));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?f32, 1.0), m.getKey(false));
}

test "F32BoolHashBiMap: remove" {
    var m = F32BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
...
test "F32BoolHashBiMap: removeValue" {
    var m = F32BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
...
test "F32BoolHashBiMap: containsKey and containsValue" {
    var m = F32BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    try std.testing.expect(m.containsKey(1.0));
...
test "F32BoolHashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?f32, 1.0), inv.get(true));
    try std.testing.expectEqual(@as(?f32, 2.0), inv.get(false));
}

test "F32BoolHashBiMap: display format" {
    var m = F32BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
...
test "F32BoolHashBiMap: eql" {
    var m1 = F32BoolHashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, true);
    _ = m1.put(2.0, false);
    var m2 = F32BoolHashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, false);
    _ = m2.put(1.0, true);
...
test "F32BoolHashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = F32BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32BoolHashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = F32BoolHashBiMap.init(failing.allocator());
    defer m.deinit();
