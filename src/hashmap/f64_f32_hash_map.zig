...
const ImmutableF64F32HashMap = @import("../immutable/immutable_f64_f32_hash_map.zig").ImmutableF64F32HashMap;

/// Hash map from `f64` keys to `f32` values.
///
...
pub const F64F32HashMap = struct {
    inner: OpenHashMap(f64, f32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64F32HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) F64F32HashMap {
        return .{
            .inner = OpenHashMap(f64, f32).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F64F32HashMap {
        return .{
            .inner = OpenHashMap(f64, f32).init(
    ...
    pub fn deinit(self: *F64F32HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *F64F32HashMap, key: f64, value: f32) ?f32 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const F64F32HashMap, key: f64) ?f32 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const F64F32HashMap, key: f64, default_value: f32) f32 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *F64F32HashMap, key: f64) ?f32 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const F64F32HashMap, key: f64) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const F64F32HashMap, value: f32) bool {
    ...
    pub fn getEntry(self: *F64F32HashMap, key: f64) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const F64F32HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const F64F32HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const F64F32HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *F64F32HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F64F32HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64F32HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64F32HashMap, f: *const fn (f64, f32) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const F64F32HashMap, f: *const fn (f64) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const F64F32HashMap, f: *const fn (f32) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F64F32HashMap, predicate: *const fn (f64, f32) bool) F64F32HashMap {
    ...
    pub fn reject(self: *const F64F32HashMap, predicate: *const fn (f64, f32) bool) F64F32HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const F64F32HashMap, predicate: *const fn (f64, f32) bool) ?struct { key: f64, value: f32 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const F64F32HashMap, predicate: *const fn (f64, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const F64F32HashMap, predicate: *const fn (f64, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const F64F32HashMap, predicate: *const fn (f64, f32) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F64F32HashMap, predicate: *const fn (f64, f32) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const F64F32HashMap, initial: f32, f: *const fn (f32, f64, f32) f32) f32 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const F64F32HashMap, allocator: Allocator) []f64 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const F64F32HashMap, allocator: Allocator) []f32 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const F64F32HashMap) f32 {
        var total: f32 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *F64F32HashMap, key: f64, delta: f32) f32 {
    ...
    pub fn toImmutable(self: *const F64F32HashMap) ImmutableF64F32HashMap {
        return ImmutableF64F32HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *F64F32HashMap, key: f64, value: f32) *F64F32HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F64F32HashMap, key: f64) *F64F32HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *F64F32HashMap, keys: []const f64) *F64F32HashMap {
    ...
    pub fn updateValue(self: *F64F32HashMap, key: f64, initial: f32, f: *const fn (f32) f32) f32 {
    ...
    pub fn format(self: *const F64F32HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const F64F32HashMap, other: *const F64F32HashMap) bool {
...
test "F64F32HashMap: put and get" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
    _ = m.put(3.0, 3.0);
    try std.testing.expectEqual(@as(?f32, 1.0), m.get(1.0));
    try std.testing.expectEqual(@as(?f32, null), m.get(99.0));
    try std.testing.expectEqual(@as(usize, 3), m.len());
}

test "F64F32HashMap: put overwrite" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    const old = m.put(1.0, 2.0);
    try std.testing.expectEqual(@as(?f32, 1.0), old);
    try std.testing.expectEqual(@as(?f32, 2.0), m.get(1.0));
}

test "F64F32HashMap: remove" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F32HashMap: containsKey and containsValue" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    try std.testing.expect(m.containsKey(1.0));
...
test "F64F32HashMap: getOrDefault" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    try std.testing.expectEqual(1.0, m.getOrDefault(1.0, 3.0));
    try std.testing.expectEqual(3.0, m.getOrDefault(99.0, 3.0));
}

test "F64F32HashMap: clear" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    m.clear();
...
test "F64F32HashMap: select and reject" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F32HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F32HashMap: keysToSlice and valuesToSlice" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F32HashMap: sumOfValues" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F32HashMap: addToValue" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.addToValue(1.0, 1.0);
    _ = m.addToValue(1.0, 2.0);
    // Value should be sum of the two adds
    try std.testing.expect(m.get(1.0) != null);
}

test "F64F32HashMap: toImmutable" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F64F32HashMap: fluent withKeyValue/withoutKey" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.withKeyValue(1.0, 1.0).withKeyValue(2.0, 2.0);
    try std.testing.expectEqual(@as(usize, 2), m.len());
    _ = m.withoutKey(1.0);
    try std.testing.expectEqual(@as(usize, 1), m.len());
}

test "F64F32HashMap: eql" {
    var m1 = F64F32HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1.0);
    _ = m1.put(2.0, 2.0);
    var m2 = F64F32HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2.0);
    _ = m2.put(1.0, 1.0);
...
test "F64F32HashMap: resize" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(f64, @floatFromInt(i)), @as(f32, @floatFromInt(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "F64F32HashMap: entry orInsert" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    // Key absent — inserts default
    const v1 = m.getEntry(1.0).orInsert(1.0);
    try std.testing.expectEqual(1.0, v1);
    // Key present — returns existing
    _ = m.put(1.0, 2.0);
    const v2 = m.getEntry(1.0).orInsert(3.0);
    try std.testing.expectEqual(2.0, v2);
}

test "F64F32HashMap: entry andModify orInsert" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F32HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F32HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F32HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64F32HashMap.init(failing.allocator());
    defer m.deinit();
...
test "F64F32HashMap: NaN key findable" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1.0);
    try std.testing.expect(m.containsKey(nan_key));
    try std.testing.expectEqual(@as(?f32, 1.0), m.get(nan_key));
}

test "F64F32HashMap: NaN key replaces, does not duplicate" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1.0);
    ...
    _ = m.put(nan_key, 3.0);
    try std.testing.expectEqual(@as(usize, 1), m.len());
    try std.testing.expectEqual(@as(?f32, 3.0), m.get(nan_key));
}

test "F64F32HashMap: NaN key remove" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1.0);
...
test "F64F32HashMap: -0.0 distinct from +0.0" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const pos_zero: f64 = @as(f64, 0.0);
    const neg_zero: f64 = @as(f64, -0.0);
    _ = m.put(pos_zero, 1.0);
    _ = m.put(neg_zero, 2.0);
    try std.testing.expectEqual(@as(usize, 2), m.len());
    try std.testing.expectEqual(@as(?f32, 1.0), m.get(pos_zero));
    try std.testing.expectEqual(@as(?f32, 2.0), m.get(neg_zero));
}

test "F64F32HashMap: +/-Infinity keys" {
    var m = F64F32HashMap.init(std.testing.allocator);
    defer m.deinit();
    ...
    try std.testing.expectEqual(@as(?f32, 1.0), m.get(pos_inf));
    try std.testing.expectEqual(@as(?f32, 2.0), m.get(neg_inf));
}
