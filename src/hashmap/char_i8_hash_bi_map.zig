...
const I8CharHashBiMap = @import("i8_char_hash_bi_map.zig").I8CharHashBiMap;

/// Bidirectional hash map from `u21` keys to `i8` values.
///
...
/// maps in sync.
pub const CharI8HashBiMap = struct {
    forward: OpenHashMap(u21, i8),
    reverse: OpenHashMap(i8, u21),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharI8HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) CharI8HashBiMap {
        return .{
            .forward = OpenHashMap(u21, i8).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i8, u21).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *CharI8HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *CharI8HashBiMap, key: u21, value: i8) ?i8 {
    ...
    pub fn removeValue(self: *CharI8HashBiMap, value: i8) ?u21 {
    ...
    pub fn containsKey(self: *const CharI8HashBiMap, key: u21) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const CharI8HashBiMap, value: i8) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const CharI8HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const CharI8HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *CharI8HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *CharI8HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI8HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI8HashBiMap, f: *const fn (u21, i8) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const CharI8HashBiMap, f: *const fn (u21) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const CharI8HashBiMap, f: *const fn (i8) void) void {
    ...
    pub fn inverse(self: *const CharI8HashBiMap) I8CharHashBiMap {
        var result = I8CharHashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const CharI8HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const CharI8HashBiMap, other: *const CharI8HashBiMap) bool {
...
test "CharI8HashBiMap: put and get and getKey" {
    var m = CharI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
    // forward lookup
    try std.testing.expectEqual(@as(?i8, 1), m.get('a'));
    try std.testing.expectEqual(@as(?i8, 2), m.get('b'));
    // reverse lookup
...
test "CharI8HashBiMap: put overwrite updates reverse" {
    var m = CharI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
...
test "CharI8HashBiMap: removeValue" {
    var m = CharI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI8HashBiMap: containsKey and containsValue" {
    var m = CharI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    try std.testing.expect(m.containsKey('a'));
...
test "CharI8HashBiMap: display format" {
    var m = CharI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
...
test "CharI8HashBiMap: eql" {
    var m1 = CharI8HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1);
    _ = m1.put('b', 2);
    var m2 = CharI8HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2);
    _ = m2.put('a', 1);
...
test "CharI8HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = CharI8HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = CharI8HashBiMap.init(failing.allocator());
    defer m.deinit();
