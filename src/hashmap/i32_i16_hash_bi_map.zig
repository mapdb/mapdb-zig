...
const I16I32HashBiMap = @import("i16_i32_hash_bi_map.zig").I16I32HashBiMap;

/// Bidirectional hash map from `i32` keys to `i16` values.
///
...
/// maps in sync.
pub const I32I16HashBiMap = struct {
    forward: OpenHashMap(i32, i16),
    reverse: OpenHashMap(i16, i32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I32I16HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I32I16HashBiMap {
        return .{
            .forward = OpenHashMap(i32, i16).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i16, i32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I32I16HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I32I16HashBiMap, key: i32, value: i16) ?i16 {
    ...
    pub fn removeValue(self: *I32I16HashBiMap, value: i16) ?i32 {
    ...
    pub fn containsKey(self: *const I32I16HashBiMap, key: i32) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I32I16HashBiMap, value: i16) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I32I16HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I32I16HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I32I16HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I32I16HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32I16HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32I16HashBiMap, f: *const fn (i32, i16) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I32I16HashBiMap, f: *const fn (i32) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I32I16HashBiMap, f: *const fn (i16) void) void {
    ...
    pub fn inverse(self: *const I32I16HashBiMap) I16I32HashBiMap {
        var result = I16I32HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I32I16HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I32I16HashBiMap, other: *const I32I16HashBiMap) bool {
...
test "I32I16HashBiMap: put and get and getKey" {
    var m = I32I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    // forward lookup
    ...
    // reverse lookup
    try std.testing.expectEqual(@as(?i32, 1), m.getKey(1));
    try std.testing.expectEqual(@as(?i32, 2), m.getKey(2));
    // missing key
    try std.testing.expectEqual(@as(?i16, null), m.get(99));
    // missing value
    try std.testing.expectEqual(@as(?i32, null), m.getKey(99));
    try std.testing.expectEqual(@as(usize, 2), m.len());
}

test "I32I16HashBiMap: put overwrite updates reverse" {
    var m = I32I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    ...
    try std.testing.expectEqual(@as(?i32, null), m.getKey(1));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?i32, 1), m.getKey(2));
}

test "I32I16HashBiMap: remove" {
    var m = I32I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I32I16HashBiMap: removeValue" {
    var m = I32I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I32I16HashBiMap: containsKey and containsValue" {
    var m = I32I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    try std.testing.expect(m.containsKey(1));
...
test "I32I16HashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?i32, 1), inv.get(1));
    try std.testing.expectEqual(@as(?i32, 2), inv.get(2));
}

test "I32I16HashBiMap: display format" {
    var m = I32I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I32I16HashBiMap: eql" {
    var m1 = I32I16HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);
    var m2 = I32I16HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I32I16HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I32I16HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I16HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I32I16HashBiMap.init(failing.allocator());
    defer m.deinit();
