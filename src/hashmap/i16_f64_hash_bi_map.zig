...
const F64I16HashBiMap = @import("f64_i16_hash_bi_map.zig").F64I16HashBiMap;

/// Bidirectional hash map from `i16` keys to `f64` values.
///
...
/// maps in sync.
pub const I16F64HashBiMap = struct {
    forward: OpenHashMap(i16, f64),
    reverse: OpenHashMap(f64, i16),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I16F64HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I16F64HashBiMap {
        return .{
            .forward = OpenHashMap(i16, f64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(f64, i16).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I16F64HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I16F64HashBiMap, key: i16, value: f64) ?f64 {
    ...
    pub fn removeValue(self: *I16F64HashBiMap, value: f64) ?i16 {
    ...
    pub fn containsKey(self: *const I16F64HashBiMap, key: i16) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I16F64HashBiMap, value: f64) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I16F64HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I16F64HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I16F64HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I16F64HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16F64HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16F64HashBiMap, f: *const fn (i16, f64) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I16F64HashBiMap, f: *const fn (i16) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I16F64HashBiMap, f: *const fn (f64) void) void {
    ...
    pub fn inverse(self: *const I16F64HashBiMap) F64I16HashBiMap {
        var result = F64I16HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I16F64HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I16F64HashBiMap, other: *const I16F64HashBiMap) bool {
...
test "I16F64HashBiMap: put and get and getKey" {
    var m = I16F64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
    // forward lookup
...
test "I16F64HashBiMap: put overwrite updates reverse" {
    var m = I16F64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
...
test "I16F64HashBiMap: removeValue" {
    var m = I16F64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
...
test "I16F64HashBiMap: containsKey and containsValue" {
    var m = I16F64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    try std.testing.expect(m.containsKey(1));
...
test "I16F64HashBiMap: display format" {
    var m = I16F64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
...
test "I16F64HashBiMap: eql" {
    var m1 = I16F64HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1.0);
    _ = m1.put(2, 2.0);
    var m2 = I16F64HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2.0);
    _ = m2.put(1, 1.0);
...
test "I16F64HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I16F64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F64HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I16F64HashBiMap.init(failing.allocator());
    defer m.deinit();
