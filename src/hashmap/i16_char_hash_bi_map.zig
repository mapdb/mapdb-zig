...
const CharI16HashBiMap = @import("char_i16_hash_bi_map.zig").CharI16HashBiMap;

/// Bidirectional hash map from `i16` keys to `u21` values.
///
...
/// maps in sync.
pub const I16CharHashBiMap = struct {
    forward: OpenHashMap(i16, u21),
    reverse: OpenHashMap(u21, i16),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I16CharHashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I16CharHashBiMap {
        return .{
            .forward = OpenHashMap(i16, u21).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(u21, i16).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I16CharHashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I16CharHashBiMap, key: i16, value: u21) ?u21 {
    ...
    pub fn removeValue(self: *I16CharHashBiMap, value: u21) ?i16 {
    ...
    pub fn containsKey(self: *const I16CharHashBiMap, key: i16) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I16CharHashBiMap, value: u21) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I16CharHashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I16CharHashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I16CharHashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I16CharHashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16CharHashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16CharHashBiMap, f: *const fn (i16, u21) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I16CharHashBiMap, f: *const fn (i16) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I16CharHashBiMap, f: *const fn (u21) void) void {
    ...
    pub fn inverse(self: *const I16CharHashBiMap) CharI16HashBiMap {
        var result = CharI16HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I16CharHashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I16CharHashBiMap, other: *const I16CharHashBiMap) bool {
...
test "I16CharHashBiMap: put and get and getKey" {
    var m = I16CharHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
    // forward lookup
    ...
    // reverse lookup
    try std.testing.expectEqual(@as(?i16, 1), m.getKey('a'));
    try std.testing.expectEqual(@as(?i16, 2), m.getKey('b'));
    // missing key
    try std.testing.expectEqual(@as(?u21, null), m.get(99));
    // missing value
    try std.testing.expectEqual(@as(?i16, null), m.getKey('z'));
    try std.testing.expectEqual(@as(usize, 2), m.len());
}

test "I16CharHashBiMap: put overwrite updates reverse" {
    var m = I16CharHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    ...
    try std.testing.expectEqual(@as(?i16, null), m.getKey('a'));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?i16, 1), m.getKey('b'));
}

test "I16CharHashBiMap: remove" {
    var m = I16CharHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
...
test "I16CharHashBiMap: removeValue" {
    var m = I16CharHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
...
test "I16CharHashBiMap: containsKey and containsValue" {
    var m = I16CharHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    try std.testing.expect(m.containsKey(1));
...
test "I16CharHashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?i16, 1), inv.get('a'));
    try std.testing.expectEqual(@as(?i16, 2), inv.get('b'));
}

test "I16CharHashBiMap: display format" {
    var m = I16CharHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
...
test "I16CharHashBiMap: eql" {
    var m1 = I16CharHashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 'a');
    _ = m1.put(2, 'b');
    var m2 = I16CharHashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 'b');
    _ = m2.put(1, 'a');
...
test "I16CharHashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I16CharHashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16CharHashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I16CharHashBiMap.init(failing.allocator());
    defer m.deinit();
