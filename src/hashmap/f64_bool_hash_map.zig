...
const ImmutableF64BoolHashMap = @import("../immutable/immutable_f64_bool_hash_map.zig").ImmutableF64BoolHashMap;
...
pub const F64BoolHashMap = struct {
    inner: OpenHashMap(f64, bool),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64BoolHashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) F64BoolHashMap {
        return .{
            .inner = OpenHashMap(f64, bool).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F64BoolHashMap {
        return .{
            .inner = OpenHashMap(f64, bool).init(
    ...
    pub fn deinit(self: *F64BoolHashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *F64BoolHashMap, key: f64, value: bool) ?bool {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const F64BoolHashMap, key: f64) ?bool {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const F64BoolHashMap, key: f64, default_value: bool) bool {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *F64BoolHashMap, key: f64) ?bool {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const F64BoolHashMap, key: f64) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const F64BoolHashMap, value: bool) bool {
    ...
    pub fn getEntry(self: *F64BoolHashMap, key: f64) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const F64BoolHashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const F64BoolHashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const F64BoolHashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *F64BoolHashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F64BoolHashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64BoolHashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64BoolHashMap, f: *const fn (f64, bool) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const F64BoolHashMap, f: *const fn (f64) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const F64BoolHashMap, f: *const fn (bool) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F64BoolHashMap, predicate: *const fn (f64, bool) bool) F64BoolHashMap {
    ...
    pub fn reject(self: *const F64BoolHashMap, predicate: *const fn (f64, bool) bool) F64BoolHashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const F64BoolHashMap, predicate: *const fn (f64, bool) bool) ?struct { key: f64, value: bool } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const F64BoolHashMap, predicate: *const fn (f64, bool) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const F64BoolHashMap, predicate: *const fn (f64, bool) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const F64BoolHashMap, predicate: *const fn (f64, bool) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F64BoolHashMap, predicate: *const fn (f64, bool) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const F64BoolHashMap, initial: bool, f: *const fn (bool, f64, bool) bool) bool {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const F64BoolHashMap, allocator: Allocator) []f64 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const F64BoolHashMap, allocator: Allocator) []bool {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Conversion ----

    pub fn toImmutable(self: *const F64BoolHashMap) ImmutableF64BoolHashMap {
        return ImmutableF64BoolHashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *F64BoolHashMap, key: f64, value: bool) *F64BoolHashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F64BoolHashMap, key: f64) *F64BoolHashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *F64BoolHashMap, keys: []const f64) *F64BoolHashMap {
    ...
    pub fn updateValue(self: *F64BoolHashMap, key: f64, initial: bool, f: *const fn (bool) bool) bool {
    ...
    pub fn format(self: *const F64BoolHashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const F64BoolHashMap, other: *const F64BoolHashMap) bool {
...
test "F64BoolHashMap: put and get" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
    _ = m.put(3.0, true);
...
test "F64BoolHashMap: put overwrite" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
...
test "F64BoolHashMap: containsKey and containsValue" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    try std.testing.expect(m.containsKey(1.0));
...
test "F64BoolHashMap: getOrDefault" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
...
test "F64BoolHashMap: select" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
    var sel = m.select(struct {
        fn f(_: f64, v: bool) bool {
            return v == true;
        }
    }.f);
    defer sel.deinit();
...
test "F64BoolHashMap: keysToSlice and valuesToSlice" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
...
test "F64BoolHashMap: toImmutable" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
...
test "F64BoolHashMap: fluent withKeyValue/withoutKey" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolHashMap: eql" {
    var m1 = F64BoolHashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, true);
    _ = m1.put(2.0, false);
    var m2 = F64BoolHashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, false);
    _ = m2.put(1.0, true);
...
test "F64BoolHashMap: resize" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(f64, @floatFromInt(i)), i % 3 == 0);
    }
    try std.testing.expect(m.len() > 0);
}

test "F64BoolHashMap: entry orInsert" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolHashMap: entry andModify orInsert" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolHashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolHashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolHashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64BoolHashMap.init(failing.allocator());
    defer m.deinit();
...
test "F64BoolHashMap: NaN key findable" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolHashMap: NaN key replaces, does not duplicate" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, true);
...
test "F64BoolHashMap: NaN key remove" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, true);
...
test "F64BoolHashMap: -0.0 distinct from +0.0" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolHashMap: +/-Infinity keys" {
    var m = F64BoolHashMap.init(std.testing.allocator);
    defer m.deinit();
