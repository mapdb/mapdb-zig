...
const F32I8HashBiMap = @import("f32_i8_hash_bi_map.zig").F32I8HashBiMap;

/// Bidirectional hash map from `i8` keys to `f32` values.
///
...
/// maps in sync.
pub const I8F32HashBiMap = struct {
    forward: OpenHashMap(i8, f32),
    reverse: OpenHashMap(f32, i8),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I8F32HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I8F32HashBiMap {
        return .{
            .forward = OpenHashMap(i8, f32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(f32, i8).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I8F32HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I8F32HashBiMap, key: i8, value: f32) ?f32 {
    ...
    pub fn removeValue(self: *I8F32HashBiMap, value: f32) ?i8 {
    ...
    pub fn containsKey(self: *const I8F32HashBiMap, key: i8) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I8F32HashBiMap, value: f32) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I8F32HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I8F32HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I8F32HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I8F32HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8F32HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8F32HashBiMap, f: *const fn (i8, f32) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I8F32HashBiMap, f: *const fn (i8) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I8F32HashBiMap, f: *const fn (f32) void) void {
    ...
    pub fn inverse(self: *const I8F32HashBiMap) F32I8HashBiMap {
        var result = F32I8HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I8F32HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I8F32HashBiMap, other: *const I8F32HashBiMap) bool {
...
test "I8F32HashBiMap: put and get and getKey" {
    var m = I8F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
    // forward lookup
    ...
    // reverse lookup
    try std.testing.expectEqual(@as(?i8, 1), m.getKey(1.0));
    try std.testing.expectEqual(@as(?i8, 2), m.getKey(2.0));
    // missing key
    try std.testing.expectEqual(@as(?f32, null), m.get(99));
    // missing value
    try std.testing.expectEqual(@as(?i8, null), m.getKey(99.0));
    try std.testing.expectEqual(@as(usize, 2), m.len());
}

test "I8F32HashBiMap: put overwrite updates reverse" {
    var m = I8F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    ...
    try std.testing.expectEqual(@as(?i8, null), m.getKey(1.0));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?i8, 1), m.getKey(2.0));
}

test "I8F32HashBiMap: remove" {
    var m = I8F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
...
test "I8F32HashBiMap: removeValue" {
    var m = I8F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
...
test "I8F32HashBiMap: containsKey and containsValue" {
    var m = I8F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    try std.testing.expect(m.containsKey(1));
...
test "I8F32HashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?i8, 1), inv.get(1.0));
    try std.testing.expectEqual(@as(?i8, 2), inv.get(2.0));
}

test "I8F32HashBiMap: display format" {
    var m = I8F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
...
test "I8F32HashBiMap: eql" {
    var m1 = I8F32HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1.0);
    _ = m1.put(2, 2.0);
    var m2 = I8F32HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2.0);
    _ = m2.put(1, 1.0);
...
test "I8F32HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I8F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8F32HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I8F32HashBiMap.init(failing.allocator());
    defer m.deinit();
