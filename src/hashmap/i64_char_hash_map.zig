...
const ImmutableI64CharHashMap = @import("../immutable/immutable_i64_char_hash_map.zig").ImmutableI64CharHashMap;

/// Hash map from `i64` keys to `u21` values.
///
...
pub const I64CharHashMap = struct {
    inner: OpenHashMap(i64, u21),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I64CharHashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) I64CharHashMap {
        return .{
            .inner = OpenHashMap(i64, u21).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I64CharHashMap {
        return .{
            .inner = OpenHashMap(i64, u21).init(
    ...
    pub fn deinit(self: *I64CharHashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *I64CharHashMap, key: i64, value: u21) ?u21 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const I64CharHashMap, key: i64) ?u21 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const I64CharHashMap, key: i64, default_value: u21) u21 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *I64CharHashMap, key: i64) ?u21 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const I64CharHashMap, key: i64) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const I64CharHashMap, value: u21) bool {
    ...
    pub fn getEntry(self: *I64CharHashMap, key: i64) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const I64CharHashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const I64CharHashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const I64CharHashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *I64CharHashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I64CharHashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I64CharHashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I64CharHashMap, f: *const fn (i64, u21) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const I64CharHashMap, f: *const fn (i64) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const I64CharHashMap, f: *const fn (u21) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I64CharHashMap, predicate: *const fn (i64, u21) bool) I64CharHashMap {
    ...
    pub fn reject(self: *const I64CharHashMap, predicate: *const fn (i64, u21) bool) I64CharHashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const I64CharHashMap, predicate: *const fn (i64, u21) bool) ?struct { key: i64, value: u21 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const I64CharHashMap, predicate: *const fn (i64, u21) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const I64CharHashMap, predicate: *const fn (i64, u21) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const I64CharHashMap, predicate: *const fn (i64, u21) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I64CharHashMap, predicate: *const fn (i64, u21) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const I64CharHashMap, initial: u21, f: *const fn (u21, i64, u21) u21) u21 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const I64CharHashMap, allocator: Allocator) []i64 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const I64CharHashMap, allocator: Allocator) []u21 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Conversion ----

    pub fn toImmutable(self: *const I64CharHashMap) ImmutableI64CharHashMap {
        return ImmutableI64CharHashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *I64CharHashMap, key: i64, value: u21) *I64CharHashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I64CharHashMap, key: i64) *I64CharHashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *I64CharHashMap, keys: []const i64) *I64CharHashMap {
    ...
    pub fn updateValue(self: *I64CharHashMap, key: i64, initial: u21, f: *const fn (u21) u21) u21 {
    ...
    pub fn format(self: *const I64CharHashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const I64CharHashMap, other: *const I64CharHashMap) bool {
...
test "I64CharHashMap: put and get" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
    _ = m.put(3, 'c');
...
test "I64CharHashMap: put overwrite" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    const old = m.put(1, 'b');
...
test "I64CharHashMap: containsKey and containsValue" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    try std.testing.expect(m.containsKey(1));
...
test "I64CharHashMap: getOrDefault" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
...
test "I64CharHashMap: select and reject" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
...
test "I64CharHashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
...
test "I64CharHashMap: keysToSlice and valuesToSlice" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
...
test "I64CharHashMap: toImmutable" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
...
test "I64CharHashMap: fluent withKeyValue/withoutKey" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharHashMap: eql" {
    var m1 = I64CharHashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 'a');
    _ = m1.put(2, 'b');
    var m2 = I64CharHashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 'b');
    _ = m2.put(1, 'a');
...
test "I64CharHashMap: resize" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(i64, @intCast(i)), @as(u21, @intCast(i % 26)) + 'A');
    }
    try std.testing.expect(m.len() > 0);
}

test "I64CharHashMap: entry orInsert" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharHashMap: entry andModify orInsert" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharHashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharHashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = I64CharHashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharHashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I64CharHashMap.init(failing.allocator());
    defer m.deinit();
