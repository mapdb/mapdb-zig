...
const ImmutableCharI16HashMap = @import("../immutable/immutable_char_i16_hash_map.zig").ImmutableCharI16HashMap;

/// Hash map from `u21` keys to `i16` values.
///
...
pub const CharI16HashMap = struct {
    inner: OpenHashMap(u21, i16),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharI16HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) CharI16HashMap {
        return .{
            .inner = OpenHashMap(u21, i16).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) CharI16HashMap {
        return .{
            .inner = OpenHashMap(u21, i16).init(
    ...
    pub fn deinit(self: *CharI16HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *CharI16HashMap, key: u21, value: i16) ?i16 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const CharI16HashMap, key: u21) ?i16 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const CharI16HashMap, key: u21, default_value: i16) i16 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *CharI16HashMap, key: u21) ?i16 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const CharI16HashMap, key: u21) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const CharI16HashMap, value: i16) bool {
    ...
    pub fn getEntry(self: *CharI16HashMap, key: u21) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const CharI16HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const CharI16HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const CharI16HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *CharI16HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *CharI16HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI16HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI16HashMap, f: *const fn (u21, i16) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const CharI16HashMap, f: *const fn (u21) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const CharI16HashMap, f: *const fn (i16) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const CharI16HashMap, predicate: *const fn (u21, i16) bool) CharI16HashMap {
    ...
    pub fn reject(self: *const CharI16HashMap, predicate: *const fn (u21, i16) bool) CharI16HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const CharI16HashMap, predicate: *const fn (u21, i16) bool) ?struct { key: u21, value: i16 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const CharI16HashMap, predicate: *const fn (u21, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const CharI16HashMap, predicate: *const fn (u21, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const CharI16HashMap, predicate: *const fn (u21, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const CharI16HashMap, predicate: *const fn (u21, i16) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const CharI16HashMap, initial: i16, f: *const fn (i16, u21, i16) i16) i16 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const CharI16HashMap, allocator: Allocator) []u21 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const CharI16HashMap, allocator: Allocator) []i16 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const CharI16HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn toImmutable(self: *const CharI16HashMap) ImmutableCharI16HashMap {
        return ImmutableCharI16HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *CharI16HashMap, key: u21, value: i16) *CharI16HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *CharI16HashMap, key: u21) *CharI16HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *CharI16HashMap, keys: []const u21) *CharI16HashMap {
    ...
    pub fn updateValue(self: *CharI16HashMap, key: u21, initial: i16, f: *const fn (i16) i16) i16 {
    ...
    pub fn format(self: *const CharI16HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const CharI16HashMap, other: *const CharI16HashMap) bool {
...
test "CharI16HashMap: put and get" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
    _ = m.put('c', 3);
...
test "CharI16HashMap: put overwrite" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    const old = m.put('a', 2);
    try std.testing.expectEqual(@as(?i16, 1), old);
    try std.testing.expectEqual(@as(?i16, 2), m.get('a'));
}

test "CharI16HashMap: remove" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI16HashMap: containsKey and containsValue" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    try std.testing.expect(m.containsKey('a'));
...
test "CharI16HashMap: getOrDefault" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
...
test "CharI16HashMap: select and reject" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI16HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI16HashMap: keysToSlice and valuesToSlice" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI16HashMap: sumOfValues" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI16HashMap: addToValue" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.addToValue('a', 1);
    _ = m.addToValue('a', 2);
...
test "CharI16HashMap: toImmutable" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
...
test "CharI16HashMap: fluent withKeyValue/withoutKey" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI16HashMap: eql" {
    var m1 = CharI16HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1);
    _ = m1.put('b', 2);
    var m2 = CharI16HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2);
    _ = m2.put('a', 1);
...
test "CharI16HashMap: resize" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(u21, @intCast(i % 26)) + 'a', @as(i16, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "CharI16HashMap: entry orInsert" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI16HashMap: entry andModify orInsert" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI16HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI16HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = CharI16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI16HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI16HashMap.init(failing.allocator());
    defer m.deinit();
