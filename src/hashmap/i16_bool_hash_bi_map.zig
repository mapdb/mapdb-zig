...
const BoolI16HashBiMap = @import("bool_i16_hash_bi_map.zig").BoolI16HashBiMap;

/// Bidirectional hash map from `i16` keys to `bool` values.
///
...
/// maps in sync.
pub const I16BoolHashBiMap = struct {
    forward: OpenHashMap(i16, bool),
    reverse: OpenHashMap(bool, i16),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I16BoolHashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I16BoolHashBiMap {
        return .{
            .forward = OpenHashMap(i16, bool).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(bool, i16).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *I16BoolHashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *I16BoolHashBiMap, key: i16, value: bool) ?bool {
    ...
    pub fn removeValue(self: *I16BoolHashBiMap, value: bool) ?i16 {
    ...
    pub fn containsKey(self: *const I16BoolHashBiMap, key: i16) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const I16BoolHashBiMap, value: bool) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const I16BoolHashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const I16BoolHashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *I16BoolHashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I16BoolHashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16BoolHashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16BoolHashBiMap, f: *const fn (i16, bool) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const I16BoolHashBiMap, f: *const fn (i16) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const I16BoolHashBiMap, f: *const fn (bool) void) void {
    ...
    pub fn inverse(self: *const I16BoolHashBiMap) BoolI16HashBiMap {
        var result = BoolI16HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const I16BoolHashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const I16BoolHashBiMap, other: *const I16BoolHashBiMap) bool {
...
test "I16BoolHashBiMap: put and get and getKey" {
    var m = I16BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
    // forward lookup
    try std.testing.expectEqual(@as(?bool, true), m.get(1));
    try std.testing.expectEqual(@as(?bool, false), m.get(2));
    // reverse lookup
    try std.testing.expectEqual(@as(?i16, 1), m.getKey(true));
    try std.testing.expectEqual(@as(?i16, 2), m.getKey(false));
    // missing key
...
test "I16BoolHashBiMap: put overwrite updates reverse" {
    var m = I16BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    ...
    try std.testing.expectEqual(@as(?i16, null), m.getKey(true));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?i16, 1), m.getKey(false));
}

test "I16BoolHashBiMap: remove" {
    var m = I16BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
...
test "I16BoolHashBiMap: removeValue" {
    var m = I16BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
...
test "I16BoolHashBiMap: containsKey and containsValue" {
    var m = I16BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    try std.testing.expect(m.containsKey(1));
...
test "I16BoolHashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?i16, 1), inv.get(true));
    try std.testing.expectEqual(@as(?i16, 2), inv.get(false));
}

test "I16BoolHashBiMap: display format" {
    var m = I16BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
...
test "I16BoolHashBiMap: eql" {
    var m1 = I16BoolHashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, true);
    _ = m1.put(2, false);
    var m2 = I16BoolHashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, false);
    _ = m2.put(1, true);
...
test "I16BoolHashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = I16BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16BoolHashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = I16BoolHashBiMap.init(failing.allocator());
    defer m.deinit();
