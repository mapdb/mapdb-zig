...
const ImmutableF64I64HashMap = @import("../immutable/immutable_f64_i64_hash_map.zig").ImmutableF64I64HashMap;

/// Hash map from `f64` keys to `i64` values.
///
...
pub const F64I64HashMap = struct {
    inner: OpenHashMap(f64, i64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64I64HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) F64I64HashMap {
        return .{
            .inner = OpenHashMap(f64, i64).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F64I64HashMap {
        return .{
            .inner = OpenHashMap(f64, i64).init(
    ...
    pub fn deinit(self: *F64I64HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *F64I64HashMap, key: f64, value: i64) ?i64 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const F64I64HashMap, key: f64) ?i64 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const F64I64HashMap, key: f64, default_value: i64) i64 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *F64I64HashMap, key: f64) ?i64 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const F64I64HashMap, key: f64) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const F64I64HashMap, value: i64) bool {
    ...
    pub fn getEntry(self: *F64I64HashMap, key: f64) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const F64I64HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const F64I64HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const F64I64HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *F64I64HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F64I64HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64I64HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64I64HashMap, f: *const fn (f64, i64) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const F64I64HashMap, f: *const fn (f64) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const F64I64HashMap, f: *const fn (i64) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F64I64HashMap, predicate: *const fn (f64, i64) bool) F64I64HashMap {
    ...
    pub fn reject(self: *const F64I64HashMap, predicate: *const fn (f64, i64) bool) F64I64HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const F64I64HashMap, predicate: *const fn (f64, i64) bool) ?struct { key: f64, value: i64 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const F64I64HashMap, predicate: *const fn (f64, i64) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const F64I64HashMap, predicate: *const fn (f64, i64) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const F64I64HashMap, predicate: *const fn (f64, i64) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F64I64HashMap, predicate: *const fn (f64, i64) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const F64I64HashMap, initial: i64, f: *const fn (i64, f64, i64) i64) i64 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const F64I64HashMap, allocator: Allocator) []f64 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const F64I64HashMap, allocator: Allocator) []i64 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const F64I64HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *F64I64HashMap, key: f64, delta: i64) i64 {
    ...
    pub fn toImmutable(self: *const F64I64HashMap) ImmutableF64I64HashMap {
        return ImmutableF64I64HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *F64I64HashMap, key: f64, value: i64) *F64I64HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F64I64HashMap, key: f64) *F64I64HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *F64I64HashMap, keys: []const f64) *F64I64HashMap {
    ...
    pub fn updateValue(self: *F64I64HashMap, key: f64, initial: i64, f: *const fn (i64) i64) i64 {
    ...
    pub fn format(self: *const F64I64HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const F64I64HashMap, other: *const F64I64HashMap) bool {
...
test "F64I64HashMap: put and get" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
    _ = m.put(3.0, 3);
...
test "F64I64HashMap: put overwrite" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    const old = m.put(1.0, 2);
    try std.testing.expectEqual(@as(?i64, 1), old);
    try std.testing.expectEqual(@as(?i64, 2), m.get(1.0));
}

test "F64I64HashMap: remove" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I64HashMap: containsKey and containsValue" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    try std.testing.expect(m.containsKey(1.0));
...
test "F64I64HashMap: getOrDefault" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F64I64HashMap: select and reject" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I64HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I64HashMap: keysToSlice and valuesToSlice" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I64HashMap: sumOfValues" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I64HashMap: toImmutable" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I64HashMap: fluent withKeyValue/withoutKey" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64HashMap: eql" {
    var m1 = F64I64HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1);
    _ = m1.put(2.0, 2);
    var m2 = F64I64HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2);
    _ = m2.put(1.0, 1);
...
test "F64I64HashMap: resize" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(f64, @floatFromInt(i)), @as(i64, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "F64I64HashMap: entry orInsert" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64HashMap: entry andModify orInsert" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64I64HashMap.init(failing.allocator());
    defer m.deinit();
...
test "F64I64HashMap: NaN key findable" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1);
    try std.testing.expect(m.containsKey(nan_key));
    try std.testing.expectEqual(@as(?i64, 1), m.get(nan_key));
}

test "F64I64HashMap: NaN key replaces, does not duplicate" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1);
    ...
    _ = m.put(nan_key, 3);
    try std.testing.expectEqual(@as(usize, 1), m.len());
    try std.testing.expectEqual(@as(?i64, 3), m.get(nan_key));
}

test "F64I64HashMap: NaN key remove" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1);
...
test "F64I64HashMap: -0.0 distinct from +0.0" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    ...
    _ = m.put(neg_zero, 2);
    try std.testing.expectEqual(@as(usize, 2), m.len());
    try std.testing.expectEqual(@as(?i64, 1), m.get(pos_zero));
    try std.testing.expectEqual(@as(?i64, 2), m.get(neg_zero));
}

test "F64I64HashMap: +/-Infinity keys" {
    var m = F64I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    ...
    try std.testing.expectEqual(@as(?i64, 1), m.get(pos_inf));
    try std.testing.expectEqual(@as(?i64, 2), m.get(neg_inf));
}
