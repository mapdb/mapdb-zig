...
/// Bidirectional hash map from `f32` keys to `f32` values.
///
...
/// maps in sync.
pub const F32F32HashBiMap = struct {
    forward: OpenHashMap(f32, f32),
    reverse: OpenHashMap(f32, f32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F32F32HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F32F32HashBiMap {
        return .{
            .forward = OpenHashMap(f32, f32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(f32, f32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *F32F32HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *F32F32HashBiMap, key: f32, value: f32) ?f32 {
    ...
    pub fn removeValue(self: *F32F32HashBiMap, value: f32) ?f32 {
    ...
    pub fn containsKey(self: *const F32F32HashBiMap, key: f32) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const F32F32HashBiMap, value: f32) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const F32F32HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const F32F32HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *F32F32HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F32F32HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F32F32HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32F32HashBiMap, f: *const fn (f32, f32) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const F32F32HashBiMap, f: *const fn (f32) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const F32F32HashBiMap, f: *const fn (f32) void) void {
    ...
    pub fn inverse(self: *const F32F32HashBiMap) F32F32HashBiMap {
        var result = F32F32HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const F32F32HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const F32F32HashBiMap, other: *const F32F32HashBiMap) bool {
...
test "F32F32HashBiMap: put and get and getKey" {
    var m = F32F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
    // forward lookup
    try std.testing.expectEqual(@as(?f32, 1.0), m.get(1.0));
    try std.testing.expectEqual(@as(?f32, 2.0), m.get(2.0));
    // reverse lookup
    try std.testing.expectEqual(@as(?f32, 1.0), m.getKey(1.0));
    try std.testing.expectEqual(@as(?f32, 2.0), m.getKey(2.0));
    // missing key
    try std.testing.expectEqual(@as(?f32, null), m.get(99.0));
    // missing value
    try std.testing.expectEqual(@as(?f32, null), m.getKey(99.0));
    try std.testing.expectEqual(@as(usize, 2), m.len());
}

test "F32F32HashBiMap: put overwrite updates reverse" {
    var m = F32F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    ...
    try std.testing.expectEqual(@as(?f32, null), m.getKey(1.0));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?f32, 1.0), m.getKey(2.0));
}

test "F32F32HashBiMap: remove" {
    var m = F32F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F32F32HashBiMap: removeValue" {
    var m = F32F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    _ = m.put(2.0, 2.0);
...
test "F32F32HashBiMap: containsKey and containsValue" {
    var m = F32F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
    try std.testing.expect(m.containsKey(1.0));
...
test "F32F32HashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?f32, 1.0), inv.get(1.0));
    try std.testing.expectEqual(@as(?f32, 2.0), inv.get(2.0));
}

test "F32F32HashBiMap: display format" {
    var m = F32F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1.0);
...
test "F32F32HashBiMap: eql" {
    var m1 = F32F32HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1.0);
    _ = m1.put(2.0, 2.0);
    var m2 = F32F32HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2.0);
    _ = m2.put(1.0, 1.0);
...
test "F32F32HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = F32F32HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32F32HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = F32F32HashBiMap.init(failing.allocator());
    defer m.deinit();
