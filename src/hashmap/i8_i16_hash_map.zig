...
const ImmutableI8I16HashMap = @import("../immutable/immutable_i8_i16_hash_map.zig").ImmutableI8I16HashMap;

/// Hash map from `i8` keys to `i16` values.
///
...
pub const I8I16HashMap = struct {
    inner: OpenHashMap(i8, i16),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I8I16HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) I8I16HashMap {
        return .{
            .inner = OpenHashMap(i8, i16).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) I8I16HashMap {
        return .{
            .inner = OpenHashMap(i8, i16).init(
    ...
    pub fn deinit(self: *I8I16HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *I8I16HashMap, key: i8, value: i16) ?i16 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const I8I16HashMap, key: i8) ?i16 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const I8I16HashMap, key: i8, default_value: i16) i16 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *I8I16HashMap, key: i8) ?i16 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const I8I16HashMap, key: i8) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const I8I16HashMap, value: i16) bool {
    ...
    pub fn getEntry(self: *I8I16HashMap, key: i8) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const I8I16HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const I8I16HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const I8I16HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *I8I16HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *I8I16HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8I16HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8I16HashMap, f: *const fn (i8, i16) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const I8I16HashMap, f: *const fn (i8) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const I8I16HashMap, f: *const fn (i16) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I8I16HashMap, predicate: *const fn (i8, i16) bool) I8I16HashMap {
    ...
    pub fn reject(self: *const I8I16HashMap, predicate: *const fn (i8, i16) bool) I8I16HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const I8I16HashMap, predicate: *const fn (i8, i16) bool) ?struct { key: i8, value: i16 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const I8I16HashMap, predicate: *const fn (i8, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const I8I16HashMap, predicate: *const fn (i8, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const I8I16HashMap, predicate: *const fn (i8, i16) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I8I16HashMap, predicate: *const fn (i8, i16) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const I8I16HashMap, initial: i16, f: *const fn (i16, i8, i16) i16) i16 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const I8I16HashMap, allocator: Allocator) []i8 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const I8I16HashMap, allocator: Allocator) []i16 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const I8I16HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *I8I16HashMap, key: i8, delta: i16) i16 {
    ...
    pub fn toImmutable(self: *const I8I16HashMap) ImmutableI8I16HashMap {
        return ImmutableI8I16HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *I8I16HashMap, key: i8, value: i16) *I8I16HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I8I16HashMap, key: i8) *I8I16HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *I8I16HashMap, keys: []const i8) *I8I16HashMap {
    ...
    pub fn updateValue(self: *I8I16HashMap, key: i8, initial: i16, f: *const fn (i16) i16) i16 {
    ...
    pub fn format(self: *const I8I16HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const I8I16HashMap, other: *const I8I16HashMap) bool {
...
test "I8I16HashMap: put and get" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    _ = m.put(3, 3);
...
test "I8I16HashMap: put overwrite" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    const old = m.put(1, 2);
...
test "I8I16HashMap: containsKey and containsValue" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    try std.testing.expect(m.containsKey(1));
...
test "I8I16HashMap: getOrDefault" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
...
test "I8I16HashMap: select and reject" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I8I16HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I8I16HashMap: keysToSlice and valuesToSlice" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
...
test "I8I16HashMap: fluent withKeyValue/withoutKey" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8I16HashMap: eql" {
    var m1 = I8I16HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);
    var m2 = I8I16HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I8I16HashMap: resize" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(i8, @intCast(i)), @as(i16, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "I8I16HashMap: entry orInsert" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8I16HashMap: entry andModify orInsert" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8I16HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8I16HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = I8I16HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8I16HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I8I16HashMap.init(failing.allocator());
    defer m.deinit();
