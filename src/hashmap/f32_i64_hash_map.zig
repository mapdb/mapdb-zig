...
const ImmutableF32I64HashMap = @import("../immutable/immutable_f32_i64_hash_map.zig").ImmutableF32I64HashMap;

/// Hash map from `f32` keys to `i64` values.
///
...
pub const F32I64HashMap = struct {
    inner: OpenHashMap(f32, i64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F32I64HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) F32I64HashMap {
        return .{
            .inner = OpenHashMap(f32, i64).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F32I64HashMap {
        return .{
            .inner = OpenHashMap(f32, i64).init(
    ...
    pub fn deinit(self: *F32I64HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *F32I64HashMap, key: f32, value: i64) ?i64 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const F32I64HashMap, key: f32) ?i64 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const F32I64HashMap, key: f32, default_value: i64) i64 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *F32I64HashMap, key: f32) ?i64 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const F32I64HashMap, key: f32) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const F32I64HashMap, value: i64) bool {
    ...
    pub fn getEntry(self: *F32I64HashMap, key: f32) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const F32I64HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const F32I64HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const F32I64HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *F32I64HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F32I64HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F32I64HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32I64HashMap, f: *const fn (f32, i64) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const F32I64HashMap, f: *const fn (f32) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const F32I64HashMap, f: *const fn (i64) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F32I64HashMap, predicate: *const fn (f32, i64) bool) F32I64HashMap {
    ...
    pub fn reject(self: *const F32I64HashMap, predicate: *const fn (f32, i64) bool) F32I64HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const F32I64HashMap, predicate: *const fn (f32, i64) bool) ?struct { key: f32, value: i64 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const F32I64HashMap, predicate: *const fn (f32, i64) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const F32I64HashMap, predicate: *const fn (f32, i64) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const F32I64HashMap, predicate: *const fn (f32, i64) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F32I64HashMap, predicate: *const fn (f32, i64) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const F32I64HashMap, initial: i64, f: *const fn (i64, f32, i64) i64) i64 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const F32I64HashMap, allocator: Allocator) []f32 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const F32I64HashMap, allocator: Allocator) []i64 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const F32I64HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *F32I64HashMap, key: f32, delta: i64) i64 {
    ...
    pub fn toImmutable(self: *const F32I64HashMap) ImmutableF32I64HashMap {
        return ImmutableF32I64HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *F32I64HashMap, key: f32, value: i64) *F32I64HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F32I64HashMap, key: f32) *F32I64HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *F32I64HashMap, keys: []const f32) *F32I64HashMap {
    ...
    pub fn updateValue(self: *F32I64HashMap, key: f32, initial: i64, f: *const fn (i64) i64) i64 {
    ...
    pub fn format(self: *const F32I64HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const F32I64HashMap, other: *const F32I64HashMap) bool {
...
test "F32I64HashMap: put and get" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
    _ = m.put(3.0, 3);
...
test "F32I64HashMap: put overwrite" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F32I64HashMap: containsKey and containsValue" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    try std.testing.expect(m.containsKey(1.0));
...
test "F32I64HashMap: getOrDefault" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F32I64HashMap: select and reject" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I64HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I64HashMap: keysToSlice and valuesToSlice" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I64HashMap: sumOfValues" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I64HashMap: toImmutable" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I64HashMap: fluent withKeyValue/withoutKey" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I64HashMap: eql" {
    var m1 = F32I64HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1);
    _ = m1.put(2.0, 2);
    var m2 = F32I64HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2);
    _ = m2.put(1.0, 1);
...
test "F32I64HashMap: resize" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(f32, @floatFromInt(i)), @as(i64, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "F32I64HashMap: entry orInsert" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I64HashMap: entry andModify orInsert" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I64HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I64HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I64HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F32I64HashMap.init(failing.allocator());
    defer m.deinit();
...
test "F32I64HashMap: NaN key findable" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I64HashMap: NaN key replaces, does not duplicate" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f32);
    _ = m.put(nan_key, 1);
...
test "F32I64HashMap: NaN key remove" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f32);
    _ = m.put(nan_key, 1);
...
test "F32I64HashMap: -0.0 distinct from +0.0" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I64HashMap: +/-Infinity keys" {
    var m = F32I64HashMap.init(std.testing.allocator);
    defer m.deinit();
