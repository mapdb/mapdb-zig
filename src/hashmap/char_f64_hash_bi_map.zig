...
const F64CharHashBiMap = @import("f64_char_hash_bi_map.zig").F64CharHashBiMap;

/// Bidirectional hash map from `u21` keys to `f64` values.
///
...
/// maps in sync.
pub const CharF64HashBiMap = struct {
    forward: OpenHashMap(u21, f64),
    reverse: OpenHashMap(f64, u21),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharF64HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) CharF64HashBiMap {
        return .{
            .forward = OpenHashMap(u21, f64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(f64, u21).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *CharF64HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *CharF64HashBiMap, key: u21, value: f64) ?f64 {
    ...
    pub fn removeValue(self: *CharF64HashBiMap, value: f64) ?u21 {
    ...
    pub fn containsKey(self: *const CharF64HashBiMap, key: u21) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const CharF64HashBiMap, value: f64) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const CharF64HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const CharF64HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *CharF64HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *CharF64HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharF64HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharF64HashBiMap, f: *const fn (u21, f64) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const CharF64HashBiMap, f: *const fn (u21) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const CharF64HashBiMap, f: *const fn (f64) void) void {
    ...
    pub fn inverse(self: *const CharF64HashBiMap) F64CharHashBiMap {
        var result = F64CharHashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const CharF64HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const CharF64HashBiMap, other: *const CharF64HashBiMap) bool {
...
test "CharF64HashBiMap: put and get and getKey" {
    var m = CharF64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
    // forward lookup
    try std.testing.expectEqual(@as(?f64, 1.0), m.get('a'));
    try std.testing.expectEqual(@as(?f64, 2.0), m.get('b'));
    // reverse lookup
...
test "CharF64HashBiMap: put overwrite updates reverse" {
    var m = CharF64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
...
test "CharF64HashBiMap: removeValue" {
    var m = CharF64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
...
test "CharF64HashBiMap: containsKey and containsValue" {
    var m = CharF64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    try std.testing.expect(m.containsKey('a'));
...
test "CharF64HashBiMap: display format" {
    var m = CharF64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
...
test "CharF64HashBiMap: eql" {
    var m1 = CharF64HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1.0);
    _ = m1.put('b', 2.0);
    var m2 = CharF64HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2.0);
    _ = m2.put('a', 1.0);
...
test "CharF64HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = CharF64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF64HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = CharF64HashBiMap.init(failing.allocator());
    defer m.deinit();
