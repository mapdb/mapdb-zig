...
const I64BoolHashBiMap = @import("i64_bool_hash_bi_map.zig").I64BoolHashBiMap;

/// Bidirectional hash map from `bool` keys to `i64` values.
///
...
/// maps in sync.
pub const BoolI64HashBiMap = struct {
    forward: OpenHashMap(bool, i64),
    reverse: OpenHashMap(i64, bool),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) BoolI64HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) BoolI64HashBiMap {
        return .{
            .forward = OpenHashMap(bool, i64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i64, bool).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *BoolI64HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *BoolI64HashBiMap, key: bool, value: i64) ?i64 {
    ...
    pub fn removeValue(self: *BoolI64HashBiMap, value: i64) ?bool {
    ...
    pub fn containsKey(self: *const BoolI64HashBiMap, key: bool) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const BoolI64HashBiMap, value: i64) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const BoolI64HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const BoolI64HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *BoolI64HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *BoolI64HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolI64HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolI64HashBiMap, f: *const fn (bool, i64) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const BoolI64HashBiMap, f: *const fn (bool) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const BoolI64HashBiMap, f: *const fn (i64) void) void {
    ...
    pub fn inverse(self: *const BoolI64HashBiMap) I64BoolHashBiMap {
        var result = I64BoolHashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const BoolI64HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const BoolI64HashBiMap, other: *const BoolI64HashBiMap) bool {
...
test "BoolI64HashBiMap: put and get and getKey" {
    var m = BoolI64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
    // forward lookup
    try std.testing.expectEqual(@as(?i64, 1), m.get(true));
    try std.testing.expectEqual(@as(?i64, 2), m.get(false));
    // reverse lookup
...
test "BoolI64HashBiMap: put overwrite updates reverse" {
    var m = BoolI64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
...
test "BoolI64HashBiMap: removeValue" {
    var m = BoolI64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI64HashBiMap: containsKey and containsValue" {
    var m = BoolI64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    try std.testing.expect(m.containsKey(true));
...
test "BoolI64HashBiMap: display format" {
    var m = BoolI64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
...
test "BoolI64HashBiMap: eql" {
    var m1 = BoolI64HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(true, 1);
    _ = m1.put(false, 2);
    var m2 = BoolI64HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(false, 2);
    _ = m2.put(true, 1);
...
test "BoolI64HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = BoolI64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI64HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = BoolI64HashBiMap.init(failing.allocator());
    defer m.deinit();
