...
const I64F64HashBiMap = @import("i64_f64_hash_bi_map.zig").I64F64HashBiMap;

/// Bidirectional hash map from `f64` keys to `i64` values.
///
...
/// maps in sync.
pub const F64I64HashBiMap = struct {
    forward: OpenHashMap(f64, i64),
    reverse: OpenHashMap(i64, f64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64I64HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F64I64HashBiMap {
        return .{
            .forward = OpenHashMap(f64, i64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i64, f64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *F64I64HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *F64I64HashBiMap, key: f64, value: i64) ?i64 {
    ...
    pub fn removeValue(self: *F64I64HashBiMap, value: i64) ?f64 {
    ...
    pub fn containsKey(self: *const F64I64HashBiMap, key: f64) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const F64I64HashBiMap, value: i64) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const F64I64HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const F64I64HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *F64I64HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F64I64HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64I64HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64I64HashBiMap, f: *const fn (f64, i64) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const F64I64HashBiMap, f: *const fn (f64) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const F64I64HashBiMap, f: *const fn (i64) void) void {
    ...
    pub fn inverse(self: *const F64I64HashBiMap) I64F64HashBiMap {
        var result = I64F64HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const F64I64HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const F64I64HashBiMap, other: *const F64I64HashBiMap) bool {
...
test "F64I64HashBiMap: put and get and getKey" {
    var m = F64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
    // forward lookup
...
test "F64I64HashBiMap: put overwrite updates reverse" {
    var m = F64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F64I64HashBiMap: removeValue" {
    var m = F64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I64HashBiMap: containsKey and containsValue" {
    var m = F64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    try std.testing.expect(m.containsKey(1.0));
...
test "F64I64HashBiMap: display format" {
    var m = F64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F64I64HashBiMap: eql" {
    var m1 = F64I64HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1);
    _ = m1.put(2.0, 2);
    var m2 = F64I64HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2);
    _ = m2.put(1.0, 1);
...
test "F64I64HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = F64I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = F64I64HashBiMap.init(failing.allocator());
    defer m.deinit();
