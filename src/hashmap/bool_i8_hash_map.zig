...
const ImmutableBoolI8HashMap = @import("../immutable/immutable_bool_i8_hash_map.zig").ImmutableBoolI8HashMap;
...
pub const BoolI8HashMap = struct {
    inner: OpenHashMap(bool, i8),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) BoolI8HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) BoolI8HashMap {
        return .{
            .inner = OpenHashMap(bool, i8).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) BoolI8HashMap {
        return .{
            .inner = OpenHashMap(bool, i8).init(
    ...
    pub fn deinit(self: *BoolI8HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *BoolI8HashMap, key: bool, value: i8) ?i8 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const BoolI8HashMap, key: bool) ?i8 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const BoolI8HashMap, key: bool, default_value: i8) i8 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *BoolI8HashMap, key: bool) ?i8 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const BoolI8HashMap, key: bool) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const BoolI8HashMap, value: i8) bool {
    ...
    pub fn getEntry(self: *BoolI8HashMap, key: bool) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const BoolI8HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const BoolI8HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const BoolI8HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *BoolI8HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *BoolI8HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolI8HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolI8HashMap, f: *const fn (bool, i8) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const BoolI8HashMap, f: *const fn (bool) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const BoolI8HashMap, f: *const fn (i8) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const BoolI8HashMap, predicate: *const fn (bool, i8) bool) BoolI8HashMap {
    ...
    pub fn reject(self: *const BoolI8HashMap, predicate: *const fn (bool, i8) bool) BoolI8HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const BoolI8HashMap, predicate: *const fn (bool, i8) bool) ?struct { key: bool, value: i8 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const BoolI8HashMap, predicate: *const fn (bool, i8) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const BoolI8HashMap, predicate: *const fn (bool, i8) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const BoolI8HashMap, predicate: *const fn (bool, i8) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const BoolI8HashMap, predicate: *const fn (bool, i8) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const BoolI8HashMap, initial: i8, f: *const fn (i8, bool, i8) i8) i8 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const BoolI8HashMap, allocator: Allocator) []bool {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const BoolI8HashMap, allocator: Allocator) []i8 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const BoolI8HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *BoolI8HashMap, key: bool, delta: i8) i8 {
    ...
    pub fn toImmutable(self: *const BoolI8HashMap) ImmutableBoolI8HashMap {
        return ImmutableBoolI8HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *BoolI8HashMap, key: bool, value: i8) *BoolI8HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *BoolI8HashMap, key: bool) *BoolI8HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *BoolI8HashMap, keys: []const bool) *BoolI8HashMap {
    ...
    pub fn updateValue(self: *BoolI8HashMap, key: bool, initial: i8, f: *const fn (i8) i8) i8 {
    ...
    pub fn format(self: *const BoolI8HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const BoolI8HashMap, other: *const BoolI8HashMap) bool {
...
test "BoolI8HashMap: put and get" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);

    try std.testing.expectEqual(@as(?i8, 1), m.get(true));

    try std.testing.expectEqual(@as(usize, 2), m.len());
}

test "BoolI8HashMap: put overwrite" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    const old = m.put(true, 2);
    try std.testing.expectEqual(@as(?i8, 1), old);
    try std.testing.expectEqual(@as(?i8, 2), m.get(true));
}

test "BoolI8HashMap: remove" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI8HashMap: containsKey and containsValue" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    try std.testing.expect(m.containsKey(true));
...
test "BoolI8HashMap: getOrDefault" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
...
test "BoolI8HashMap: keysToSlice and valuesToSlice" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI8HashMap: sumOfValues" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI8HashMap: toImmutable" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI8HashMap: fluent withKeyValue/withoutKey" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8HashMap: eql" {
    var m1 = BoolI8HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(true, 1);
    _ = m1.put(false, 2);
    var m2 = BoolI8HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(false, 2);
    _ = m2.put(true, 1);
...
test "BoolI8HashMap: resize" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(i % 2 == 0, @as(i8, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "BoolI8HashMap: entry orInsert" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8HashMap: entry andModify orInsert" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = BoolI8HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolI8HashMap.init(failing.allocator());
    defer m.deinit();
