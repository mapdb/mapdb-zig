...
const BoolF64HashBiMap = @import("bool_f64_hash_bi_map.zig").BoolF64HashBiMap;

/// Bidirectional hash map from `f64` keys to `bool` values.
///
...
/// maps in sync.
pub const F64BoolHashBiMap = struct {
    forward: OpenHashMap(f64, bool),
    reverse: OpenHashMap(bool, f64),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64BoolHashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F64BoolHashBiMap {
        return .{
            .forward = OpenHashMap(f64, bool).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(bool, f64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *F64BoolHashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *F64BoolHashBiMap, key: f64, value: bool) ?bool {
    ...
    pub fn removeValue(self: *F64BoolHashBiMap, value: bool) ?f64 {
    ...
    pub fn containsKey(self: *const F64BoolHashBiMap, key: f64) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const F64BoolHashBiMap, value: bool) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const F64BoolHashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const F64BoolHashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *F64BoolHashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F64BoolHashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64BoolHashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64BoolHashBiMap, f: *const fn (f64, bool) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const F64BoolHashBiMap, f: *const fn (f64) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const F64BoolHashBiMap, f: *const fn (bool) void) void {
    ...
    pub fn inverse(self: *const F64BoolHashBiMap) BoolF64HashBiMap {
        var result = BoolF64HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const F64BoolHashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const F64BoolHashBiMap, other: *const F64BoolHashBiMap) bool {
...
test "F64BoolHashBiMap: put and get and getKey" {
    var m = F64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
    // forward lookup
    try std.testing.expectEqual(@as(?bool, true), m.get(1.0));
    try std.testing.expectEqual(@as(?bool, false), m.get(2.0));
    // reverse lookup
    try std.testing.expectEqual(@as(?f64, 1.0), m.getKey(true));
    try std.testing.expectEqual(@as(?f64, 2.0), m.getKey(false));
    // missing key
...
test "F64BoolHashBiMap: put overwrite updates reverse" {
    var m = F64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    ...
    try std.testing.expectEqual(@as(?f64, null), m.getKey(true));
    // reverse for new value should point to key
    try std.testing.expectEqual(@as(?f64, 1.0), m.getKey(false));
}

test "F64BoolHashBiMap: remove" {
    var m = F64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
...
test "F64BoolHashBiMap: removeValue" {
    var m = F64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
...
test "F64BoolHashBiMap: containsKey and containsValue" {
    var m = F64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    try std.testing.expect(m.containsKey(1.0));
...
test "F64BoolHashBiMap: inverse" {
    ...
    try std.testing.expectEqual(@as(?f64, 1.0), inv.get(true));
    try std.testing.expectEqual(@as(?f64, 2.0), inv.get(false));
}

test "F64BoolHashBiMap: display format" {
    var m = F64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
...
test "F64BoolHashBiMap: eql" {
    var m1 = F64BoolHashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, true);
    _ = m1.put(2.0, false);
    var m2 = F64BoolHashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, false);
    _ = m2.put(1.0, true);
...
test "F64BoolHashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = F64BoolHashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolHashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = F64BoolHashBiMap.init(failing.allocator());
    defer m.deinit();
