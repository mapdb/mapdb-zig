...
const I64F32HashBiMap = @import("i64_f32_hash_bi_map.zig").I64F32HashBiMap;

/// Bidirectional hash map from `f32` keys to `i64` values.
///
...
/// maps in sync.
pub const F32I64HashBiMap = struct {
    forward: OpenHashMap(f32, i64),
    reverse: OpenHashMap(i64, f32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F32I64HashBiMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F32I64HashBiMap {
        return .{
            .forward = OpenHashMap(f32, i64).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
            .reverse = OpenHashMap(i64, f32).init(
                config.keysAllocator(),
                config.valuesAllocator(),
                config.indexAllocator(),
            ) catch @panic("out of memory"),
    ...
    pub fn deinit(self: *F32I64HashBiMap) void {
        self.forward.deinit();
        self.reverse.deinit();
    }
    ...
    pub fn put(self: *F32I64HashBiMap, key: f32, value: i64) ?i64 {
    ...
    pub fn removeValue(self: *F32I64HashBiMap, value: i64) ?f32 {
    ...
    pub fn containsKey(self: *const F32I64HashBiMap, key: f32) bool {
        return self.forward.containsKey(key);
    }

    pub fn containsValue(self: *const F32I64HashBiMap, value: i64) bool {
        return self.reverse.containsKey(value);
    }

    pub fn len(self: *const F32I64HashBiMap) usize {
        return self.forward.len();
    }

    pub fn isEmpty(self: *const F32I64HashBiMap) bool {
        return self.forward.isEmpty();
    }

    pub fn clear(self: *F32I64HashBiMap) void {
        self.forward.clear();
        self.reverse.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F32I64HashBiMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F32I64HashBiMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32I64HashBiMap, f: *const fn (f32, i64) void) void {
        self.forward.forEach(f);
    }

    pub fn forEachKey(self: *const F32I64HashBiMap, f: *const fn (f32) void) void {
        self.forward.forEachKey(f);
    }

    pub fn forEachValue(self: *const F32I64HashBiMap, f: *const fn (i64) void) void {
    ...
    pub fn inverse(self: *const F32I64HashBiMap) I64F32HashBiMap {
        var result = I64F32HashBiMap.init(self.config.base);
    ...
    pub fn format(self: *const F32I64HashBiMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.forward.capacity) |i| {
            if (self.forward.entries[i].occupied) {
    ...
    pub fn eql(self: *const F32I64HashBiMap, other: *const F32I64HashBiMap) bool {
...
test "F32I64HashBiMap: put and get and getKey" {
    var m = F32I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
    // forward lookup
    try std.testing.expectEqual(@as(?i64, 1), m.get(1.0));
    try std.testing.expectEqual(@as(?i64, 2), m.get(2.0));
    // reverse lookup
...
test "F32I64HashBiMap: put overwrite updates reverse" {
    var m = F32I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F32I64HashBiMap: removeValue" {
    var m = F32I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F32I64HashBiMap: containsKey and containsValue" {
    var m = F32I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    try std.testing.expect(m.containsKey(1.0));
...
test "F32I64HashBiMap: display format" {
    var m = F32I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F32I64HashBiMap: eql" {
    var m1 = F32I64HashBiMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1);
    _ = m1.put(2.0, 2);
    var m2 = F32I64HashBiMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2);
    _ = m2.put(1.0, 1);
...
test "F32I64HashBiMap: ensureUnusedCapacity reserves both directions" {
    var m = F32I64HashBiMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I64HashBiMap: ensureUnusedCapacity propagates allocator error" {
    // init allocates two tables (forward + reverse). Allow both, fail the
    // next alloc.
    var failing = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 2 });
    var m = F32I64HashBiMap.init(failing.allocator());
    defer m.deinit();
