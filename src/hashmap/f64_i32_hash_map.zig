...
const ImmutableF64I32HashMap = @import("../immutable/immutable_f64_i32_hash_map.zig").ImmutableF64I32HashMap;

/// Hash map from `f64` keys to `i32` values.
///
...
pub const F64I32HashMap = struct {
    inner: OpenHashMap(f64, i32),
    config: AllocatorConfig,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64I32HashMap {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    /// Create with pre-allocated capacity.
    pub fn initWithCapacity(allocator: Allocator, capacity: usize) F64I32HashMap {
        return .{
            .inner = OpenHashMap(f64, i32).initCapacity(allocator, capacity) catch @panic("out of memory"),
            .config = AllocatorConfig.init(allocator),
        };
    }

    /// Create with fine-grained allocator control.
    pub fn initWithConfig(config: AllocatorConfig) F64I32HashMap {
        return .{
            .inner = OpenHashMap(f64, i32).init(
    ...
    pub fn deinit(self: *F64I32HashMap) void {
        self.inner.deinit();
    }
    ...
    pub fn put(self: *F64I32HashMap, key: f64, value: i32) ?i32 {
        return self.inner.put(key, value) catch @panic("out of memory");
    }

    /// Returns the value for the key, or null.
    pub fn get(self: *const F64I32HashMap, key: f64) ?i32 {
        return self.inner.get(key);
    }

    /// Returns the value for the key, or the default.
    pub fn getOrDefault(self: *const F64I32HashMap, key: f64, default_value: i32) i32 {
        return self.get(key) orelse default_value;
    }

    /// Removes the key. Returns the old value if present.
    pub fn remove(self: *F64I32HashMap, key: f64) ?i32 {
        return self.inner.remove(key);
    }

    pub fn containsKey(self: *const F64I32HashMap, key: f64) bool {
        return self.inner.containsKey(key);
    }

    pub fn containsValue(self: *const F64I32HashMap, value: i32) bool {
    ...
    pub fn getEntry(self: *F64I32HashMap, key: f64) Entry {
        return .{ .map_ptr = self, .key = key };
    }

    pub fn len(self: *const F64I32HashMap) usize {
        return self.inner.len();
    }

    pub fn size(self: *const F64I32HashMap) usize {
        return self.inner.len();
    }

    pub fn isEmpty(self: *const F64I32HashMap) bool {
        return self.inner.isEmpty();
    }

    pub fn clear(self: *F64I32HashMap) void {
        self.inner.clear();
    }
    ...
    pub fn ensureUnusedCapacity(self: *F64I32HashMap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64I32HashMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64I32HashMap, f: *const fn (f64, i32) void) void {
        self.inner.forEach(f);
    }

    pub fn forEachKey(self: *const F64I32HashMap, f: *const fn (f64) void) void {
        self.inner.forEachKey(f);
    }

    pub fn forEachValue(self: *const F64I32HashMap, f: *const fn (i32) void) void {
        self.inner.forEachValue(f);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F64I32HashMap, predicate: *const fn (f64, i32) bool) F64I32HashMap {
    ...
    pub fn reject(self: *const F64I32HashMap, predicate: *const fn (f64, i32) bool) F64I32HashMap {
        var result = init(self.config.base);
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (!predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn detect(self: *const F64I32HashMap, predicate: *const fn (f64, i32) bool) ?struct { key: f64, value: i32 } {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
                if (predicate(self.inner.entries[i].key, self.inner.entries[i].value))
    ...
    pub fn anySatisfy(self: *const F64I32HashMap, predicate: *const fn (f64, i32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn allSatisfy(self: *const F64I32HashMap, predicate: *const fn (f64, i32) bool) bool {
        for (0..self.inner.capacity) |i| {
    ...
    pub fn noneSatisfy(self: *const F64I32HashMap, predicate: *const fn (f64, i32) bool) bool {
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied and predicate(self.inner.entries[i].key, self.inner.entries[i].value)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F64I32HashMap, predicate: *const fn (f64, i32) bool) usize {
        var c: usize = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn injectInto(self: *const F64I32HashMap, initial: i32, f: *const fn (i32, f64, i32) i32) i32 {
        var acc = initial;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn keysToSlice(self: *const F64I32HashMap, allocator: Allocator) []f64 {
        return self.inner.keysToSlice(allocator) catch @panic("out of memory");
    }

    pub fn valuesToSlice(self: *const F64I32HashMap, allocator: Allocator) []i32 {
        return self.inner.valuesToSlice(allocator) catch @panic("out of memory");
    }

    // ---- Numeric Value Operations ----

    pub fn sumOfValues(self: *const F64I32HashMap) i64 {
        var total: i64 = 0;
        for (0..self.inner.capacity) |i| {
    ...
    pub fn addToValue(self: *F64I32HashMap, key: f64, delta: i32) i32 {
    ...
    pub fn toImmutable(self: *const F64I32HashMap) ImmutableF64I32HashMap {
        return ImmutableF64I32HashMap.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    pub fn withKeyValue(self: *F64I32HashMap, key: f64, value: i32) *F64I32HashMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F64I32HashMap, key: f64) *F64I32HashMap {
        _ = self.remove(key);
        return self;
    }

    pub fn withoutAllKeys(self: *F64I32HashMap, keys: []const f64) *F64I32HashMap {
    ...
    pub fn updateValue(self: *F64I32HashMap, key: f64, initial: i32, f: *const fn (i32) i32) i32 {
    ...
    pub fn format(self: *const F64I32HashMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.inner.capacity) |i| {
            if (self.inner.entries[i].occupied) {
    ...
    pub fn eql(self: *const F64I32HashMap, other: *const F64I32HashMap) bool {
...
test "F64I32HashMap: put and get" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
    _ = m.put(3.0, 3);
...
test "F64I32HashMap: put overwrite" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F64I32HashMap: containsKey and containsValue" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    try std.testing.expect(m.containsKey(1.0));
...
test "F64I32HashMap: getOrDefault" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
...
test "F64I32HashMap: select and reject" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I32HashMap: anySatisfy allSatisfy noneSatisfy count" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I32HashMap: keysToSlice and valuesToSlice" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I32HashMap: sumOfValues" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I32HashMap: toImmutable" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
...
test "F64I32HashMap: fluent withKeyValue/withoutKey" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I32HashMap: eql" {
    var m1 = F64I32HashMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1);
    _ = m1.put(2.0, 2);
    var m2 = F64I32HashMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2);
    _ = m2.put(1.0, 1);
...
test "F64I32HashMap: resize" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        _ = m.put(@as(f64, @floatFromInt(i)), @as(i32, @intCast(i)));
    }
    try std.testing.expect(m.len() > 0);
}

test "F64I32HashMap: entry orInsert" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I32HashMap: entry andModify orInsert" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I32HashMap: ensureUnusedCapacity reserves and subsequent put does not resize" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I32HashMap: ensureTotalCapacity is idempotent when capacity already sufficient" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I32HashMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64I32HashMap.init(failing.allocator());
    defer m.deinit();
...
test "F64I32HashMap: NaN key findable" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I32HashMap: NaN key replaces, does not duplicate" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1);
...
test "F64I32HashMap: NaN key remove" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
    const nan_key = std.math.nan(f64);
    _ = m.put(nan_key, 1);
...
test "F64I32HashMap: -0.0 distinct from +0.0" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I32HashMap: +/-Infinity keys" {
    var m = F64I32HashMap.init(std.testing.allocator);
    defer m.deinit();
