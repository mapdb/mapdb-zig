...
/// Sorted bag (multiset) of `i8` values with occurrence counting.
///
...
pub const I8TreeBag = struct {
    ...
    pub fn init(allocator: Allocator) I8TreeBag {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    pub fn initWithConfig(config: AllocatorConfig) I8TreeBag {
        return .{ .config = config };
    }

    pub fn deinit(self: *I8TreeBag) void {
    ...
    fn addOccurrences(self: *I8TreeBag, value: i8, occ: usize) void {
    ...
    pub fn removeAll(self: *I8TreeBag, value: i8) bool {
        var entry = self.treap.getEntryFor(value);
        const treap_node = entry.node orelse return false;
        const bag_node = bagNodeFromTreapNode(treap_node);
    ...
    pub fn occurrencesOf(self: *const I8TreeBag, value: i8) usize {
    ...
    pub fn contains(self: *const I8TreeBag, value: i8) bool {
        var node = self.treap.root;
        while (node) |current| {
            const ord = orderFn(value, current.key);
    ...
    pub fn totalSize(self: *const I8TreeBag) usize {
        return self.total_size;
    }

    /// Number of distinct values.
    pub fn sizeDistinct(self: *const I8TreeBag) usize {
        return self.distinct_count;
    }

    /// Alias for totalSize().
    pub fn len(self: *const I8TreeBag) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I8TreeBag) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I8TreeBag) void {
        destroySubtree(self.treap.root, self.config.keysAllocator());
    ...
    pub fn ensureUnusedCapacity(self: *I8TreeBag, additional: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8TreeBag, f: *const fn (i8) void) void {
    ...
    pub fn forEachWithOccurrences(self: *const I8TreeBag, f: *const fn (i8, usize) void) void {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
    ...
    pub fn select(self: *const I8TreeBag, predicate: *const fn (i8) bool) I8TreeBag {
    ...
    pub fn reject(self: *const I8TreeBag, predicate: *const fn (i8) bool) I8TreeBag {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
            if (!predicate(treap_node.key)) {
    ...
    pub fn detect(self: *const I8TreeBag, predicate: *const fn (i8) bool) ?i8 {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
    ...
    pub fn anySatisfy(self: *const I8TreeBag, predicate: *const fn (i8) bool) bool {
        return self.detect(predicate) != null;
    }

    pub fn allSatisfy(self: *const I8TreeBag, predicate: *const fn (i8) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
    ...
    pub fn noneSatisfy(self: *const I8TreeBag, predicate: *const fn (i8) bool) bool {
        return self.detect(predicate) == null;
    }
    ...
    pub fn toSlice(self: *const I8TreeBag, allocator: Allocator) []i8 {
        var buf = std.ArrayListUnmanaged(i8){};
    ...
    pub fn with(self: *I8TreeBag, value: i8) *I8TreeBag {
        self.add(value);
        return self;
    }

    pub fn without(self: *I8TreeBag, value: i8) *I8TreeBag {
        _ = self.remove(value);
        return self;
    }
    ...
    pub fn format(self: *const I8TreeBag, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
    ...
    pub fn eql(self: *const I8TreeBag, other: *const I8TreeBag) bool {
...
test "I8TreeBag: add and occurrences" {
    var b = I8TreeBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(1);
    b.add(1);
    b.add(2);
...
test "I8TreeBag: sorted order and min/max" {
    var b = I8TreeBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(3);
    b.add(1);
    b.add(2);
    try std.testing.expectEqual(@as(?i8, 1), b.min());
    try std.testing.expectEqual(@as(?i8, 3), b.max());
}

test "I8TreeBag: select and reject" {
    var b = I8TreeBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(1);
    b.add(2);
...
test "I8TreeBag: fluent with/without" {
    var b = I8TreeBag.init(std.testing.allocator);
    defer b.deinit();
...
test "I8TreeBag: eql" {
    var b1 = I8TreeBag.init(std.testing.allocator);
    defer b1.deinit();
    b1.add(1);
    b1.add(1);
    b1.add(2);
    var b2 = I8TreeBag.init(std.testing.allocator);
    defer b2.deinit();
    b2.add(2);
    b2.add(1);
    b2.add(1);
...
test "I8TreeBag: forEachWithOccurrences" {
    var b = I8TreeBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(1);
    b.add(1);
    ...
    b.forEachWithOccurrences(struct {
        fn f(_: i8, _: usize) void {}
    }.f);
    try std.testing.expectEqual(@as(usize, 2), b.sizeDistinct());
}

test "I8TreeBag: ensureUnusedCapacity probes allocator" {
    var b = I8TreeBag.init(std.testing.allocator);
    defer b.deinit();
...
test "I8TreeBag: ensureUnusedCapacity propagates allocator error" {
    ...
    var b = I8TreeBag.init(failing.allocator());
    defer b.deinit();
