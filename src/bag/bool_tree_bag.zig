...
pub const BoolTreeBag = struct {
    ...
    pub fn init(allocator: Allocator) BoolTreeBag {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    pub fn initWithConfig(config: AllocatorConfig) BoolTreeBag {
        return .{ .config = config };
    }

    pub fn deinit(self: *BoolTreeBag) void {
    ...
    fn addOccurrences(self: *BoolTreeBag, value: bool, occ: usize) void {
    ...
    pub fn add(self: *BoolTreeBag, value: bool) void {
        self.addOccurrences(value, 1);
    }

    /// Remove one occurrence. Returns true if the value was present.
    pub fn remove(self: *BoolTreeBag, value: bool) bool {
        var entry = self.treap.getEntryFor(value);
    ...
    pub fn removeAll(self: *BoolTreeBag, value: bool) bool {
        var entry = self.treap.getEntryFor(value);
        const treap_node = entry.node orelse return false;
        const bag_node = bagNodeFromTreapNode(treap_node);
    ...
    pub fn occurrencesOf(self: *const BoolTreeBag, value: bool) usize {
    ...
    pub fn contains(self: *const BoolTreeBag, value: bool) bool {
        var node = self.treap.root;
        while (node) |current| {
            const ord = orderFn(value, current.key);
    ...
    pub fn totalSize(self: *const BoolTreeBag) usize {
        return self.total_size;
    }

    /// Number of distinct values.
    pub fn sizeDistinct(self: *const BoolTreeBag) usize {
        return self.distinct_count;
    }

    /// Alias for totalSize().
    pub fn len(self: *const BoolTreeBag) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const BoolTreeBag) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *BoolTreeBag) void {
        destroySubtree(self.treap.root, self.config.keysAllocator());
    ...
    pub fn ensureUnusedCapacity(self: *BoolTreeBag, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolTreeBag, new_capacity: usize) Allocator.Error!void {
        if (new_capacity <= self.distinct_count) return;
        try self.ensureUnusedCapacity(new_capacity - self.distinct_count);
    }

    pub fn min(self: *const BoolTreeBag) ?bool {
        const node = self.treap.getMin() orelse return null;
        return node.key;
    }

    pub fn max(self: *const BoolTreeBag) ?bool {
    ...
    pub fn forEach(self: *const BoolTreeBag, f: *const fn (bool) void) void {
    ...
    pub fn forEachWithOccurrences(self: *const BoolTreeBag, f: *const fn (bool, usize) void) void {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
    ...
    pub fn select(self: *const BoolTreeBag, predicate: *const fn (bool) bool) BoolTreeBag {
    ...
    pub fn reject(self: *const BoolTreeBag, predicate: *const fn (bool) bool) BoolTreeBag {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
            if (!predicate(treap_node.key)) {
    ...
    pub fn detect(self: *const BoolTreeBag, predicate: *const fn (bool) bool) ?bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
    ...
    pub fn anySatisfy(self: *const BoolTreeBag, predicate: *const fn (bool) bool) bool {
        return self.detect(predicate) != null;
    }

    pub fn allSatisfy(self: *const BoolTreeBag, predicate: *const fn (bool) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
    ...
    pub fn noneSatisfy(self: *const BoolTreeBag, predicate: *const fn (bool) bool) bool {
        return self.detect(predicate) == null;
    }
    ...
    pub fn toSlice(self: *const BoolTreeBag, allocator: Allocator) []bool {
        var buf = std.ArrayListUnmanaged(bool){};
    ...
    pub fn with(self: *BoolTreeBag, value: bool) *BoolTreeBag {
        self.add(value);
        return self;
    }

    pub fn without(self: *BoolTreeBag, value: bool) *BoolTreeBag {
        _ = self.remove(value);
        return self;
    }
    ...
    pub fn format(self: *const BoolTreeBag, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |treap_node| {
    ...
    pub fn eql(self: *const BoolTreeBag, other: *const BoolTreeBag) bool {
...
test "BoolTreeBag: add and occurrences" {
    var b = BoolTreeBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(true);
    b.add(true);
    b.add(false);
...
test "BoolTreeBag: sorted order and min/max" {
    var b = BoolTreeBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(true);
    b.add(false);
    try std.testing.expectEqual(@as(?bool, false), b.min());
    try std.testing.expectEqual(@as(?bool, true), b.max());
}

test "BoolTreeBag: select and reject" {
    var b = BoolTreeBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(true);
    b.add(false);
...
test "BoolTreeBag: fluent with/without" {
    var b = BoolTreeBag.init(std.testing.allocator);
    defer b.deinit();
...
test "BoolTreeBag: eql" {
    var b1 = BoolTreeBag.init(std.testing.allocator);
    defer b1.deinit();
    b1.add(true);
    b1.add(true);
    b1.add(false);
    var b2 = BoolTreeBag.init(std.testing.allocator);
    defer b2.deinit();
    b2.add(false);
    b2.add(true);
    b2.add(true);
...
test "BoolTreeBag: forEachWithOccurrences" {
    var b = BoolTreeBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(true);
    b.add(true);
    ...
    b.forEachWithOccurrences(struct {
        fn f(_: bool, _: usize) void {}
    }.f);
    try std.testing.expectEqual(@as(usize, 2), b.sizeDistinct());
}

test "BoolTreeBag: ensureUnusedCapacity probes allocator" {
    var b = BoolTreeBag.init(std.testing.allocator);
    defer b.deinit();
...
test "BoolTreeBag: ensureUnusedCapacity propagates allocator error" {
    ...
    var b = BoolTreeBag.init(failing.allocator());
    defer b.deinit();
