...
/// Bag (multiset) of `i8` values with occurrence counting.
///
/// Backed by OpenHashMap(i8, usize).
pub const I8HashBag = struct {
    counts: OpenHashMap(i8, usize),
    size: usize,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) I8HashBag {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    pub fn initWithConfig(config: AllocatorConfig) I8HashBag {
        return .{
            .counts = OpenHashMap(i8, usize).init(config.keysAllocator(), config.indexAllocator(), config.keysAllocator()) catch @panic("out of memory"),
            .size = 0,
            .config = config,
        };
    }

    pub fn deinit(self: *I8HashBag) void {
        self.counts.deinit();
    }

    pub fn of(allocator: Allocator, values: []const i8) I8HashBag {
    ...
    pub fn add(self: *I8HashBag, value: i8) void {
    ...
    pub fn remove(self: *I8HashBag, value: i8) bool {
        if (self.counts.getPtr(value)) |count_ptr| {
            count_ptr.* -= 1;
            if (count_ptr.* == 0) {
    ...
    pub fn removeAll(self: *I8HashBag, value: i8) bool {
        if (self.counts.getPtr(value)) |count_ptr| {
            self.size -= count_ptr.*;
            _ = self.counts.remove(value);
            return true;
    ...
    pub fn occurrencesOf(self: *const I8HashBag, value: i8) usize {
        return self.counts.get(value) orelse 0;
    }

    pub fn contains(self: *const I8HashBag, value: i8) bool {
        return self.occurrencesOf(value) > 0;
    }

    /// Total number of elements (counting duplicates).
    pub fn totalSize(self: *const I8HashBag) usize {
        return self.size;
    }

    /// Number of distinct values.
    pub fn sizeDistinct(self: *const I8HashBag) usize {
        return self.counts.len();
    }

    pub fn isEmpty(self: *const I8HashBag) bool {
        return self.size == 0;
    }

    pub fn clear(self: *I8HashBag) void {
        self.counts.clear();
        self.size = 0;
    }
    ...
    pub fn ensureUnusedCapacity(self: *I8HashBag, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8HashBag, new_capacity: usize) Allocator.Error!void {
        const cur = self.counts.len();
        if (new_capacity <= cur) return;
        return self.counts.ensureCapacity(new_capacity - cur);
    }

    pub fn len(self: *const I8HashBag) usize {
        return self.size;
    }

    /// Add multiple occurrences of a value.
    pub fn addOccurrences(self: *I8HashBag, value: i8, n: usize) void {
        var i: usize = 0;
        while (i < n) : (i += 1) self.add(value);
    }

    /// Remove up to n occurrences. Returns number actually removed.
    pub fn removeOccurrences(self: *I8HashBag, value: i8, n: usize) usize {
        var removed: usize = 0;
        var i: usize = 0;
        while (i < n) : (i += 1) {
    ...
    pub fn forEachWithOccurrences(self: *const I8HashBag, f: *const fn (i8, usize) void) void {
    ...
    pub fn forEach(self: *const I8HashBag, f: *const fn (i8) void) void {
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
                var j: usize = 0;
    ...
    pub fn select(self: *const I8HashBag, predicate: *const fn (i8) bool) I8HashBag {
    ...
    pub fn reject(self: *const I8HashBag, predicate: *const fn (i8) bool) I8HashBag {
        var result = init(self.config.base);
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
                if (!predicate(self.counts.entries[i].key)) result.addOccurrences(self.counts.entries[i].key, self.counts.entries[i].value);
    ...
    pub fn detect(self: *const I8HashBag, predicate: *const fn (i8) bool) ?i8 {
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
    ...
    pub fn anySatisfy(self: *const I8HashBag, predicate: *const fn (i8) bool) bool {
        return self.detect(predicate) != null;
    }

    pub fn allSatisfy(self: *const I8HashBag, predicate: *const fn (i8) bool) bool {
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
    ...
    pub fn noneSatisfy(self: *const I8HashBag, predicate: *const fn (i8) bool) bool {
        return self.detect(predicate) == null;
    }
    ...
    pub fn toSlice(self: *const I8HashBag, allocator: Allocator) []i8 {
    ...
    pub fn with(self: *I8HashBag, value: i8) *I8HashBag {
        self.add(value);
        return self;
    }

    pub fn without(self: *I8HashBag, value: i8) *I8HashBag {
        _ = self.remove(value);
        return self;
    }

    pub fn withAll(self: *I8HashBag, values: []const i8) *I8HashBag {
        for (values) |val| self.add(val);
        return self;
    }

    pub fn withoutAll(self: *I8HashBag, values: []const i8) *I8HashBag {
    ...
    pub fn topOccurrences(self: *const I8HashBag, allocator: Allocator, n: usize) []struct { value: i8, count: usize } {
        const Entry = struct { value: i8, count: usize };
    ...
    pub fn format(self: *const I8HashBag, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
    ...
    pub fn eql(self: *const I8HashBag, other: *const I8HashBag) bool {
...
test "I8HashBag: add and occurrences" {
    var b = I8HashBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(1);
    b.add(1);
    b.add(2);
...
test "I8HashBag: remove" {
    var b = I8HashBag.of(std.testing.allocator, &[_]i8{ 1, 1, 1 });
    defer b.deinit();
    try std.testing.expect(b.remove(1));
    try std.testing.expectEqual(@as(usize, 2), b.occurrencesOf(1));
}

test "I8HashBag: removeAll" {
    var b = I8HashBag.of(std.testing.allocator, &[_]i8{ 1, 1, 2 });
    defer b.deinit();
...
test "I8HashBag: clear" {
    var b = I8HashBag.of(std.testing.allocator, &[_]i8{1});
    defer b.deinit();
    b.clear();
    try std.testing.expect(b.isEmpty());
}

test "I8HashBag: addOccurrences" {
    var b = I8HashBag.init(std.testing.allocator);
    defer b.deinit();
...
test "I8HashBag: select and reject" {
    var b = I8HashBag.of(std.testing.allocator, &[_]i8{ 1, 2, 3 });
    defer b.deinit();
...
test "I8HashBag: toImmutable" {
    var b = I8HashBag.of(std.testing.allocator, &[_]i8{ 1, 1 });
    defer b.deinit();
...
test "I8HashBag: fluent with/without" {
    var b = I8HashBag.init(std.testing.allocator);
    defer b.deinit();
...
test "I8HashBag: eql" {
    var b1 = I8HashBag.of(std.testing.allocator, &[_]i8{ 1, 1, 2 });
    defer b1.deinit();
    var b2 = I8HashBag.of(std.testing.allocator, &[_]i8{ 2, 1, 1 });
    defer b2.deinit();
    try std.testing.expect(b1.eql(&b2));
}

test "I8HashBag: ensureUnusedCapacity reserves the count map" {
    var b = I8HashBag.init(std.testing.allocator);
    defer b.deinit();
...
test "I8HashBag: ensureUnusedCapacity propagates allocator error" {
    ...
    var b = I8HashBag.init(failing.allocator());
    defer b.deinit();
