...
/// Bag (multiset) of `i64` values with occurrence counting.
///
/// Backed by OpenHashMap(i64, usize).
pub const I64HashBag = struct {
    counts: OpenHashMap(i64, usize),
    size: usize,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) I64HashBag {
        return initWithConfig(AllocatorConfig.init(allocator));
    }

    pub fn initWithConfig(config: AllocatorConfig) I64HashBag {
        return .{
            .counts = OpenHashMap(i64, usize).init(config.keysAllocator(), config.indexAllocator(), config.keysAllocator()) catch @panic("out of memory"),
            .size = 0,
            .config = config,
        };
    }

    pub fn deinit(self: *I64HashBag) void {
        self.counts.deinit();
    }

    pub fn of(allocator: Allocator, values: []const i64) I64HashBag {
    ...
    pub fn add(self: *I64HashBag, value: i64) void {
    ...
    pub fn remove(self: *I64HashBag, value: i64) bool {
        if (self.counts.getPtr(value)) |count_ptr| {
            count_ptr.* -= 1;
            if (count_ptr.* == 0) {
    ...
    pub fn removeAll(self: *I64HashBag, value: i64) bool {
        if (self.counts.getPtr(value)) |count_ptr| {
            self.size -= count_ptr.*;
            _ = self.counts.remove(value);
            return true;
    ...
    pub fn occurrencesOf(self: *const I64HashBag, value: i64) usize {
        return self.counts.get(value) orelse 0;
    }

    pub fn contains(self: *const I64HashBag, value: i64) bool {
        return self.occurrencesOf(value) > 0;
    }

    /// Total number of elements (counting duplicates).
    pub fn totalSize(self: *const I64HashBag) usize {
        return self.size;
    }

    /// Number of distinct values.
    pub fn sizeDistinct(self: *const I64HashBag) usize {
        return self.counts.len();
    }

    pub fn isEmpty(self: *const I64HashBag) bool {
        return self.size == 0;
    }

    pub fn clear(self: *I64HashBag) void {
        self.counts.clear();
        self.size = 0;
    }
    ...
    pub fn ensureUnusedCapacity(self: *I64HashBag, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I64HashBag, new_capacity: usize) Allocator.Error!void {
        const cur = self.counts.len();
        if (new_capacity <= cur) return;
        return self.counts.ensureCapacity(new_capacity - cur);
    }

    pub fn len(self: *const I64HashBag) usize {
        return self.size;
    }

    /// Add multiple occurrences of a value.
    pub fn addOccurrences(self: *I64HashBag, value: i64, n: usize) void {
        var i: usize = 0;
        while (i < n) : (i += 1) self.add(value);
    }

    /// Remove up to n occurrences. Returns number actually removed.
    pub fn removeOccurrences(self: *I64HashBag, value: i64, n: usize) usize {
        var removed: usize = 0;
        var i: usize = 0;
        while (i < n) : (i += 1) {
    ...
    pub fn forEachWithOccurrences(self: *const I64HashBag, f: *const fn (i64, usize) void) void {
    ...
    pub fn forEach(self: *const I64HashBag, f: *const fn (i64) void) void {
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
                var j: usize = 0;
    ...
    pub fn select(self: *const I64HashBag, predicate: *const fn (i64) bool) I64HashBag {
    ...
    pub fn reject(self: *const I64HashBag, predicate: *const fn (i64) bool) I64HashBag {
        var result = init(self.config.base);
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
                if (!predicate(self.counts.entries[i].key)) result.addOccurrences(self.counts.entries[i].key, self.counts.entries[i].value);
    ...
    pub fn detect(self: *const I64HashBag, predicate: *const fn (i64) bool) ?i64 {
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
    ...
    pub fn anySatisfy(self: *const I64HashBag, predicate: *const fn (i64) bool) bool {
        return self.detect(predicate) != null;
    }

    pub fn allSatisfy(self: *const I64HashBag, predicate: *const fn (i64) bool) bool {
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
    ...
    pub fn noneSatisfy(self: *const I64HashBag, predicate: *const fn (i64) bool) bool {
        return self.detect(predicate) == null;
    }
    ...
    pub fn toSlice(self: *const I64HashBag, allocator: Allocator) []i64 {
    ...
    pub fn with(self: *I64HashBag, value: i64) *I64HashBag {
        self.add(value);
        return self;
    }

    pub fn without(self: *I64HashBag, value: i64) *I64HashBag {
        _ = self.remove(value);
        return self;
    }

    pub fn withAll(self: *I64HashBag, values: []const i64) *I64HashBag {
        for (values) |val| self.add(val);
        return self;
    }

    pub fn withoutAll(self: *I64HashBag, values: []const i64) *I64HashBag {
    ...
    pub fn topOccurrences(self: *const I64HashBag, allocator: Allocator, n: usize) []struct { value: i64, count: usize } {
        const Entry = struct { value: i64, count: usize };
    ...
    pub fn format(self: *const I64HashBag, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        for (0..self.counts.capacity) |i| {
            if (self.counts.entries[i].occupied) {
    ...
    pub fn eql(self: *const I64HashBag, other: *const I64HashBag) bool {
...
test "I64HashBag: add and occurrences" {
    var b = I64HashBag.init(std.testing.allocator);
    defer b.deinit();
    b.add(1);
    b.add(1);
    b.add(2);
...
test "I64HashBag: remove" {
    var b = I64HashBag.of(std.testing.allocator, &[_]i64{ 1, 1, 1 });
    defer b.deinit();
    try std.testing.expect(b.remove(1));
    try std.testing.expectEqual(@as(usize, 2), b.occurrencesOf(1));
}

test "I64HashBag: removeAll" {
    var b = I64HashBag.of(std.testing.allocator, &[_]i64{ 1, 1, 2 });
    defer b.deinit();
...
test "I64HashBag: clear" {
    var b = I64HashBag.of(std.testing.allocator, &[_]i64{1});
    defer b.deinit();
    b.clear();
    try std.testing.expect(b.isEmpty());
}

test "I64HashBag: addOccurrences" {
    var b = I64HashBag.init(std.testing.allocator);
    defer b.deinit();
...
test "I64HashBag: select and reject" {
    var b = I64HashBag.of(std.testing.allocator, &[_]i64{ 1, 2, 3 });
    defer b.deinit();
...
test "I64HashBag: toImmutable" {
    var b = I64HashBag.of(std.testing.allocator, &[_]i64{ 1, 1 });
    defer b.deinit();
...
test "I64HashBag: fluent with/without" {
    var b = I64HashBag.init(std.testing.allocator);
    defer b.deinit();
...
test "I64HashBag: eql" {
    var b1 = I64HashBag.of(std.testing.allocator, &[_]i64{ 1, 1, 2 });
    defer b1.deinit();
    var b2 = I64HashBag.of(std.testing.allocator, &[_]i64{ 2, 1, 1 });
    defer b2.deinit();
    try std.testing.expect(b1.eql(&b2));
}

test "I64HashBag: ensureUnusedCapacity reserves the count map" {
    var b = I64HashBag.init(std.testing.allocator);
    defer b.deinit();
...
test "I64HashBag: ensureUnusedCapacity propagates allocator error" {
    ...
    var b = I64HashBag.init(failing.allocator());
    defer b.deinit();
