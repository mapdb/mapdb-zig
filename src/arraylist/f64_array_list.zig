...
pub const F64ArrayList = struct {
    ...
    pub fn get(self: *const F64ArrayList, index: usize) ?f64 {
        if (index >= self.items.items.len) return null;
        return self.items.items[index];
    }

    /// Sets the element at the given index. Returns the old value.
    pub fn set(self: *F64ArrayList, index: usize, value: f64) f64 {
    ...
    pub fn removeAtIndex(self: *F64ArrayList, index: usize) f64 {
        return self.items.orderedRemove(index);
    }

    /// Removes the first occurrence of the value. Returns true if found.
    pub fn remove(self: *F64ArrayList, value: f64) bool {
    ...
    pub fn contains(self: *const F64ArrayList, value: f64) bool {
    ...
    pub fn indexOf(self: *const F64ArrayList, value: f64) ?usize {
    ...
    pub fn len(self: *const F64ArrayList) usize {
        return self.items.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const F64ArrayList) usize {
        return self.len();
    }

    /// Returns true if the list is empty.
    pub fn isEmpty(self: *const F64ArrayList) bool {
        return self.items.items.len == 0;
    }

    /// Alias for push() — matches Go/Java Add naming.
    pub fn add(self: *F64ArrayList, value: f64) void {
        self.push(value);
    }

    /// Alias for pushAll() — matches Go/Java AddAll naming.
    pub fn addAll(self: *F64ArrayList, values: []const f64) void {
        self.pushAll(values);
    }

    /// Removes all elements.
    pub fn clear(self: *F64ArrayList) void {
    ...
    pub fn ensureUnusedCapacity(self: *F64ArrayList, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64ArrayList, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn select(self: *const F64ArrayList, predicate: *const fn (f64) bool) F64ArrayList {
    ...
    pub fn reject(self: *const F64ArrayList, predicate: *const fn (f64) bool) F64ArrayList {
        var result = init(self.config.base);
        for (self.items.items) |item| {
            if (!predicate(item)) result.push(item);
        }
    ...
    pub fn detect(self: *const F64ArrayList, predicate: *const fn (f64) bool) ?f64 {
        for (self.items.items) |item| {
    ...
    pub fn anySatisfy(self: *const F64ArrayList, predicate: *const fn (f64) bool) bool {
        for (self.items.items) |item| {
    ...
    pub fn allSatisfy(self: *const F64ArrayList, predicate: *const fn (f64) bool) bool {
        for (self.items.items) |item| {
    ...
    pub fn noneSatisfy(self: *const F64ArrayList, predicate: *const fn (f64) bool) bool {
        for (self.items.items) |item| {
            if (predicate(item)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F64ArrayList, predicate: *const fn (f64) bool) usize {
        var c: usize = 0;
        for (self.items.items) |item| {
    ...
    pub fn sum(self: *const F64ArrayList) f64 {
        var total: f64 = 0;
        for (self.items.items) |item| {
            total += item;
        }
    ...
    pub fn min(self: *const F64ArrayList) ?f64 {
    ...
    pub fn max(self: *const F64ArrayList) ?f64 {
        if (self.items.items.len == 0) return null;
        var result = self.items.items[0];
        for (self.items.items[1..]) |item| {
    ...
    pub fn sort(self: *F64ArrayList) void {
        std.mem.sort(f64, self.items.items, {}, struct {
            pub fn f(_: void, a: f64, b: f64) bool {
                return float_order.totalCmpF64(a, b) == .lt;
            }
        }.f);
    }

    /// Returns a new list with elements in reverse order.
    pub fn reversed(self: *const F64ArrayList) F64ArrayList {
        var result = init(self.config.base);
    ...
    pub fn toSlice(self: *const F64ArrayList) []const f64 {
    ...
    pub fn forEach(self: *const F64ArrayList, f: *const fn (f64) void) void {
        for (self.items.items) |item| f(item);
    }

    /// Calls f(index, value) for each element.
    pub fn forEachWithIndex(self: *const F64ArrayList, f: *const fn (usize, f64) void) void {
    ...
    pub fn injectInto(self: *const F64ArrayList, initial: f64, f: *const fn (f64, f64) f64) f64 {
        var acc = initial;
    ...
    pub fn distinct(self: *const F64ArrayList) F64ArrayList {
        var result = init(self.config.base);
        for (self.items.items) |item| {
    ...
    pub fn binarySearch(self: *const F64ArrayList, value: f64) ?usize {
        var lo: usize = 0;
        var hi: usize = self.items.items.len;
        while (lo < hi) {
            const mid = lo + (hi - lo) / 2;
            const ord = float_order.totalCmpF64(self.items.items[mid], value);
            if (ord == .lt) {
                lo = mid + 1;
            } else if (ord == .gt) {
                hi = mid;
    ...
    pub fn toImmutable(self: *const F64ArrayList) @import("../immutable/immutable_f64_array_list.zig").ImmutableF64ArrayList {
        return @import("../immutable/immutable_f64_array_list.zig").ImmutableF64ArrayList.fromMutable(self.config.base, self);
    }

    // ---- Fluent API ----

    /// Appends a value (fluent).
    pub fn with(self: *F64ArrayList, value: f64) *F64ArrayList {
        self.push(value);
        return self;
    }

    /// Removes the first occurrence (fluent).
    pub fn without(self: *F64ArrayList, value: f64) *F64ArrayList {
        _ = self.remove(value);
        return self;
    }

    /// Appends all values (fluent).
    pub fn withAll(self: *F64ArrayList, values: []const f64) *F64ArrayList {
        self.pushAll(values);
        return self;
    }

    /// Removes all occurrences of each value (fluent).
    pub fn withoutAll(self: *F64ArrayList, values: []const f64) *F64ArrayList {
    ...
    pub fn format(self: *const F64ArrayList, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("[");
        for (self.items.items, 0..) |item, i| {
    ...
    pub fn eql(self: *const F64ArrayList, other: *const F64ArrayList) bool {
...
test "F64ArrayList: push and get" {
    var l = F64ArrayList.init(std.testing.allocator);
    defer l.deinit();
    l.push(1.0);
    l.push(2.0);
    l.push(3.0);
    try std.testing.expectEqual(@as(usize, 3), l.len());
    try std.testing.expectEqual(@as(?f64, 1.0), l.get(0));
    try std.testing.expectEqual(@as(?f64, 3.0), l.get(2));
    try std.testing.expectEqual(@as(?f64, null), l.get(99));
}

test "F64ArrayList: of" {
    var l = F64ArrayList.of(std.testing.allocator, &[_]f64{ 1.0, 2.0, 3.0 });
    defer l.deinit();
    try std.testing.expectEqual(@as(usize, 3), l.len());
}

test "F64ArrayList: set" {
    var l = F64ArrayList.of(std.testing.allocator, &[_]f64{ 1.0, 2.0 });
    defer l.deinit();
    const old = l.set(0, 3.0);
    try std.testing.expectEqual(1.0, old);
    try std.testing.expectEqual(@as(?f64, 3.0), l.get(0));
}

test "F64ArrayList: removeAtIndex" {
    var l = F64ArrayList.of(std.testing.allocator, &[_]f64{ 1.0, 2.0, 3.0 });
    defer l.deinit();
...
test "F64ArrayList: isEmpty and clear" {
    var l = F64ArrayList.init(std.testing.allocator);
    defer l.deinit();
...
test "F64ArrayList: sum min max" {
    var l = F64ArrayList.of(std.testing.allocator, &[_]f64{ 3.0, 1.0, 2.0 });
    defer l.deinit();
    try std.testing.expectEqual(@as(?f64, 1.0), l.min());
    try std.testing.expectEqual(@as(?f64, 3.0), l.max());
}

test "F64ArrayList: sort" {
    var l = F64ArrayList.of(std.testing.allocator, &[_]f64{ 3.0, 1.0, 2.0 });
    defer l.deinit();
    l.sort();
    try std.testing.expectEqual(@as(?f64, 1.0), l.get(0));
    try std.testing.expectEqual(@as(?f64, 2.0), l.get(1));
    try std.testing.expectEqual(@as(?f64, 3.0), l.get(2));
}

test "F64ArrayList: reversed" {
    var l = F64ArrayList.of(std.testing.allocator, &[_]f64{ 1.0, 2.0, 3.0 });
    defer l.deinit();
    var r = l.reversed();
    defer r.deinit();
    try std.testing.expectEqual(@as(?f64, 3.0), r.get(0));
    try std.testing.expectEqual(@as(?f64, 1.0), r.get(2));
}

test "F64ArrayList: eql" {
    var l1 = F64ArrayList.of(std.testing.allocator, &[_]f64{ 1.0, 2.0 });
    defer l1.deinit();
    var l2 = F64ArrayList.of(std.testing.allocator, &[_]f64{ 1.0, 2.0 });
    defer l2.deinit();
    var l3 = F64ArrayList.of(std.testing.allocator, &[_]f64{1.0});
    defer l3.deinit();
    try std.testing.expect(l1.eql(&l2));
    try std.testing.expect(!l1.eql(&l3));
}

test "F64ArrayList: resize" {
    var l = F64ArrayList.init(std.testing.allocator);
    defer l.deinit();
    var i: i32 = 0;
    while (i < 100) : (i += 1) {
        l.push(@as(f64, @floatFromInt(i)));
    }
    try std.testing.expectEqual(@as(usize, 100), l.len());
}

test "F64ArrayList: distinct" {
    var l = F64ArrayList.of(std.testing.allocator, &[_]f64{ 1.0, 2.0, 1.0, 3.0, 2.0 });
    defer l.deinit();
    var d = l.distinct();
    defer d.deinit();
...
test "F64ArrayList: binarySearch" {
    var l = F64ArrayList.of(std.testing.allocator, &[_]f64{ 1.0, 2.0, 3.0 });
    defer l.deinit();
    l.sort();
...
test "F64ArrayList: fluent with/without" {
    var l = F64ArrayList.init(std.testing.allocator);
    defer l.deinit();
...
test "F64ArrayList: ensureUnusedCapacity reserves and subsequent push does not reallocate" {
    var l = F64ArrayList.init(std.testing.allocator);
    defer l.deinit();
...
test "F64ArrayList: ensureTotalCapacity sets minimum capacity" {
    var l = F64ArrayList.init(std.testing.allocator);
    defer l.deinit();
...
test "F64ArrayList: ensureUnusedCapacity propagates allocator error" {
    ...
    var l = F64ArrayList.initWithConfig(AllocatorConfig.init(failing.allocator()));
    defer l.deinit();
