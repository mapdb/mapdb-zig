...
/// Comptime interface for read-only maps from `i64` keys to `f64` values.
///
/// Equivalent to:
///   Go:   I64F64MapIterable interface
///   Rust: I64F64Map trait
///   Java: I64F64Map interface
pub fn assertI64F64Map(comptime T: type) void {
    comptime {
...
/// Comptime interface for mutable maps from `i64` to `f64`.
pub fn assertI64F64MutableMap(comptime T: type) void {
    assertI64F64Map(T);
    comptime {
...
// Compile-time verification: every concrete `i64` → `f64` map type
...
comptime {
    ...
    const HashMap = hashmap.i64_f64_hash_map.I64F64HashMap;
    assertI64F64MutableMap(HashMap);

    const TreeMap = treemap.i64_f64_tree_map.I64F64TreeMap;
    assertI64F64MutableMap(TreeMap);

    const HashBiMap = hashmap.i64_f64_hash_bi_map.I64F64HashBiMap;
    assertI64F64MutableMap(HashBiMap);

    const ImmutableHashMap = immutable.immutable_i64_f64_hash_map.ImmutableI64F64HashMap;
    assertI64F64Map(ImmutableHashMap);
}
