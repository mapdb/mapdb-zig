...
/// Comptime interface for read-only maps from `f64` keys to `i32` values.
///
/// Equivalent to:
///   Go:   F64I32MapIterable interface
///   Rust: F64I32Map trait
///   Java: F64I32Map interface
pub fn assertF64I32Map(comptime T: type) void {
    comptime {
...
/// Comptime interface for mutable maps from `f64` to `i32`.
pub fn assertF64I32MutableMap(comptime T: type) void {
    assertF64I32Map(T);
    comptime {
...
// Compile-time verification: every concrete `f64` → `i32` map type
...
comptime {
    ...
    const HashMap = hashmap.f64_i32_hash_map.F64I32HashMap;
    assertF64I32MutableMap(HashMap);

    const TreeMap = treemap.f64_i32_tree_map.F64I32TreeMap;
    assertF64I32MutableMap(TreeMap);

    const HashBiMap = hashmap.f64_i32_hash_bi_map.F64I32HashBiMap;
    assertF64I32MutableMap(HashBiMap);

    const ImmutableHashMap = immutable.immutable_f64_i32_hash_map.ImmutableF64I32HashMap;
    assertF64I32Map(ImmutableHashMap);
}
