...
/// Sorted set of unique `f32` values, backed by `std.Treap` for
/// O(log n) insert, remove, and lookup.
pub const F32TreeSet = struct {
    ...
    pub fn init(allocator: Allocator) F32TreeSet {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) F32TreeSet {
        return .{ .config = config };
    }

    pub fn deinit(self: *F32TreeSet) void {
    ...
    pub fn of(allocator: Allocator, values: []const f32) F32TreeSet {
    ...
    pub fn add(self: *F32TreeSet, value: f32) bool {
    ...
    pub fn remove(self: *F32TreeSet, value: f32) bool {
        var entry = self.treap.getEntryFor(value);
    ...
    pub fn contains(self: *const F32TreeSet, value: f32) bool {
    ...
    pub fn len(self: *const F32TreeSet) usize {
        return self.node_count;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const F32TreeSet) usize {
        return self.node_count;
    }

    pub fn isEmpty(self: *const F32TreeSet) bool {
        return self.node_count == 0;
    }

    pub fn clear(self: *F32TreeSet) void {
        destroySubtree(self.treap.root, self.config.keysAllocator());
    ...
    pub fn ensureUnusedCapacity(self: *F32TreeSet, additional: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32TreeSet, f: *const fn (f32) void) void {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
    ...
    pub fn select(self: *const F32TreeSet, predicate: *const fn (f32) bool) F32TreeSet {
    ...
    pub fn reject(self: *const F32TreeSet, predicate: *const fn (f32) bool) F32TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (!predicate(node.key)) _ = result.add(node.key);
    ...
    pub fn detect(self: *const F32TreeSet, predicate: *const fn (f32) bool) ?f32 {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn anySatisfy(self: *const F32TreeSet, predicate: *const fn (f32) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn allSatisfy(self: *const F32TreeSet, predicate: *const fn (f32) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn noneSatisfy(self: *const F32TreeSet, predicate: *const fn (f32) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (predicate(node.key)) return false;
        }
    ...
    pub fn count(self: *const F32TreeSet, predicate: *const fn (f32) bool) usize {
        var c: usize = 0;
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn setUnion(self: *const F32TreeSet, other: *const F32TreeSet) F32TreeSet {
        var result = init(self.config.base);
    ...
    pub fn intersect(self: *const F32TreeSet, other: *const F32TreeSet) F32TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn difference(self: *const F32TreeSet, other: *const F32TreeSet) F32TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (!other.contains(node.key)) _ = result.add(node.key);
    ...
    fn ceilingNode(self: *const F32TreeSet, value: f32) ?*TreapType.Node {
        var node = self.treap.root;
    ...
    pub fn rangeValues(self: *const F32TreeSet, from: f32, to: f32, allocator: Allocator) []f32 {
        var result = std.ArrayListUnmanaged(f32){};
    ...
    pub fn format(self: *const F32TreeSet, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn with(self: *F32TreeSet, value: f32) *F32TreeSet {
        _ = self.add(value);
        return self;
    }

    pub fn without(self: *F32TreeSet, value: f32) *F32TreeSet {
        _ = self.remove(value);
        return self;
    }
    ...
    pub fn eql(self: *const F32TreeSet, other: *const F32TreeSet) bool {
...
test "F32TreeSet: add and contains" {
    var set = F32TreeSet.init(std.testing.allocator);
    defer set.deinit();
    _ = set.add(1.0);
    _ = set.add(2.0);
    _ = set.add(3.0);
...
test "F32TreeSet: sorted iteration" {
    var set = F32TreeSet.of(std.testing.allocator, &[_]f32{ 3.0, 1.0, 2.0 });
    defer set.deinit();
...
test "F32TreeSet: min max" {
    var set = F32TreeSet.of(std.testing.allocator, &[_]f32{ 3.0, 1.0, 2.0 });
    defer set.deinit();
    try std.testing.expectEqual(@as(?f32, 1.0), set.min());
    try std.testing.expectEqual(@as(?f32, 3.0), set.max());
}

test "F32TreeSet: remove" {
    var set = F32TreeSet.of(std.testing.allocator, &[_]f32{ 1.0, 2.0 });
    defer set.deinit();
    try std.testing.expect(set.remove(1.0));
    try std.testing.expect(!set.contains(1.0));
}

test "F32TreeSet: union" {
    var a = F32TreeSet.of(std.testing.allocator, &[_]f32{ 1.0, 2.0 });
    defer a.deinit();
    var b = F32TreeSet.of(std.testing.allocator, &[_]f32{ 2.0, 3.0 });
    defer b.deinit();
    var u = a.setUnion(&b);
    defer u.deinit();
...
test "F32TreeSet: ensureUnusedCapacity probes allocator" {
    var set = F32TreeSet.init(std.testing.allocator);
    defer set.deinit();
...
test "F32TreeSet: ensureUnusedCapacity propagates allocator error" {
    ...
    var set = F32TreeSet.init(failing.allocator());
    defer set.deinit();
