...
/// Sorted set of unique `i32` values, backed by `std.Treap` for
/// O(log n) insert, remove, and lookup.
pub const I32TreeSet = struct {
    ...
    pub fn init(allocator: Allocator) I32TreeSet {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) I32TreeSet {
        return .{ .config = config };
    }

    pub fn deinit(self: *I32TreeSet) void {
    ...
    pub fn of(allocator: Allocator, values: []const i32) I32TreeSet {
    ...
    pub fn add(self: *I32TreeSet, value: i32) bool {
    ...
    pub fn remove(self: *I32TreeSet, value: i32) bool {
        var entry = self.treap.getEntryFor(value);
    ...
    pub fn contains(self: *const I32TreeSet, value: i32) bool {
    ...
    pub fn len(self: *const I32TreeSet) usize {
        return self.node_count;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const I32TreeSet) usize {
        return self.node_count;
    }

    pub fn isEmpty(self: *const I32TreeSet) bool {
        return self.node_count == 0;
    }

    pub fn clear(self: *I32TreeSet) void {
        destroySubtree(self.treap.root, self.config.keysAllocator());
    ...
    pub fn ensureUnusedCapacity(self: *I32TreeSet, additional: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32TreeSet, f: *const fn (i32) void) void {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
    ...
    pub fn select(self: *const I32TreeSet, predicate: *const fn (i32) bool) I32TreeSet {
    ...
    pub fn reject(self: *const I32TreeSet, predicate: *const fn (i32) bool) I32TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (!predicate(node.key)) _ = result.add(node.key);
    ...
    pub fn detect(self: *const I32TreeSet, predicate: *const fn (i32) bool) ?i32 {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn anySatisfy(self: *const I32TreeSet, predicate: *const fn (i32) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn allSatisfy(self: *const I32TreeSet, predicate: *const fn (i32) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn noneSatisfy(self: *const I32TreeSet, predicate: *const fn (i32) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (predicate(node.key)) return false;
        }
    ...
    pub fn count(self: *const I32TreeSet, predicate: *const fn (i32) bool) usize {
        var c: usize = 0;
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn setUnion(self: *const I32TreeSet, other: *const I32TreeSet) I32TreeSet {
        var result = init(self.config.base);
    ...
    pub fn intersect(self: *const I32TreeSet, other: *const I32TreeSet) I32TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn difference(self: *const I32TreeSet, other: *const I32TreeSet) I32TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (!other.contains(node.key)) _ = result.add(node.key);
    ...
    fn ceilingNode(self: *const I32TreeSet, value: i32) ?*TreapType.Node {
        var node = self.treap.root;
    ...
    pub fn rangeValues(self: *const I32TreeSet, from: i32, to: i32, allocator: Allocator) []i32 {
        var result = std.ArrayListUnmanaged(i32){};
    ...
    pub fn format(self: *const I32TreeSet, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn with(self: *I32TreeSet, value: i32) *I32TreeSet {
        _ = self.add(value);
        return self;
    }

    pub fn without(self: *I32TreeSet, value: i32) *I32TreeSet {
        _ = self.remove(value);
        return self;
    }
    ...
    pub fn eql(self: *const I32TreeSet, other: *const I32TreeSet) bool {
...
test "I32TreeSet: add and contains" {
    var set = I32TreeSet.init(std.testing.allocator);
    defer set.deinit();
    _ = set.add(1);
    _ = set.add(2);
    _ = set.add(3);
...
test "I32TreeSet: sorted iteration" {
    var set = I32TreeSet.of(std.testing.allocator, &[_]i32{ 3, 1, 2 });
    defer set.deinit();
...
test "I32TreeSet: min max" {
    var set = I32TreeSet.of(std.testing.allocator, &[_]i32{ 3, 1, 2 });
    defer set.deinit();
    try std.testing.expectEqual(@as(?i32, 1), set.min());
    try std.testing.expectEqual(@as(?i32, 3), set.max());
}

test "I32TreeSet: remove" {
    var set = I32TreeSet.of(std.testing.allocator, &[_]i32{ 1, 2 });
    defer set.deinit();
    try std.testing.expect(set.remove(1));
    try std.testing.expect(!set.contains(1));
}

test "I32TreeSet: union" {
    var a = I32TreeSet.of(std.testing.allocator, &[_]i32{ 1, 2 });
    defer a.deinit();
    var b = I32TreeSet.of(std.testing.allocator, &[_]i32{ 2, 3 });
    defer b.deinit();
    var u = a.setUnion(&b);
    defer u.deinit();
...
test "I32TreeSet: ensureUnusedCapacity probes allocator" {
    var set = I32TreeSet.init(std.testing.allocator);
    defer set.deinit();
...
test "I32TreeSet: ensureUnusedCapacity propagates allocator error" {
    ...
    var set = I32TreeSet.init(failing.allocator());
    defer set.deinit();
