...
/// Sorted set of unique `i64` values, backed by `std.Treap` for
/// O(log n) insert, remove, and lookup.
pub const I64TreeSet = struct {
    ...
    pub fn init(allocator: Allocator) I64TreeSet {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) I64TreeSet {
        return .{ .config = config };
    }

    pub fn deinit(self: *I64TreeSet) void {
    ...
    pub fn of(allocator: Allocator, values: []const i64) I64TreeSet {
    ...
    pub fn add(self: *I64TreeSet, value: i64) bool {
    ...
    pub fn remove(self: *I64TreeSet, value: i64) bool {
        var entry = self.treap.getEntryFor(value);
    ...
    pub fn contains(self: *const I64TreeSet, value: i64) bool {
    ...
    pub fn len(self: *const I64TreeSet) usize {
        return self.node_count;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const I64TreeSet) usize {
        return self.node_count;
    }

    pub fn isEmpty(self: *const I64TreeSet) bool {
        return self.node_count == 0;
    }

    pub fn clear(self: *I64TreeSet) void {
        destroySubtree(self.treap.root, self.config.keysAllocator());
    ...
    pub fn ensureUnusedCapacity(self: *I64TreeSet, additional: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I64TreeSet, f: *const fn (i64) void) void {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
    ...
    pub fn select(self: *const I64TreeSet, predicate: *const fn (i64) bool) I64TreeSet {
    ...
    pub fn reject(self: *const I64TreeSet, predicate: *const fn (i64) bool) I64TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (!predicate(node.key)) _ = result.add(node.key);
    ...
    pub fn detect(self: *const I64TreeSet, predicate: *const fn (i64) bool) ?i64 {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn anySatisfy(self: *const I64TreeSet, predicate: *const fn (i64) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn allSatisfy(self: *const I64TreeSet, predicate: *const fn (i64) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn noneSatisfy(self: *const I64TreeSet, predicate: *const fn (i64) bool) bool {
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (predicate(node.key)) return false;
        }
    ...
    pub fn count(self: *const I64TreeSet, predicate: *const fn (i64) bool) usize {
        var c: usize = 0;
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn setUnion(self: *const I64TreeSet, other: *const I64TreeSet) I64TreeSet {
        var result = init(self.config.base);
    ...
    pub fn intersect(self: *const I64TreeSet, other: *const I64TreeSet) I64TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn difference(self: *const I64TreeSet, other: *const I64TreeSet) I64TreeSet {
        var result = init(self.config.base);
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
            if (!other.contains(node.key)) _ = result.add(node.key);
    ...
    fn ceilingNode(self: *const I64TreeSet, value: i64) ?*TreapType.Node {
        var node = self.treap.root;
    ...
    pub fn rangeValues(self: *const I64TreeSet, from: i64, to: i64, allocator: Allocator) []i64 {
        var result = std.ArrayListUnmanaged(i64){};
    ...
    pub fn format(self: *const I64TreeSet, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = TreapType.InorderIterator{ .current = self.treap.getMin() };
        while (it.next()) |node| {
    ...
    pub fn with(self: *I64TreeSet, value: i64) *I64TreeSet {
        _ = self.add(value);
        return self;
    }

    pub fn without(self: *I64TreeSet, value: i64) *I64TreeSet {
        _ = self.remove(value);
        return self;
    }
    ...
    pub fn eql(self: *const I64TreeSet, other: *const I64TreeSet) bool {
...
test "I64TreeSet: add and contains" {
    var set = I64TreeSet.init(std.testing.allocator);
    defer set.deinit();
    _ = set.add(1);
    _ = set.add(2);
    _ = set.add(3);
...
test "I64TreeSet: sorted iteration" {
    var set = I64TreeSet.of(std.testing.allocator, &[_]i64{ 3, 1, 2 });
    defer set.deinit();
...
test "I64TreeSet: min max" {
    var set = I64TreeSet.of(std.testing.allocator, &[_]i64{ 3, 1, 2 });
    defer set.deinit();
    try std.testing.expectEqual(@as(?i64, 1), set.min());
    try std.testing.expectEqual(@as(?i64, 3), set.max());
}

test "I64TreeSet: remove" {
    var set = I64TreeSet.of(std.testing.allocator, &[_]i64{ 1, 2 });
    defer set.deinit();
    try std.testing.expect(set.remove(1));
    try std.testing.expect(!set.contains(1));
}

test "I64TreeSet: union" {
    var a = I64TreeSet.of(std.testing.allocator, &[_]i64{ 1, 2 });
    defer a.deinit();
    var b = I64TreeSet.of(std.testing.allocator, &[_]i64{ 2, 3 });
    defer b.deinit();
    var u = a.setUnion(&b);
    defer u.deinit();
...
test "I64TreeSet: ensureUnusedCapacity probes allocator" {
    var set = I64TreeSet.init(std.testing.allocator);
    defer set.deinit();
...
test "I64TreeSet: ensureUnusedCapacity propagates allocator error" {
    ...
    var set = I64TreeSet.init(failing.allocator());
    defer set.deinit();
