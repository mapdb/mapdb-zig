...
pub const F32PriorityQueue = struct {
    items: std.ArrayListUnmanaged(f32) = .empty,
    allocator: Allocator,

    pub fn init(allocator: Allocator) F32PriorityQueue {
        return .{ .allocator = allocator };
    }

    pub fn deinit(self: *F32PriorityQueue) void {
        self.items.deinit(self.allocator);
    }

    pub fn of(allocator: Allocator, values: []const f32) F32PriorityQueue {
    ...
    pub fn push(self: *F32PriorityQueue, value: f32) void {
    ...
    pub fn pop(self: *F32PriorityQueue) ?f32 {
    ...
    pub fn peek(self: *const F32PriorityQueue) ?f32 {
        if (self.items.items.len == 0) return null;
        return self.items.items[0];
    }

    pub fn len(self: *const F32PriorityQueue) usize {
        return self.items.items.len;
    }
    pub fn isEmpty(self: *const F32PriorityQueue) bool {
        return self.items.items.len == 0;
    }
    pub fn clear(self: *F32PriorityQueue) void {
        self.items.clearRetainingCapacity();
    }

    pub fn ensureUnusedCapacity(self: *F32PriorityQueue, additional: usize) Allocator.Error!void {
        try self.items.ensureUnusedCapacity(self.allocator, additional);
    }

    pub fn contains(self: *const F32PriorityQueue, value: f32) bool {
    ...
    pub fn slice(self: *const F32PriorityQueue) []const f32 {
        return self.items.items;
    }

    /// Drains the heap into a caller-owned slice in ascending order.
    pub fn drainSorted(self: *F32PriorityQueue, allocator: Allocator) []f32 {
    ...
    fn siftUp(self: *F32PriorityQueue, start: usize) void {
        var i = start;
        while (i > 0) {
            const parent = (i - 1) / 2;
    ...
    fn siftDown(self: *F32PriorityQueue, start: usize) void {
        var i = start;
...
test "F32PriorityQueue: push peek pop min" {
    var q = F32PriorityQueue.init(std.testing.allocator);
    defer q.deinit();
    q.push(3.0);
    q.push(1.0);
    q.push(2.0);
    ...
    try std.testing.expectEqual(@as(?f32, 1.0), q.peek());
    const a = q.pop().?;
    const b = q.pop().?;
    const c = q.pop().?;
    ...
    try std.testing.expectEqual(@as(?f32, null), q.pop());
}

test "F32PriorityQueue: of heapify" {
    var q = F32PriorityQueue.of(std.testing.allocator, &[_]f32{ 3.0, 1.0, 2.0 });
    defer q.deinit();
    try std.testing.expectEqual(@as(usize, 3), q.len());
    try std.testing.expectEqual(@as(?f32, 1.0), q.peek());
}

test "F32PriorityQueue: empty" {
    var q = F32PriorityQueue.init(std.testing.allocator);
    defer q.deinit();
    try std.testing.expect(q.isEmpty());
    try std.testing.expectEqual(@as(?f32, null), q.peek());
    try std.testing.expectEqual(@as(?f32, null), q.pop());
}

test "F32PriorityQueue: contains clear" {
    var q = F32PriorityQueue.init(std.testing.allocator);
    defer q.deinit();
    q.push(1.0);
...
test "F32PriorityQueue: drainSorted" {
    var q = F32PriorityQueue.of(std.testing.allocator, &[_]f32{ 3.0, 1.0, 2.0 });
    defer q.deinit();
