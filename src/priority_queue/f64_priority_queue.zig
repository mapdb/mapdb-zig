...
pub const F64PriorityQueue = struct {
    ...
    fn siftUp(self: *F64PriorityQueue, start: usize) void {
        ...
        while (i > 0) {
            const parent = (i - 1) / 2;
            if (float_order.totalCmpF64(self.items.items[i], self.items.items[parent]) == .lt) {
    ...
    fn siftDown(self: *F64PriorityQueue, start: usize) void {
        ...
        while (true) {
            ...
            const right = left + 1;
            var best = left;
            if (right < n and float_order.totalCmpF64(self.items.items[right], self.items.items[left]) == .lt) {
                best = right;
            }
            if (float_order.totalCmpF64(self.items.items[best], self.items.items[i]) == .lt) {
