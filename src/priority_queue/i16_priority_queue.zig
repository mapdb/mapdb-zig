...
/// Primitive min-heap priority queue of `i16` values, backed by an
/// `ArrayListUnmanaged`. O(log n) push/pop, O(1) peek.
pub const I16PriorityQueue = struct {
    items: std.ArrayListUnmanaged(i16) = .empty,
    allocator: Allocator,

    pub fn init(allocator: Allocator) I16PriorityQueue {
        return .{ .allocator = allocator };
    }

    pub fn deinit(self: *I16PriorityQueue) void {
        self.items.deinit(self.allocator);
    }

    pub fn of(allocator: Allocator, values: []const i16) I16PriorityQueue {
    ...
    pub fn push(self: *I16PriorityQueue, value: i16) void {
    ...
    pub fn pop(self: *I16PriorityQueue) ?i16 {
    ...
    pub fn peek(self: *const I16PriorityQueue) ?i16 {
        if (self.items.items.len == 0) return null;
        return self.items.items[0];
    }

    pub fn len(self: *const I16PriorityQueue) usize {
        return self.items.items.len;
    }
    pub fn isEmpty(self: *const I16PriorityQueue) bool {
        return self.items.items.len == 0;
    }
    pub fn clear(self: *I16PriorityQueue) void {
        self.items.clearRetainingCapacity();
    }

    pub fn ensureUnusedCapacity(self: *I16PriorityQueue, additional: usize) Allocator.Error!void {
        try self.items.ensureUnusedCapacity(self.allocator, additional);
    }

    pub fn contains(self: *const I16PriorityQueue, value: i16) bool {
    ...
    pub fn slice(self: *const I16PriorityQueue) []const i16 {
        return self.items.items;
    }

    /// Drains the heap into a caller-owned slice in ascending order.
    pub fn drainSorted(self: *I16PriorityQueue, allocator: Allocator) []i16 {
    ...
    fn siftUp(self: *I16PriorityQueue, start: usize) void {
        var i = start;
        while (i > 0) {
            const parent = (i - 1) / 2;
    ...
    fn siftDown(self: *I16PriorityQueue, start: usize) void {
        var i = start;
...
test "I16PriorityQueue: push peek pop min" {
    var q = I16PriorityQueue.init(std.testing.allocator);
    defer q.deinit();
    q.push(3);
    q.push(1);
    q.push(2);
    ...
    try std.testing.expectEqual(@as(?i16, 1), q.peek());
    const a = q.pop().?;
    const b = q.pop().?;
    const c = q.pop().?;
    ...
    try std.testing.expectEqual(@as(?i16, null), q.pop());
}

test "I16PriorityQueue: of heapify" {
    var q = I16PriorityQueue.of(std.testing.allocator, &[_]i16{ 3, 1, 2 });
    defer q.deinit();
    try std.testing.expectEqual(@as(usize, 3), q.len());
    try std.testing.expectEqual(@as(?i16, 1), q.peek());
}

test "I16PriorityQueue: empty" {
    var q = I16PriorityQueue.init(std.testing.allocator);
    defer q.deinit();
    try std.testing.expect(q.isEmpty());
    try std.testing.expectEqual(@as(?i16, null), q.peek());
    try std.testing.expectEqual(@as(?i16, null), q.pop());
}

test "I16PriorityQueue: contains clear" {
    var q = I16PriorityQueue.init(std.testing.allocator);
    defer q.deinit();
    q.push(1);
...
test "I16PriorityQueue: drainSorted" {
    var q = I16PriorityQueue.of(std.testing.allocator, &[_]i16{ 3, 1, 2 });
    defer q.deinit();
