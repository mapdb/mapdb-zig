...
/// List multimap from `f64` keys to `f64` values.
///
...
pub const F64F64ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(u64, std.ArrayListUnmanaged(f64)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64F64ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u64, std.ArrayListUnmanaged(f64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *F64F64ListMultimap) void {
    ...
    pub fn put(self: *F64F64ListMultimap, key: f64, value: f64) void {
    ...
    pub fn get(self: *const F64F64ListMultimap, key: f64) []const f64 {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn getCount(self: *const F64F64ListMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *F64F64ListMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn containsKey(self: *const F64F64ListMultimap, key: f64) bool {
        const map_key = @as(u64, @bitCast(key));
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const F64F64ListMultimap, key: f64, value: f64) bool {
    ...
    pub fn keysCount(self: *const F64F64ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const F64F64ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const F64F64ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const F64F64ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *F64F64ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *F64F64ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64F64ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64F64ListMultimap, f: *const fn (f64, f64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const F64F64ListMultimap, predicate: *const fn (f64, f64) bool) F64F64ListMultimap {
        var result = F64F64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const F64F64ListMultimap, predicate: *const fn (f64, f64) bool) F64F64ListMultimap {
        var result = F64F64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn anySatisfy(self: *const F64F64ListMultimap, predicate: *const fn (f64, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const F64F64ListMultimap, predicate: *const fn (f64, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const F64F64ListMultimap, predicate: *const fn (f64, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const F64F64ListMultimap, predicate: *const fn (f64, f64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn uniqueKeys(self: *const F64F64ListMultimap, allocator: Allocator) []f64 {
    ...
    pub fn valuesToSlice(self: *const F64F64ListMultimap, allocator: Allocator) []f64 {
        var result = std.ArrayListUnmanaged(f64){};
    ...
    pub fn withKeyValue(self: *F64F64ListMultimap, key: f64, value: f64) *F64F64ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const F64F64ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const F64F64ListMultimap, other: *const F64F64ListMultimap) bool {
...
test "F64F64ListMultimap: put and get" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1.0);
    m.put(1.0, 2.0);
    m.put(2.0, 3.0);
...
test "F64F64ListMultimap: get returns values" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1.0);
    m.put(1.0, 2.0);
...
test "F64F64ListMultimap: get empty key" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F64ListMultimap: removeAll" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1.0);
    m.put(1.0, 2.0);
...
test "F64F64ListMultimap: containsKey and containsKeyValue" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1.0);
    m.put(1.0, 2.0);
...
test "F64F64ListMultimap: select and reject" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1.0);
    m.put(1.0, 2.0);
...
test "F64F64ListMultimap: anySatisfy noneSatisfy count" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1.0);
    m.put(2.0, 2.0);
...
test "F64F64ListMultimap: uniqueKeys and valuesToSlice" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1.0);
    m.put(1.0, 2.0);
...
test "F64F64ListMultimap: fluent withKeyValue" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F64ListMultimap: eql" {
    var m1 = F64F64ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1.0, 1.0);
    m1.put(1.0, 2.0);
    var m2 = F64F64ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1.0, 1.0);
    m2.put(1.0, 2.0);
...
test "F64F64ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = F64F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64F64ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64F64ListMultimap.init(failing.allocator());
    defer m.deinit();
