...
pub const CharI32SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) CharI32SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharI32SetMultimap) void {
    ...
    pub fn put(self: *CharI32SetMultimap, key: u21, value: i32) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharI32SetMultimap, key: u21) []const i32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharI32SetMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharI32SetMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharI32SetMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharI32SetMultimap, key: u21, value: i32) bool {
    ...
    pub fn keysCount(self: *const CharI32SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharI32SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharI32SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharI32SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharI32SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharI32SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI32SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI32SetMultimap, f: *const fn (u21, i32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharI32SetMultimap, predicate: *const fn (u21, i32) bool) CharI32SetMultimap {
        var result = CharI32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharI32SetMultimap, predicate: *const fn (u21, i32) bool) CharI32SetMultimap {
        var result = CharI32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharI32SetMultimap, predicate: *const fn (u21, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharI32SetMultimap, predicate: *const fn (u21, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharI32SetMultimap, predicate: *const fn (u21, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharI32SetMultimap, predicate: *const fn (u21, i32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharI32SetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharI32SetMultimap, allocator: Allocator) []i32 {
    ...
    pub fn withKeyValue(self: *CharI32SetMultimap, key: u21, value: i32) *CharI32SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharI32SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharI32SetMultimap, other: *const CharI32SetMultimap) bool {
...
test "CharI32SetMultimap: put and get" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
    m.put('b', 3);
...
test "CharI32SetMultimap: get returns values" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32SetMultimap: get empty key" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharI32SetMultimap: removeAll" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32SetMultimap: containsKey and containsKeyValue" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32SetMultimap: select and reject" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32SetMultimap: anySatisfy noneSatisfy count" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI32SetMultimap: uniqueKeys and valuesToSlice" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32SetMultimap: fluent withKeyValue" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI32SetMultimap: eql" {
    var m1 = CharI32SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1);
    m1.put('a', 2);
    var m2 = CharI32SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1);
    m2.put('a', 2);
...
test "CharI32SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharI32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI32SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI32SetMultimap.init(failing.allocator());
    defer m.deinit();
