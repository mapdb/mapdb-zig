...
pub const BoolI16ListMultimap = struct {
    ...
    pub fn init(allocator: Allocator) BoolI16ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(bool, std.ArrayListUnmanaged(i16)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *BoolI16ListMultimap) void {
    ...
    pub fn put(self: *BoolI16ListMultimap, key: bool, value: i16) void {
        const map_key = key;
    ...
    pub fn get(self: *const BoolI16ListMultimap, key: bool) []const i16 {
        const map_key = key;
    ...
    pub fn getCount(self: *const BoolI16ListMultimap, key: bool) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *BoolI16ListMultimap, key: bool) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const BoolI16ListMultimap, key: bool) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const BoolI16ListMultimap, key: bool, value: i16) bool {
    ...
    pub fn keysCount(self: *const BoolI16ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const BoolI16ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const BoolI16ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const BoolI16ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *BoolI16ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *BoolI16ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolI16ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolI16ListMultimap, f: *const fn (bool, i16) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const BoolI16ListMultimap, predicate: *const fn (bool, i16) bool) BoolI16ListMultimap {
        var result = BoolI16ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const BoolI16ListMultimap, predicate: *const fn (bool, i16) bool) BoolI16ListMultimap {
        var result = BoolI16ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const BoolI16ListMultimap, predicate: *const fn (bool, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const BoolI16ListMultimap, predicate: *const fn (bool, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const BoolI16ListMultimap, predicate: *const fn (bool, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const BoolI16ListMultimap, predicate: *const fn (bool, i16) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const BoolI16ListMultimap, allocator: Allocator) []bool {
    ...
    pub fn valuesToSlice(self: *const BoolI16ListMultimap, allocator: Allocator) []i16 {
    ...
    pub fn withKeyValue(self: *BoolI16ListMultimap, key: bool, value: i16) *BoolI16ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const BoolI16ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const BoolI16ListMultimap, other: *const BoolI16ListMultimap) bool {
...
test "BoolI16ListMultimap: put and get" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
    m.put(false, 3);
...
test "BoolI16ListMultimap: get returns values" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI16ListMultimap: get empty key" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16ListMultimap: removeAll" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI16ListMultimap: containsKey and containsKeyValue" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI16ListMultimap: clear" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(false, 2);
...
test "BoolI16ListMultimap: select" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(false, 2);
...
test "BoolI16ListMultimap: uniqueKeys and valuesToSlice" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI16ListMultimap: fluent withKeyValue" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16ListMultimap: eql" {
    var m1 = BoolI16ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(true, 1);
    m1.put(true, 2);
    var m2 = BoolI16ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(true, 1);
    m2.put(true, 2);
...
test "BoolI16ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = BoolI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolI16ListMultimap.init(failing.allocator());
    defer m.deinit();
