...
/// List multimap from `bool` keys to `bool` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const BoolBoolListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(bool, std.ArrayListUnmanaged(bool)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) BoolBoolListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(bool, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *BoolBoolListMultimap) void {
    ...
    pub fn put(self: *BoolBoolListMultimap, key: bool, value: bool) void {
        const map_key = key;
    ...
    pub fn get(self: *const BoolBoolListMultimap, key: bool) []const bool {
        const map_key = key;
    ...
    pub fn getCount(self: *const BoolBoolListMultimap, key: bool) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *BoolBoolListMultimap, key: bool) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const BoolBoolListMultimap, key: bool) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const BoolBoolListMultimap, key: bool, value: bool) bool {
    ...
    pub fn keysCount(self: *const BoolBoolListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const BoolBoolListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const BoolBoolListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const BoolBoolListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *BoolBoolListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *BoolBoolListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolBoolListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolBoolListMultimap, f: *const fn (bool, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const BoolBoolListMultimap, predicate: *const fn (bool, bool) bool) BoolBoolListMultimap {
        var result = BoolBoolListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const BoolBoolListMultimap, predicate: *const fn (bool, bool) bool) BoolBoolListMultimap {
        var result = BoolBoolListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const BoolBoolListMultimap, predicate: *const fn (bool, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const BoolBoolListMultimap, predicate: *const fn (bool, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const BoolBoolListMultimap, predicate: *const fn (bool, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const BoolBoolListMultimap, predicate: *const fn (bool, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const BoolBoolListMultimap, allocator: Allocator) []bool {
    ...
    pub fn valuesToSlice(self: *const BoolBoolListMultimap, allocator: Allocator) []bool {
        var result = std.ArrayListUnmanaged(bool){};
    ...
    pub fn withKeyValue(self: *BoolBoolListMultimap, key: bool, value: bool) *BoolBoolListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const BoolBoolListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const BoolBoolListMultimap, other: *const BoolBoolListMultimap) bool {
...
test "BoolBoolListMultimap: put and get" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
    m.put(false, true);
...
test "BoolBoolListMultimap: get returns values" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
...
test "BoolBoolListMultimap: get empty key" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolBoolListMultimap: removeAll" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
...
test "BoolBoolListMultimap: containsKey and containsKeyValue" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
    ...
    try std.testing.expect(m.containsKeyValue(true, true));
    try std.testing.expect(m.containsKeyValue(true, false));
    // bool has only 2 distinct values; vv[2]=true equals vv[0]
}

test "BoolBoolListMultimap: clear" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(false, false);
...
test "BoolBoolListMultimap: select" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(false, false);
...
test "BoolBoolListMultimap: uniqueKeys and valuesToSlice" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
...
test "BoolBoolListMultimap: fluent withKeyValue" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.withKeyValue(true, true).withKeyValue(true, false).withKeyValue(false, true);
    try std.testing.expectEqual(@as(usize, 3), m.size());
    try std.testing.expectEqual(@as(usize, 2), m.keysCount());
}

test "BoolBoolListMultimap: eql" {
    var m1 = BoolBoolListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(true, true);
    m1.put(true, false);
    var m2 = BoolBoolListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(true, true);
    m2.put(true, false);
...
test "BoolBoolListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = BoolBoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolBoolListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolBoolListMultimap.init(failing.allocator());
    defer m.deinit();
