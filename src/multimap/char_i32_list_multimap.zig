...
/// List multimap from `u21` keys to `i32` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const CharI32ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i32)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharI32ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharI32ListMultimap) void {
    ...
    pub fn put(self: *CharI32ListMultimap, key: u21, value: i32) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharI32ListMultimap, key: u21) []const i32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharI32ListMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharI32ListMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharI32ListMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharI32ListMultimap, key: u21, value: i32) bool {
    ...
    pub fn keysCount(self: *const CharI32ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharI32ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharI32ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharI32ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharI32ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharI32ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI32ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI32ListMultimap, f: *const fn (u21, i32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharI32ListMultimap, predicate: *const fn (u21, i32) bool) CharI32ListMultimap {
        var result = CharI32ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharI32ListMultimap, predicate: *const fn (u21, i32) bool) CharI32ListMultimap {
        var result = CharI32ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharI32ListMultimap, predicate: *const fn (u21, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharI32ListMultimap, predicate: *const fn (u21, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharI32ListMultimap, predicate: *const fn (u21, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharI32ListMultimap, predicate: *const fn (u21, i32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharI32ListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharI32ListMultimap, allocator: Allocator) []i32 {
    ...
    pub fn withKeyValue(self: *CharI32ListMultimap, key: u21, value: i32) *CharI32ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharI32ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharI32ListMultimap, other: *const CharI32ListMultimap) bool {
...
test "CharI32ListMultimap: put and get" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
    m.put('b', 3);
...
test "CharI32ListMultimap: get returns values" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32ListMultimap: get empty key" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharI32ListMultimap: removeAll" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32ListMultimap: containsKey and containsKeyValue" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32ListMultimap: clear" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI32ListMultimap: select and reject" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32ListMultimap: anySatisfy noneSatisfy count" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI32ListMultimap: uniqueKeys and valuesToSlice" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI32ListMultimap: fluent withKeyValue" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI32ListMultimap: eql" {
    var m1 = CharI32ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1);
    m1.put('a', 2);
    var m2 = CharI32ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1);
    m2.put('a', 2);
...
test "CharI32ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI32ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI32ListMultimap.init(failing.allocator());
    defer m.deinit();
