...
pub const CharBoolSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) CharBoolSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharBoolSetMultimap) void {
    ...
    pub fn put(self: *CharBoolSetMultimap, key: u21, value: bool) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharBoolSetMultimap, key: u21) []const bool {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharBoolSetMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharBoolSetMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharBoolSetMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharBoolSetMultimap, key: u21, value: bool) bool {
    ...
    pub fn keysCount(self: *const CharBoolSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharBoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharBoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharBoolSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharBoolSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharBoolSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharBoolSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharBoolSetMultimap, f: *const fn (u21, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharBoolSetMultimap, predicate: *const fn (u21, bool) bool) CharBoolSetMultimap {
        var result = CharBoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharBoolSetMultimap, predicate: *const fn (u21, bool) bool) CharBoolSetMultimap {
        var result = CharBoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharBoolSetMultimap, predicate: *const fn (u21, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharBoolSetMultimap, predicate: *const fn (u21, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharBoolSetMultimap, predicate: *const fn (u21, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharBoolSetMultimap, predicate: *const fn (u21, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharBoolSetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharBoolSetMultimap, allocator: Allocator) []bool {
    ...
    pub fn withKeyValue(self: *CharBoolSetMultimap, key: u21, value: bool) *CharBoolSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharBoolSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharBoolSetMultimap, other: *const CharBoolSetMultimap) bool {
...
test "CharBoolSetMultimap: put and get" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', true);
    m.put('a', false);
    m.put('b', true);
...
test "CharBoolSetMultimap: get returns values" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', true);
    m.put('a', false);
...
test "CharBoolSetMultimap: get empty key" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharBoolSetMultimap: removeAll" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', true);
    m.put('a', false);
...
test "CharBoolSetMultimap: containsKey and containsKeyValue" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', true);
    m.put('a', false);
...
test "CharBoolSetMultimap: clear" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', true);
    m.put('b', false);
...
test "CharBoolSetMultimap: select" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', true);
    m.put('b', false);
...
test "CharBoolSetMultimap: uniqueKeys and valuesToSlice" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', true);
    m.put('a', false);
...
test "CharBoolSetMultimap: fluent withKeyValue" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharBoolSetMultimap: eql" {
    var m1 = CharBoolSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', true);
    m1.put('a', false);
    var m2 = CharBoolSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', true);
    m2.put('a', false);
...
test "CharBoolSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharBoolSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharBoolSetMultimap.init(failing.allocator());
    defer m.deinit();
