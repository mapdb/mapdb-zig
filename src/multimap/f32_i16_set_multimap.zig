...
/// Set multimap from `f32` keys to `i16` values.
///
...
pub const F32I16SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) F32I16SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u32, std.ArrayListUnmanaged(i16)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *F32I16SetMultimap) void {
    ...
    pub fn put(self: *F32I16SetMultimap, key: f32, value: i16) void {
    ...
    pub fn get(self: *const F32I16SetMultimap, key: f32) []const i16 {
        const map_key = @as(u32, @bitCast(key));
    ...
    pub fn getCount(self: *const F32I16SetMultimap, key: f32) usize {
        const map_key = @as(u32, @bitCast(key));
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *F32I16SetMultimap, key: f32) usize {
        const map_key = @as(u32, @bitCast(key));
    ...
    pub fn containsKey(self: *const F32I16SetMultimap, key: f32) bool {
        const map_key = @as(u32, @bitCast(key));
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const F32I16SetMultimap, key: f32, value: i16) bool {
    ...
    pub fn keysCount(self: *const F32I16SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const F32I16SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const F32I16SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const F32I16SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *F32I16SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *F32I16SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F32I16SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32I16SetMultimap, f: *const fn (f32, i16) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const F32I16SetMultimap, predicate: *const fn (f32, i16) bool) F32I16SetMultimap {
        var result = F32I16SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const F32I16SetMultimap, predicate: *const fn (f32, i16) bool) F32I16SetMultimap {
        var result = F32I16SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
    ...
    pub fn anySatisfy(self: *const F32I16SetMultimap, predicate: *const fn (f32, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const F32I16SetMultimap, predicate: *const fn (f32, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const F32I16SetMultimap, predicate: *const fn (f32, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const F32I16SetMultimap, predicate: *const fn (f32, i16) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
    ...
    pub fn uniqueKeys(self: *const F32I16SetMultimap, allocator: Allocator) []f32 {
    ...
    pub fn valuesToSlice(self: *const F32I16SetMultimap, allocator: Allocator) []i16 {
    ...
    pub fn withKeyValue(self: *F32I16SetMultimap, key: f32, value: i16) *F32I16SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const F32I16SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const F32I16SetMultimap, other: *const F32I16SetMultimap) bool {
...
test "F32I16SetMultimap: put and get" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
    m.put(2.0, 3);
...
test "F32I16SetMultimap: get returns values" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F32I16SetMultimap: get empty key" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I16SetMultimap: removeAll" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F32I16SetMultimap: containsKey and containsKeyValue" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F32I16SetMultimap: select and reject" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F32I16SetMultimap: anySatisfy noneSatisfy count" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(2.0, 2);
...
test "F32I16SetMultimap: uniqueKeys and valuesToSlice" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F32I16SetMultimap: fluent withKeyValue" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I16SetMultimap: eql" {
    var m1 = F32I16SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1.0, 1);
    m1.put(1.0, 2);
    var m2 = F32I16SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1.0, 1);
    m2.put(1.0, 2);
...
test "F32I16SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = F32I16SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32I16SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F32I16SetMultimap.init(failing.allocator());
    defer m.deinit();
