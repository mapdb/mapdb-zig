...
/// List multimap from `i16` keys to `i64` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I16I64ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i16, std.ArrayListUnmanaged(i64)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I16I64ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i16, std.ArrayListUnmanaged(i64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I16I64ListMultimap) void {
    ...
    pub fn put(self: *I16I64ListMultimap, key: i16, value: i64) void {
        const map_key = key;
    ...
    pub fn get(self: *const I16I64ListMultimap, key: i16) []const i64 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I16I64ListMultimap, key: i16) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I16I64ListMultimap, key: i16) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I16I64ListMultimap, key: i16) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I16I64ListMultimap, key: i16, value: i64) bool {
    ...
    pub fn keysCount(self: *const I16I64ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I16I64ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I16I64ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I16I64ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I16I64ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I16I64ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16I64ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16I64ListMultimap, f: *const fn (i16, i64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I16I64ListMultimap, predicate: *const fn (i16, i64) bool) I16I64ListMultimap {
        var result = I16I64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I16I64ListMultimap, predicate: *const fn (i16, i64) bool) I16I64ListMultimap {
        var result = I16I64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I16I64ListMultimap, predicate: *const fn (i16, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I16I64ListMultimap, predicate: *const fn (i16, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I16I64ListMultimap, predicate: *const fn (i16, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I16I64ListMultimap, predicate: *const fn (i16, i64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I16I64ListMultimap, allocator: Allocator) []i16 {
    ...
    pub fn valuesToSlice(self: *const I16I64ListMultimap, allocator: Allocator) []i64 {
    ...
    pub fn withKeyValue(self: *I16I64ListMultimap, key: i16, value: i64) *I16I64ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I16I64ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I16I64ListMultimap, other: *const I16I64ListMultimap) bool {
...
test "I16I64ListMultimap: put and get" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
    m.put(2, 3);
...
test "I16I64ListMultimap: get returns values" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I64ListMultimap: get empty key" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I16I64ListMultimap: removeAll" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I64ListMultimap: containsKey and containsKeyValue" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I64ListMultimap: select and reject" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I64ListMultimap: anySatisfy noneSatisfy count" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(2, 2);
...
test "I16I64ListMultimap: uniqueKeys and valuesToSlice" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I64ListMultimap: fluent withKeyValue" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16I64ListMultimap: eql" {
    var m1 = I16I64ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1);
    m1.put(1, 2);
    var m2 = I16I64ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1);
    m2.put(1, 2);
...
test "I16I64ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I16I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16I64ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16I64ListMultimap.init(failing.allocator());
    defer m.deinit();
