...
pub const F64I64ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(u64, std.ArrayListUnmanaged(i64)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64I64ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u64, std.ArrayListUnmanaged(i64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *F64I64ListMultimap) void {
    ...
    pub fn put(self: *F64I64ListMultimap, key: f64, value: i64) void {
    ...
    pub fn get(self: *const F64I64ListMultimap, key: f64) []const i64 {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn getCount(self: *const F64I64ListMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *F64I64ListMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn containsKey(self: *const F64I64ListMultimap, key: f64) bool {
        const map_key = @as(u64, @bitCast(key));
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const F64I64ListMultimap, key: f64, value: i64) bool {
    ...
    pub fn keysCount(self: *const F64I64ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const F64I64ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const F64I64ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const F64I64ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *F64I64ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *F64I64ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64I64ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64I64ListMultimap, f: *const fn (f64, i64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const F64I64ListMultimap, predicate: *const fn (f64, i64) bool) F64I64ListMultimap {
        var result = F64I64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const F64I64ListMultimap, predicate: *const fn (f64, i64) bool) F64I64ListMultimap {
        var result = F64I64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn anySatisfy(self: *const F64I64ListMultimap, predicate: *const fn (f64, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const F64I64ListMultimap, predicate: *const fn (f64, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const F64I64ListMultimap, predicate: *const fn (f64, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const F64I64ListMultimap, predicate: *const fn (f64, i64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn uniqueKeys(self: *const F64I64ListMultimap, allocator: Allocator) []f64 {
    ...
    pub fn valuesToSlice(self: *const F64I64ListMultimap, allocator: Allocator) []i64 {
    ...
    pub fn withKeyValue(self: *F64I64ListMultimap, key: f64, value: i64) *F64I64ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const F64I64ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const F64I64ListMultimap, other: *const F64I64ListMultimap) bool {
...
test "F64I64ListMultimap: put and get" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
    m.put(2.0, 3);
...
test "F64I64ListMultimap: get returns values" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F64I64ListMultimap: get empty key" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64ListMultimap: removeAll" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F64I64ListMultimap: containsKey and containsKeyValue" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F64I64ListMultimap: select and reject" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F64I64ListMultimap: anySatisfy noneSatisfy count" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(2.0, 2);
...
test "F64I64ListMultimap: uniqueKeys and valuesToSlice" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 1);
    m.put(1.0, 2);
...
test "F64I64ListMultimap: fluent withKeyValue" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64ListMultimap: eql" {
    var m1 = F64I64ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1.0, 1);
    m1.put(1.0, 2);
    var m2 = F64I64ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1.0, 1);
    m2.put(1.0, 2);
...
test "F64I64ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = F64I64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I64ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64I64ListMultimap.init(failing.allocator());
    defer m.deinit();
