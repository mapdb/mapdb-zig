...
pub const CharF32SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) CharF32SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(f32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharF32SetMultimap) void {
    ...
    pub fn put(self: *CharF32SetMultimap, key: u21, value: f32) void {
        const map_key = key;
        ...
        for (gop.value_ptr.items) |existing| {
            if (@as(u32, @bitCast(existing)) == @as(u32, @bitCast(value))) return;
        }
    ...
    pub fn get(self: *const CharF32SetMultimap, key: u21) []const f32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharF32SetMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharF32SetMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharF32SetMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharF32SetMultimap, key: u21, value: f32) bool {
    ...
    pub fn keysCount(self: *const CharF32SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharF32SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharF32SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharF32SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharF32SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharF32SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharF32SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharF32SetMultimap, f: *const fn (u21, f32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharF32SetMultimap, predicate: *const fn (u21, f32) bool) CharF32SetMultimap {
        var result = CharF32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharF32SetMultimap, predicate: *const fn (u21, f32) bool) CharF32SetMultimap {
        var result = CharF32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharF32SetMultimap, predicate: *const fn (u21, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharF32SetMultimap, predicate: *const fn (u21, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharF32SetMultimap, predicate: *const fn (u21, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharF32SetMultimap, predicate: *const fn (u21, f32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharF32SetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharF32SetMultimap, allocator: Allocator) []f32 {
    ...
    pub fn withKeyValue(self: *CharF32SetMultimap, key: u21, value: f32) *CharF32SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharF32SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharF32SetMultimap, other: *const CharF32SetMultimap) bool {
...
test "CharF32SetMultimap: put and get" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
    m.put('b', 3.0);
...
test "CharF32SetMultimap: get returns values" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF32SetMultimap: get empty key" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharF32SetMultimap: removeAll" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF32SetMultimap: containsKey and containsKeyValue" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF32SetMultimap: select and reject" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF32SetMultimap: anySatisfy noneSatisfy count" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('b', 2.0);
...
test "CharF32SetMultimap: uniqueKeys and valuesToSlice" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF32SetMultimap: fluent withKeyValue" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF32SetMultimap: eql" {
    var m1 = CharF32SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1.0);
    m1.put('a', 2.0);
    var m2 = CharF32SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1.0);
    m2.put('a', 2.0);
...
test "CharF32SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF32SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharF32SetMultimap.init(failing.allocator());
    defer m.deinit();
