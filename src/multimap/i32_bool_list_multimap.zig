...
/// List multimap from `i32` keys to `bool` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I32BoolListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(bool)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I32BoolListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I32BoolListMultimap) void {
    ...
    pub fn put(self: *I32BoolListMultimap, key: i32, value: bool) void {
        const map_key = key;
    ...
    pub fn get(self: *const I32BoolListMultimap, key: i32) []const bool {
        const map_key = key;
    ...
    pub fn getCount(self: *const I32BoolListMultimap, key: i32) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I32BoolListMultimap, key: i32) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I32BoolListMultimap, key: i32) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I32BoolListMultimap, key: i32, value: bool) bool {
    ...
    pub fn keysCount(self: *const I32BoolListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I32BoolListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I32BoolListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I32BoolListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I32BoolListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I32BoolListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32BoolListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32BoolListMultimap, f: *const fn (i32, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I32BoolListMultimap, predicate: *const fn (i32, bool) bool) I32BoolListMultimap {
        var result = I32BoolListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I32BoolListMultimap, predicate: *const fn (i32, bool) bool) I32BoolListMultimap {
        var result = I32BoolListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I32BoolListMultimap, predicate: *const fn (i32, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I32BoolListMultimap, predicate: *const fn (i32, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I32BoolListMultimap, predicate: *const fn (i32, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I32BoolListMultimap, predicate: *const fn (i32, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I32BoolListMultimap, allocator: Allocator) []i32 {
    ...
    pub fn valuesToSlice(self: *const I32BoolListMultimap, allocator: Allocator) []bool {
    ...
    pub fn withKeyValue(self: *I32BoolListMultimap, key: i32, value: bool) *I32BoolListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I32BoolListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I32BoolListMultimap, other: *const I32BoolListMultimap) bool {
...
test "I32BoolListMultimap: put and get" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
    m.put(2, true);
...
test "I32BoolListMultimap: get returns values" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I32BoolListMultimap: get empty key" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I32BoolListMultimap: removeAll" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I32BoolListMultimap: containsKey and containsKeyValue" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I32BoolListMultimap: clear" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(2, false);
...
test "I32BoolListMultimap: select" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(2, false);
...
test "I32BoolListMultimap: uniqueKeys and valuesToSlice" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I32BoolListMultimap: fluent withKeyValue" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32BoolListMultimap: eql" {
    var m1 = I32BoolListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, true);
    m1.put(1, false);
    var m2 = I32BoolListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, true);
    m2.put(1, false);
...
test "I32BoolListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I32BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32BoolListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32BoolListMultimap.init(failing.allocator());
    defer m.deinit();
