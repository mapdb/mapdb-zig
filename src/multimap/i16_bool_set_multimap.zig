...
pub const I16BoolSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I16BoolSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i16, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I16BoolSetMultimap) void {
    ...
    pub fn put(self: *I16BoolSetMultimap, key: i16, value: bool) void {
        const map_key = key;
    ...
    pub fn get(self: *const I16BoolSetMultimap, key: i16) []const bool {
        const map_key = key;
    ...
    pub fn getCount(self: *const I16BoolSetMultimap, key: i16) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I16BoolSetMultimap, key: i16) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I16BoolSetMultimap, key: i16) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I16BoolSetMultimap, key: i16, value: bool) bool {
    ...
    pub fn keysCount(self: *const I16BoolSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I16BoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I16BoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I16BoolSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I16BoolSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I16BoolSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16BoolSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16BoolSetMultimap, f: *const fn (i16, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I16BoolSetMultimap, predicate: *const fn (i16, bool) bool) I16BoolSetMultimap {
        var result = I16BoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I16BoolSetMultimap, predicate: *const fn (i16, bool) bool) I16BoolSetMultimap {
        var result = I16BoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I16BoolSetMultimap, predicate: *const fn (i16, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I16BoolSetMultimap, predicate: *const fn (i16, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I16BoolSetMultimap, predicate: *const fn (i16, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I16BoolSetMultimap, predicate: *const fn (i16, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I16BoolSetMultimap, allocator: Allocator) []i16 {
    ...
    pub fn valuesToSlice(self: *const I16BoolSetMultimap, allocator: Allocator) []bool {
    ...
    pub fn withKeyValue(self: *I16BoolSetMultimap, key: i16, value: bool) *I16BoolSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I16BoolSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I16BoolSetMultimap, other: *const I16BoolSetMultimap) bool {
...
test "I16BoolSetMultimap: put and get" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
    m.put(2, true);
...
test "I16BoolSetMultimap: get returns values" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I16BoolSetMultimap: get empty key" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I16BoolSetMultimap: removeAll" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I16BoolSetMultimap: containsKey and containsKeyValue" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I16BoolSetMultimap: select" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(2, false);
...
test "I16BoolSetMultimap: uniqueKeys and valuesToSlice" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I16BoolSetMultimap: fluent withKeyValue" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16BoolSetMultimap: eql" {
    var m1 = I16BoolSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, true);
    m1.put(1, false);
    var m2 = I16BoolSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, true);
    m2.put(1, false);
...
test "I16BoolSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I16BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16BoolSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16BoolSetMultimap.init(failing.allocator());
    defer m.deinit();
