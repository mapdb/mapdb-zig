...
pub const I32I64SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I32I64SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(i64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I32I64SetMultimap) void {
    ...
    pub fn put(self: *I32I64SetMultimap, key: i32, value: i64) void {
        const map_key = key;
    ...
    pub fn get(self: *const I32I64SetMultimap, key: i32) []const i64 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I32I64SetMultimap, key: i32) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I32I64SetMultimap, key: i32) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I32I64SetMultimap, key: i32) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I32I64SetMultimap, key: i32, value: i64) bool {
    ...
    pub fn keysCount(self: *const I32I64SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I32I64SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I32I64SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I32I64SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I32I64SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I32I64SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32I64SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32I64SetMultimap, f: *const fn (i32, i64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I32I64SetMultimap, predicate: *const fn (i32, i64) bool) I32I64SetMultimap {
        var result = I32I64SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I32I64SetMultimap, predicate: *const fn (i32, i64) bool) I32I64SetMultimap {
        var result = I32I64SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I32I64SetMultimap, predicate: *const fn (i32, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I32I64SetMultimap, predicate: *const fn (i32, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I32I64SetMultimap, predicate: *const fn (i32, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I32I64SetMultimap, predicate: *const fn (i32, i64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I32I64SetMultimap, allocator: Allocator) []i32 {
    ...
    pub fn valuesToSlice(self: *const I32I64SetMultimap, allocator: Allocator) []i64 {
    ...
    pub fn withKeyValue(self: *I32I64SetMultimap, key: i32, value: i64) *I32I64SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I32I64SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I32I64SetMultimap, other: *const I32I64SetMultimap) bool {
...
test "I32I64SetMultimap: put and get" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
    m.put(2, 3);
...
test "I32I64SetMultimap: get returns values" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I64SetMultimap: get empty key" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I32I64SetMultimap: removeAll" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I64SetMultimap: containsKey and containsKeyValue" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I64SetMultimap: select and reject" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I64SetMultimap: anySatisfy noneSatisfy count" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(2, 2);
...
test "I32I64SetMultimap: uniqueKeys and valuesToSlice" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I64SetMultimap: fluent withKeyValue" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I64SetMultimap: eql" {
    var m1 = I32I64SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1);
    m1.put(1, 2);
    var m2 = I32I64SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1);
    m2.put(1, 2);
...
test "I32I64SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I32I64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I64SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32I64SetMultimap.init(failing.allocator());
    defer m.deinit();
