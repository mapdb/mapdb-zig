...
/// Set multimap from `f64` keys to `bool` values.
///
...
pub const F64BoolSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) F64BoolSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u64, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *F64BoolSetMultimap) void {
    ...
    pub fn put(self: *F64BoolSetMultimap, key: f64, value: bool) void {
    ...
    pub fn get(self: *const F64BoolSetMultimap, key: f64) []const bool {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn getCount(self: *const F64BoolSetMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *F64BoolSetMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn containsKey(self: *const F64BoolSetMultimap, key: f64) bool {
        const map_key = @as(u64, @bitCast(key));
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const F64BoolSetMultimap, key: f64, value: bool) bool {
    ...
    pub fn keysCount(self: *const F64BoolSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const F64BoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const F64BoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const F64BoolSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *F64BoolSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *F64BoolSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64BoolSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64BoolSetMultimap, f: *const fn (f64, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const F64BoolSetMultimap, predicate: *const fn (f64, bool) bool) F64BoolSetMultimap {
        var result = F64BoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const F64BoolSetMultimap, predicate: *const fn (f64, bool) bool) F64BoolSetMultimap {
        var result = F64BoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn anySatisfy(self: *const F64BoolSetMultimap, predicate: *const fn (f64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const F64BoolSetMultimap, predicate: *const fn (f64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const F64BoolSetMultimap, predicate: *const fn (f64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const F64BoolSetMultimap, predicate: *const fn (f64, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn uniqueKeys(self: *const F64BoolSetMultimap, allocator: Allocator) []f64 {
    ...
    pub fn valuesToSlice(self: *const F64BoolSetMultimap, allocator: Allocator) []bool {
    ...
    pub fn withKeyValue(self: *F64BoolSetMultimap, key: f64, value: bool) *F64BoolSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const F64BoolSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const F64BoolSetMultimap, other: *const F64BoolSetMultimap) bool {
...
test "F64BoolSetMultimap: put and get" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
    m.put(2.0, true);
...
test "F64BoolSetMultimap: get returns values" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
...
test "F64BoolSetMultimap: get empty key" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolSetMultimap: removeAll" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
...
test "F64BoolSetMultimap: containsKey and containsKeyValue" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
...
test "F64BoolSetMultimap: select" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(2.0, false);
...
test "F64BoolSetMultimap: uniqueKeys and valuesToSlice" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
...
test "F64BoolSetMultimap: fluent withKeyValue" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolSetMultimap: eql" {
    var m1 = F64BoolSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1.0, true);
    m1.put(1.0, false);
    var m2 = F64BoolSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1.0, true);
    m2.put(1.0, false);
...
test "F64BoolSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = F64BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64BoolSetMultimap.init(failing.allocator());
    defer m.deinit();
