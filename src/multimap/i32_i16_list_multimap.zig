...
/// List multimap from `i32` keys to `i16` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I32I16ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(i16)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I32I16ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(i16)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I32I16ListMultimap) void {
    ...
    pub fn put(self: *I32I16ListMultimap, key: i32, value: i16) void {
        const map_key = key;
    ...
    pub fn get(self: *const I32I16ListMultimap, key: i32) []const i16 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I32I16ListMultimap, key: i32) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I32I16ListMultimap, key: i32) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I32I16ListMultimap, key: i32) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I32I16ListMultimap, key: i32, value: i16) bool {
    ...
    pub fn keysCount(self: *const I32I16ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I32I16ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I32I16ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I32I16ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I32I16ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I32I16ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32I16ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32I16ListMultimap, f: *const fn (i32, i16) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I32I16ListMultimap, predicate: *const fn (i32, i16) bool) I32I16ListMultimap {
        var result = I32I16ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I32I16ListMultimap, predicate: *const fn (i32, i16) bool) I32I16ListMultimap {
        var result = I32I16ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I32I16ListMultimap, predicate: *const fn (i32, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I32I16ListMultimap, predicate: *const fn (i32, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I32I16ListMultimap, predicate: *const fn (i32, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I32I16ListMultimap, predicate: *const fn (i32, i16) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I32I16ListMultimap, allocator: Allocator) []i32 {
    ...
    pub fn valuesToSlice(self: *const I32I16ListMultimap, allocator: Allocator) []i16 {
    ...
    pub fn withKeyValue(self: *I32I16ListMultimap, key: i32, value: i16) *I32I16ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I32I16ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I32I16ListMultimap, other: *const I32I16ListMultimap) bool {
...
test "I32I16ListMultimap: put and get" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
    m.put(2, 3);
...
test "I32I16ListMultimap: get returns values" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I16ListMultimap: get empty key" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I32I16ListMultimap: removeAll" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I16ListMultimap: containsKey and containsKeyValue" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I16ListMultimap: select and reject" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I16ListMultimap: anySatisfy noneSatisfy count" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(2, 2);
...
test "I32I16ListMultimap: uniqueKeys and valuesToSlice" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I16ListMultimap: fluent withKeyValue" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I16ListMultimap: eql" {
    var m1 = I32I16ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1);
    m1.put(1, 2);
    var m2 = I32I16ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1);
    m2.put(1, 2);
...
test "I32I16ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I32I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I16ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32I16ListMultimap.init(failing.allocator());
    defer m.deinit();
