...
pub const I32I32SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I32I32SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(i32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I32I32SetMultimap) void {
    ...
    pub fn put(self: *I32I32SetMultimap, key: i32, value: i32) void {
        const map_key = key;
    ...
    pub fn get(self: *const I32I32SetMultimap, key: i32) []const i32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I32I32SetMultimap, key: i32) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I32I32SetMultimap, key: i32) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I32I32SetMultimap, key: i32) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I32I32SetMultimap, key: i32, value: i32) bool {
    ...
    pub fn keysCount(self: *const I32I32SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I32I32SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I32I32SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I32I32SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I32I32SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I32I32SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32I32SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32I32SetMultimap, f: *const fn (i32, i32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I32I32SetMultimap, predicate: *const fn (i32, i32) bool) I32I32SetMultimap {
        var result = I32I32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I32I32SetMultimap, predicate: *const fn (i32, i32) bool) I32I32SetMultimap {
        var result = I32I32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I32I32SetMultimap, predicate: *const fn (i32, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I32I32SetMultimap, predicate: *const fn (i32, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I32I32SetMultimap, predicate: *const fn (i32, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I32I32SetMultimap, predicate: *const fn (i32, i32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I32I32SetMultimap, allocator: Allocator) []i32 {
    ...
    pub fn valuesToSlice(self: *const I32I32SetMultimap, allocator: Allocator) []i32 {
        var result = std.ArrayListUnmanaged(i32){};
    ...
    pub fn withKeyValue(self: *I32I32SetMultimap, key: i32, value: i32) *I32I32SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I32I32SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I32I32SetMultimap, other: *const I32I32SetMultimap) bool {
...
test "I32I32SetMultimap: put and get" {
...
test "I32I32SetMultimap: get returns values" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32SetMultimap: get empty key" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I32I32SetMultimap: removeAll" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32SetMultimap: containsKey and containsKeyValue" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32SetMultimap: select and reject" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32SetMultimap: anySatisfy noneSatisfy count" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(2, 2);
...
test "I32I32SetMultimap: uniqueKeys and valuesToSlice" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32SetMultimap: fluent withKeyValue" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I32SetMultimap: eql" {
    var m1 = I32I32SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1);
    m1.put(1, 2);
    var m2 = I32I32SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1);
    m2.put(1, 2);
...
test "I32I32SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I32I32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I32SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32I32SetMultimap.init(failing.allocator());
    defer m.deinit();
