...
pub const I64CharSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I64CharSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i64, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I64CharSetMultimap) void {
    ...
    pub fn put(self: *I64CharSetMultimap, key: i64, value: u21) void {
        const map_key = key;
    ...
    pub fn get(self: *const I64CharSetMultimap, key: i64) []const u21 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I64CharSetMultimap, key: i64) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I64CharSetMultimap, key: i64) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I64CharSetMultimap, key: i64) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I64CharSetMultimap, key: i64, value: u21) bool {
    ...
    pub fn keysCount(self: *const I64CharSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I64CharSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I64CharSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I64CharSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I64CharSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I64CharSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I64CharSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I64CharSetMultimap, f: *const fn (i64, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I64CharSetMultimap, predicate: *const fn (i64, u21) bool) I64CharSetMultimap {
        var result = I64CharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I64CharSetMultimap, predicate: *const fn (i64, u21) bool) I64CharSetMultimap {
        var result = I64CharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I64CharSetMultimap, predicate: *const fn (i64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I64CharSetMultimap, predicate: *const fn (i64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I64CharSetMultimap, predicate: *const fn (i64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I64CharSetMultimap, predicate: *const fn (i64, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I64CharSetMultimap, allocator: Allocator) []i64 {
    ...
    pub fn valuesToSlice(self: *const I64CharSetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *I64CharSetMultimap, key: i64, value: u21) *I64CharSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I64CharSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I64CharSetMultimap, other: *const I64CharSetMultimap) bool {
...
test "I64CharSetMultimap: put and get" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
    m.put(2, 'c');
...
test "I64CharSetMultimap: get returns values" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharSetMultimap: get empty key" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I64CharSetMultimap: removeAll" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharSetMultimap: containsKey and containsKeyValue" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharSetMultimap: select and reject" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharSetMultimap: anySatisfy noneSatisfy count" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I64CharSetMultimap: uniqueKeys and valuesToSlice" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharSetMultimap: fluent withKeyValue" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharSetMultimap: eql" {
    var m1 = I64CharSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 'a');
    m1.put(1, 'b');
    var m2 = I64CharSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 'a');
    m2.put(1, 'b');
...
test "I64CharSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I64CharSetMultimap.init(failing.allocator());
    defer m.deinit();
