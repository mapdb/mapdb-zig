...
pub const BoolF32SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) BoolF32SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(bool, std.ArrayListUnmanaged(f32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *BoolF32SetMultimap) void {
    ...
    pub fn put(self: *BoolF32SetMultimap, key: bool, value: f32) void {
        const map_key = key;
    ...
    pub fn get(self: *const BoolF32SetMultimap, key: bool) []const f32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const BoolF32SetMultimap, key: bool) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *BoolF32SetMultimap, key: bool) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const BoolF32SetMultimap, key: bool) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const BoolF32SetMultimap, key: bool, value: f32) bool {
    ...
    pub fn keysCount(self: *const BoolF32SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const BoolF32SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const BoolF32SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const BoolF32SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *BoolF32SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *BoolF32SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolF32SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolF32SetMultimap, f: *const fn (bool, f32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const BoolF32SetMultimap, predicate: *const fn (bool, f32) bool) BoolF32SetMultimap {
        var result = BoolF32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const BoolF32SetMultimap, predicate: *const fn (bool, f32) bool) BoolF32SetMultimap {
        var result = BoolF32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const BoolF32SetMultimap, predicate: *const fn (bool, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const BoolF32SetMultimap, predicate: *const fn (bool, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const BoolF32SetMultimap, predicate: *const fn (bool, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const BoolF32SetMultimap, predicate: *const fn (bool, f32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const BoolF32SetMultimap, allocator: Allocator) []bool {
    ...
    pub fn valuesToSlice(self: *const BoolF32SetMultimap, allocator: Allocator) []f32 {
    ...
    pub fn withKeyValue(self: *BoolF32SetMultimap, key: bool, value: f32) *BoolF32SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const BoolF32SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const BoolF32SetMultimap, other: *const BoolF32SetMultimap) bool {
...
test "BoolF32SetMultimap: put and get" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1.0);
    m.put(true, 2.0);
    m.put(false, 3.0);
...
test "BoolF32SetMultimap: get returns values" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1.0);
    m.put(true, 2.0);
...
test "BoolF32SetMultimap: get empty key" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32SetMultimap: removeAll" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1.0);
    m.put(true, 2.0);
...
test "BoolF32SetMultimap: containsKey and containsKeyValue" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1.0);
    m.put(true, 2.0);
...
test "BoolF32SetMultimap: select" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1.0);
    m.put(false, 2.0);
...
test "BoolF32SetMultimap: uniqueKeys and valuesToSlice" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1.0);
    m.put(true, 2.0);
...
test "BoolF32SetMultimap: fluent withKeyValue" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32SetMultimap: eql" {
    var m1 = BoolF32SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(true, 1.0);
    m1.put(true, 2.0);
    var m2 = BoolF32SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(true, 1.0);
    m2.put(true, 2.0);
...
test "BoolF32SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = BoolF32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolF32SetMultimap.init(failing.allocator());
    defer m.deinit();
