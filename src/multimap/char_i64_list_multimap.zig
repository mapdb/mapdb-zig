...
/// List multimap from `u21` keys to `i64` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const CharI64ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i64)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharI64ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharI64ListMultimap) void {
    ...
    pub fn put(self: *CharI64ListMultimap, key: u21, value: i64) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharI64ListMultimap, key: u21) []const i64 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharI64ListMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharI64ListMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharI64ListMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharI64ListMultimap, key: u21, value: i64) bool {
    ...
    pub fn keysCount(self: *const CharI64ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharI64ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharI64ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharI64ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharI64ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharI64ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI64ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI64ListMultimap, f: *const fn (u21, i64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharI64ListMultimap, predicate: *const fn (u21, i64) bool) CharI64ListMultimap {
        var result = CharI64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharI64ListMultimap, predicate: *const fn (u21, i64) bool) CharI64ListMultimap {
        var result = CharI64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharI64ListMultimap, predicate: *const fn (u21, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharI64ListMultimap, predicate: *const fn (u21, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharI64ListMultimap, predicate: *const fn (u21, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharI64ListMultimap, predicate: *const fn (u21, i64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharI64ListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharI64ListMultimap, allocator: Allocator) []i64 {
    ...
    pub fn withKeyValue(self: *CharI64ListMultimap, key: u21, value: i64) *CharI64ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharI64ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharI64ListMultimap, other: *const CharI64ListMultimap) bool {
...
test "CharI64ListMultimap: put and get" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
    m.put('b', 3);
...
test "CharI64ListMultimap: get returns values" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64ListMultimap: get empty key" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharI64ListMultimap: removeAll" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64ListMultimap: containsKey and containsKeyValue" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64ListMultimap: clear" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI64ListMultimap: select and reject" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64ListMultimap: anySatisfy noneSatisfy count" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI64ListMultimap: uniqueKeys and valuesToSlice" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64ListMultimap: fluent withKeyValue" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI64ListMultimap: eql" {
    var m1 = CharI64ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1);
    m1.put('a', 2);
    var m2 = CharI64ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1);
    m2.put('a', 2);
...
test "CharI64ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharI64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI64ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI64ListMultimap.init(failing.allocator());
    defer m.deinit();
