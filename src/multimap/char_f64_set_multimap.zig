...
pub const CharF64SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) CharF64SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(f64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharF64SetMultimap) void {
    ...
    pub fn put(self: *CharF64SetMultimap, key: u21, value: f64) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharF64SetMultimap, key: u21) []const f64 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharF64SetMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharF64SetMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharF64SetMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharF64SetMultimap, key: u21, value: f64) bool {
    ...
    pub fn keysCount(self: *const CharF64SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharF64SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharF64SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharF64SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharF64SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharF64SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharF64SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharF64SetMultimap, f: *const fn (u21, f64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharF64SetMultimap, predicate: *const fn (u21, f64) bool) CharF64SetMultimap {
        var result = CharF64SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharF64SetMultimap, predicate: *const fn (u21, f64) bool) CharF64SetMultimap {
        var result = CharF64SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharF64SetMultimap, predicate: *const fn (u21, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharF64SetMultimap, predicate: *const fn (u21, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharF64SetMultimap, predicate: *const fn (u21, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharF64SetMultimap, predicate: *const fn (u21, f64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharF64SetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharF64SetMultimap, allocator: Allocator) []f64 {
    ...
    pub fn withKeyValue(self: *CharF64SetMultimap, key: u21, value: f64) *CharF64SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharF64SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharF64SetMultimap, other: *const CharF64SetMultimap) bool {
...
test "CharF64SetMultimap: put and get" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
    m.put('b', 3.0);
...
test "CharF64SetMultimap: get returns values" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF64SetMultimap: get empty key" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharF64SetMultimap: removeAll" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF64SetMultimap: containsKey and containsKeyValue" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF64SetMultimap: select and reject" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF64SetMultimap: anySatisfy noneSatisfy count" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('b', 2.0);
...
test "CharF64SetMultimap: uniqueKeys and valuesToSlice" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1.0);
    m.put('a', 2.0);
...
test "CharF64SetMultimap: fluent withKeyValue" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF64SetMultimap: eql" {
    var m1 = CharF64SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1.0);
    m1.put('a', 2.0);
    var m2 = CharF64SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1.0);
    m2.put('a', 2.0);
...
test "CharF64SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharF64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF64SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharF64SetMultimap.init(failing.allocator());
    defer m.deinit();
