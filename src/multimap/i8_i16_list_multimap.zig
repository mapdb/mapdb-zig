...
/// List multimap from `i8` keys to `i16` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I8I16ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i8, std.ArrayListUnmanaged(i16)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I8I16ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i8, std.ArrayListUnmanaged(i16)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I8I16ListMultimap) void {
    ...
    pub fn put(self: *I8I16ListMultimap, key: i8, value: i16) void {
        const map_key = key;
    ...
    pub fn get(self: *const I8I16ListMultimap, key: i8) []const i16 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I8I16ListMultimap, key: i8) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I8I16ListMultimap, key: i8) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I8I16ListMultimap, key: i8) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I8I16ListMultimap, key: i8, value: i16) bool {
    ...
    pub fn keysCount(self: *const I8I16ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I8I16ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I8I16ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I8I16ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I8I16ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I8I16ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8I16ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8I16ListMultimap, f: *const fn (i8, i16) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I8I16ListMultimap, predicate: *const fn (i8, i16) bool) I8I16ListMultimap {
        var result = I8I16ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I8I16ListMultimap, predicate: *const fn (i8, i16) bool) I8I16ListMultimap {
        var result = I8I16ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I8I16ListMultimap, predicate: *const fn (i8, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I8I16ListMultimap, predicate: *const fn (i8, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I8I16ListMultimap, predicate: *const fn (i8, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I8I16ListMultimap, predicate: *const fn (i8, i16) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I8I16ListMultimap, allocator: Allocator) []i8 {
    ...
    pub fn valuesToSlice(self: *const I8I16ListMultimap, allocator: Allocator) []i16 {
    ...
    pub fn withKeyValue(self: *I8I16ListMultimap, key: i8, value: i16) *I8I16ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I8I16ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I8I16ListMultimap, other: *const I8I16ListMultimap) bool {
...
test "I8I16ListMultimap: put and get" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
    m.put(2, 3);
...
test "I8I16ListMultimap: get returns values" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I8I16ListMultimap: get empty key" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I8I16ListMultimap: removeAll" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I8I16ListMultimap: containsKey and containsKeyValue" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I8I16ListMultimap: select and reject" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I8I16ListMultimap: anySatisfy noneSatisfy count" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(2, 2);
...
test "I8I16ListMultimap: uniqueKeys and valuesToSlice" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I8I16ListMultimap: fluent withKeyValue" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8I16ListMultimap: eql" {
    var m1 = I8I16ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1);
    m1.put(1, 2);
    var m2 = I8I16ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1);
    m2.put(1, 2);
...
test "I8I16ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I8I16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8I16ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I8I16ListMultimap.init(failing.allocator());
    defer m.deinit();
