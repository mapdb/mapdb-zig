...
/// List multimap from `i32` keys to `i32` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I32I32ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(i32)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I32I32ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(i32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I32I32ListMultimap) void {
    ...
    pub fn put(self: *I32I32ListMultimap, key: i32, value: i32) void {
        const map_key = key;
    ...
    pub fn get(self: *const I32I32ListMultimap, key: i32) []const i32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I32I32ListMultimap, key: i32) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I32I32ListMultimap, key: i32) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I32I32ListMultimap, key: i32) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I32I32ListMultimap, key: i32, value: i32) bool {
    ...
    pub fn keysCount(self: *const I32I32ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I32I32ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I32I32ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I32I32ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I32I32ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I32I32ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32I32ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32I32ListMultimap, f: *const fn (i32, i32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I32I32ListMultimap, predicate: *const fn (i32, i32) bool) I32I32ListMultimap {
        var result = I32I32ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I32I32ListMultimap, predicate: *const fn (i32, i32) bool) I32I32ListMultimap {
        var result = I32I32ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I32I32ListMultimap, predicate: *const fn (i32, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I32I32ListMultimap, predicate: *const fn (i32, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I32I32ListMultimap, predicate: *const fn (i32, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I32I32ListMultimap, predicate: *const fn (i32, i32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I32I32ListMultimap, allocator: Allocator) []i32 {
    ...
    pub fn valuesToSlice(self: *const I32I32ListMultimap, allocator: Allocator) []i32 {
        var result = std.ArrayListUnmanaged(i32){};
    ...
    pub fn withKeyValue(self: *I32I32ListMultimap, key: i32, value: i32) *I32I32ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I32I32ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I32I32ListMultimap, other: *const I32I32ListMultimap) bool {
...
test "I32I32ListMultimap: put and get" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
    m.put(2, 3);
...
test "I32I32ListMultimap: get returns values" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32ListMultimap: get empty key" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I32I32ListMultimap: removeAll" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32ListMultimap: containsKey and containsKeyValue" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32ListMultimap: select and reject" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32ListMultimap: anySatisfy noneSatisfy count" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(2, 2);
...
test "I32I32ListMultimap: uniqueKeys and valuesToSlice" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I32I32ListMultimap: fluent withKeyValue" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I32ListMultimap: eql" {
    var m1 = I32I32ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1);
    m1.put(1, 2);
    var m2 = I32I32ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1);
    m2.put(1, 2);
...
test "I32I32ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32I32ListMultimap: get returns []const i32 borrowed view" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 10);
    m.put(1, 20);
...
test "I32I32ListMultimap: uniqueKeys returns independent caller-owned copy" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 10);
    m.put(2, 20);
...
test "I32I32ListMultimap: valuesToSlice returns independent caller-owned copy" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 10);
    m.put(1, 20);
...
test "I32I32ListMultimap: forEach passes values by copy" {
    var m = I32I32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 10);
    m.put(1, 20);
    comptime {
        const F = @TypeOf(I32I32ListMultimap.forEach);
...
test "I32I32ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32I32ListMultimap.init(failing.allocator());
    defer m.deinit();
