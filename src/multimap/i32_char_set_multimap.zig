...
pub const I32CharSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I32CharSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I32CharSetMultimap) void {
    ...
    pub fn put(self: *I32CharSetMultimap, key: i32, value: u21) void {
        const map_key = key;
    ...
    pub fn get(self: *const I32CharSetMultimap, key: i32) []const u21 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I32CharSetMultimap, key: i32) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I32CharSetMultimap, key: i32) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I32CharSetMultimap, key: i32) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I32CharSetMultimap, key: i32, value: u21) bool {
    ...
    pub fn keysCount(self: *const I32CharSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I32CharSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I32CharSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I32CharSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I32CharSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I32CharSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32CharSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32CharSetMultimap, f: *const fn (i32, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I32CharSetMultimap, predicate: *const fn (i32, u21) bool) I32CharSetMultimap {
        var result = I32CharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I32CharSetMultimap, predicate: *const fn (i32, u21) bool) I32CharSetMultimap {
        var result = I32CharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I32CharSetMultimap, predicate: *const fn (i32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I32CharSetMultimap, predicate: *const fn (i32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I32CharSetMultimap, predicate: *const fn (i32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I32CharSetMultimap, predicate: *const fn (i32, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I32CharSetMultimap, allocator: Allocator) []i32 {
    ...
    pub fn valuesToSlice(self: *const I32CharSetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *I32CharSetMultimap, key: i32, value: u21) *I32CharSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I32CharSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I32CharSetMultimap, other: *const I32CharSetMultimap) bool {
...
test "I32CharSetMultimap: put and get" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
    m.put(2, 'c');
...
test "I32CharSetMultimap: get returns values" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharSetMultimap: get empty key" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I32CharSetMultimap: removeAll" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharSetMultimap: containsKey and containsKeyValue" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharSetMultimap: select and reject" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharSetMultimap: anySatisfy noneSatisfy count" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I32CharSetMultimap: uniqueKeys and valuesToSlice" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharSetMultimap: fluent withKeyValue" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32CharSetMultimap: eql" {
    var m1 = I32CharSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 'a');
    m1.put(1, 'b');
    var m2 = I32CharSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 'a');
    m2.put(1, 'b');
...
test "I32CharSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32CharSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32CharSetMultimap.init(failing.allocator());
    defer m.deinit();
