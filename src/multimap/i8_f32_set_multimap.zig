...
/// Set multimap from `i8` keys to `f32` values.
///
/// Each key maps to a set of unique values (duplicates on put are ignored).
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I8F32SetMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i8, std.ArrayListUnmanaged(f32)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I8F32SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i8, std.ArrayListUnmanaged(f32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I8F32SetMultimap) void {
    ...
    pub fn put(self: *I8F32SetMultimap, key: i8, value: f32) void {
        const map_key = key;
    ...
    pub fn get(self: *const I8F32SetMultimap, key: i8) []const f32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I8F32SetMultimap, key: i8) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I8F32SetMultimap, key: i8) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I8F32SetMultimap, key: i8) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I8F32SetMultimap, key: i8, value: f32) bool {
    ...
    pub fn keysCount(self: *const I8F32SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I8F32SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I8F32SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I8F32SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I8F32SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I8F32SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8F32SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8F32SetMultimap, f: *const fn (i8, f32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I8F32SetMultimap, predicate: *const fn (i8, f32) bool) I8F32SetMultimap {
        var result = I8F32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I8F32SetMultimap, predicate: *const fn (i8, f32) bool) I8F32SetMultimap {
        var result = I8F32SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I8F32SetMultimap, predicate: *const fn (i8, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I8F32SetMultimap, predicate: *const fn (i8, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I8F32SetMultimap, predicate: *const fn (i8, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I8F32SetMultimap, predicate: *const fn (i8, f32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I8F32SetMultimap, allocator: Allocator) []i8 {
    ...
    pub fn valuesToSlice(self: *const I8F32SetMultimap, allocator: Allocator) []f32 {
    ...
    pub fn withKeyValue(self: *I8F32SetMultimap, key: i8, value: f32) *I8F32SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I8F32SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I8F32SetMultimap, other: *const I8F32SetMultimap) bool {
...
test "I8F32SetMultimap: put and get" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
    m.put(2, 3.0);
...
test "I8F32SetMultimap: get returns values" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I8F32SetMultimap: get empty key" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I8F32SetMultimap: removeAll" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I8F32SetMultimap: containsKey and containsKeyValue" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I8F32SetMultimap: select and reject" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I8F32SetMultimap: anySatisfy noneSatisfy count" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(2, 2.0);
...
test "I8F32SetMultimap: uniqueKeys and valuesToSlice" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I8F32SetMultimap: fluent withKeyValue" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8F32SetMultimap: eql" {
    var m1 = I8F32SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1.0);
    m1.put(1, 2.0);
    var m2 = I8F32SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1.0);
    m2.put(1, 2.0);
...
test "I8F32SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I8F32SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8F32SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I8F32SetMultimap.init(failing.allocator());
    defer m.deinit();
