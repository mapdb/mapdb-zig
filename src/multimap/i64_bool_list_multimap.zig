...
pub const I64BoolListMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I64BoolListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i64, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I64BoolListMultimap) void {
    ...
    pub fn put(self: *I64BoolListMultimap, key: i64, value: bool) void {
        const map_key = key;
    ...
    pub fn get(self: *const I64BoolListMultimap, key: i64) []const bool {
        const map_key = key;
    ...
    pub fn getCount(self: *const I64BoolListMultimap, key: i64) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I64BoolListMultimap, key: i64) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I64BoolListMultimap, key: i64) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I64BoolListMultimap, key: i64, value: bool) bool {
    ...
    pub fn keysCount(self: *const I64BoolListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I64BoolListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I64BoolListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I64BoolListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I64BoolListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I64BoolListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I64BoolListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I64BoolListMultimap, f: *const fn (i64, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I64BoolListMultimap, predicate: *const fn (i64, bool) bool) I64BoolListMultimap {
        var result = I64BoolListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I64BoolListMultimap, predicate: *const fn (i64, bool) bool) I64BoolListMultimap {
        var result = I64BoolListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I64BoolListMultimap, predicate: *const fn (i64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I64BoolListMultimap, predicate: *const fn (i64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I64BoolListMultimap, predicate: *const fn (i64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I64BoolListMultimap, predicate: *const fn (i64, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I64BoolListMultimap, allocator: Allocator) []i64 {
    ...
    pub fn valuesToSlice(self: *const I64BoolListMultimap, allocator: Allocator) []bool {
    ...
    pub fn withKeyValue(self: *I64BoolListMultimap, key: i64, value: bool) *I64BoolListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I64BoolListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I64BoolListMultimap, other: *const I64BoolListMultimap) bool {
...
test "I64BoolListMultimap: put and get" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
    m.put(2, true);
...
test "I64BoolListMultimap: get returns values" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I64BoolListMultimap: get empty key" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I64BoolListMultimap: removeAll" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I64BoolListMultimap: containsKey and containsKeyValue" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I64BoolListMultimap: clear" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(2, false);
...
test "I64BoolListMultimap: select" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(2, false);
...
test "I64BoolListMultimap: uniqueKeys and valuesToSlice" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I64BoolListMultimap: fluent withKeyValue" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64BoolListMultimap: eql" {
    var m1 = I64BoolListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, true);
    m1.put(1, false);
    var m2 = I64BoolListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, true);
    m2.put(1, false);
...
test "I64BoolListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64BoolListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I64BoolListMultimap.init(failing.allocator());
    defer m.deinit();
