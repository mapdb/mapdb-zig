...
pub const BoolI32ListMultimap = struct {
    ...
    pub fn init(allocator: Allocator) BoolI32ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(bool, std.ArrayListUnmanaged(i32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *BoolI32ListMultimap) void {
    ...
    pub fn put(self: *BoolI32ListMultimap, key: bool, value: i32) void {
        const map_key = key;
    ...
    pub fn get(self: *const BoolI32ListMultimap, key: bool) []const i32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const BoolI32ListMultimap, key: bool) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *BoolI32ListMultimap, key: bool) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const BoolI32ListMultimap, key: bool) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const BoolI32ListMultimap, key: bool, value: i32) bool {
    ...
    pub fn keysCount(self: *const BoolI32ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const BoolI32ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const BoolI32ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const BoolI32ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *BoolI32ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *BoolI32ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolI32ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolI32ListMultimap, f: *const fn (bool, i32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const BoolI32ListMultimap, predicate: *const fn (bool, i32) bool) BoolI32ListMultimap {
        var result = BoolI32ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const BoolI32ListMultimap, predicate: *const fn (bool, i32) bool) BoolI32ListMultimap {
        var result = BoolI32ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const BoolI32ListMultimap, predicate: *const fn (bool, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const BoolI32ListMultimap, predicate: *const fn (bool, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const BoolI32ListMultimap, predicate: *const fn (bool, i32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const BoolI32ListMultimap, predicate: *const fn (bool, i32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const BoolI32ListMultimap, allocator: Allocator) []bool {
    ...
    pub fn valuesToSlice(self: *const BoolI32ListMultimap, allocator: Allocator) []i32 {
    ...
    pub fn withKeyValue(self: *BoolI32ListMultimap, key: bool, value: i32) *BoolI32ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const BoolI32ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const BoolI32ListMultimap, other: *const BoolI32ListMultimap) bool {
...
test "BoolI32ListMultimap: put and get" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
    m.put(false, 3);
...
test "BoolI32ListMultimap: get returns values" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI32ListMultimap: get empty key" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI32ListMultimap: removeAll" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI32ListMultimap: containsKey and containsKeyValue" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI32ListMultimap: clear" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(false, 2);
...
test "BoolI32ListMultimap: select" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(false, 2);
...
test "BoolI32ListMultimap: uniqueKeys and valuesToSlice" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI32ListMultimap: fluent withKeyValue" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI32ListMultimap: eql" {
    var m1 = BoolI32ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(true, 1);
    m1.put(true, 2);
    var m2 = BoolI32ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(true, 1);
    m2.put(true, 2);
...
test "BoolI32ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = BoolI32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI32ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolI32ListMultimap.init(failing.allocator());
    defer m.deinit();
