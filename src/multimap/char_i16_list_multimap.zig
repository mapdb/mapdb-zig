...
pub const CharI16ListMultimap = struct {
    ...
    pub fn init(allocator: Allocator) CharI16ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i16)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharI16ListMultimap) void {
    ...
    pub fn put(self: *CharI16ListMultimap, key: u21, value: i16) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharI16ListMultimap, key: u21) []const i16 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharI16ListMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharI16ListMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharI16ListMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharI16ListMultimap, key: u21, value: i16) bool {
    ...
    pub fn keysCount(self: *const CharI16ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharI16ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharI16ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharI16ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharI16ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharI16ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI16ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI16ListMultimap, f: *const fn (u21, i16) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharI16ListMultimap, predicate: *const fn (u21, i16) bool) CharI16ListMultimap {
        var result = CharI16ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharI16ListMultimap, predicate: *const fn (u21, i16) bool) CharI16ListMultimap {
        var result = CharI16ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharI16ListMultimap, predicate: *const fn (u21, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharI16ListMultimap, predicate: *const fn (u21, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharI16ListMultimap, predicate: *const fn (u21, i16) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharI16ListMultimap, predicate: *const fn (u21, i16) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharI16ListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharI16ListMultimap, allocator: Allocator) []i16 {
    ...
    pub fn withKeyValue(self: *CharI16ListMultimap, key: u21, value: i16) *CharI16ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharI16ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharI16ListMultimap, other: *const CharI16ListMultimap) bool {
...
test "CharI16ListMultimap: put and get" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
    m.put('b', 3);
...
test "CharI16ListMultimap: get returns values" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI16ListMultimap: get empty key" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharI16ListMultimap: removeAll" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI16ListMultimap: containsKey and containsKeyValue" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI16ListMultimap: clear" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI16ListMultimap: select and reject" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI16ListMultimap: anySatisfy noneSatisfy count" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI16ListMultimap: uniqueKeys and valuesToSlice" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI16ListMultimap: fluent withKeyValue" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI16ListMultimap: eql" {
    var m1 = CharI16ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1);
    m1.put('a', 2);
    var m2 = CharI16ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1);
    m2.put('a', 2);
...
test "CharI16ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharI16ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI16ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI16ListMultimap.init(failing.allocator());
    defer m.deinit();
