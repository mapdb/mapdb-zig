...
pub const CharI8SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) CharI8SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i8)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharI8SetMultimap) void {
    ...
    pub fn put(self: *CharI8SetMultimap, key: u21, value: i8) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharI8SetMultimap, key: u21) []const i8 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharI8SetMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharI8SetMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharI8SetMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharI8SetMultimap, key: u21, value: i8) bool {
    ...
    pub fn keysCount(self: *const CharI8SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharI8SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharI8SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharI8SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharI8SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharI8SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI8SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI8SetMultimap, f: *const fn (u21, i8) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharI8SetMultimap, predicate: *const fn (u21, i8) bool) CharI8SetMultimap {
        var result = CharI8SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharI8SetMultimap, predicate: *const fn (u21, i8) bool) CharI8SetMultimap {
        var result = CharI8SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharI8SetMultimap, predicate: *const fn (u21, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharI8SetMultimap, predicate: *const fn (u21, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharI8SetMultimap, predicate: *const fn (u21, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharI8SetMultimap, predicate: *const fn (u21, i8) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharI8SetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharI8SetMultimap, allocator: Allocator) []i8 {
    ...
    pub fn withKeyValue(self: *CharI8SetMultimap, key: u21, value: i8) *CharI8SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharI8SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharI8SetMultimap, other: *const CharI8SetMultimap) bool {
...
test "CharI8SetMultimap: put and get" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
    m.put('b', 3);
...
test "CharI8SetMultimap: get returns values" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8SetMultimap: get empty key" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharI8SetMultimap: removeAll" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8SetMultimap: containsKey and containsKeyValue" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8SetMultimap: select and reject" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8SetMultimap: anySatisfy noneSatisfy count" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI8SetMultimap: uniqueKeys and valuesToSlice" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8SetMultimap: fluent withKeyValue" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8SetMultimap: eql" {
    var m1 = CharI8SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1);
    m1.put('a', 2);
    var m2 = CharI8SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1);
    m2.put('a', 2);
...
test "CharI8SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI8SetMultimap.init(failing.allocator());
    defer m.deinit();
