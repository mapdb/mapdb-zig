...
pub const I16F64ListMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I16F64ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i16, std.ArrayListUnmanaged(f64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I16F64ListMultimap) void {
    ...
    pub fn put(self: *I16F64ListMultimap, key: i16, value: f64) void {
        const map_key = key;
    ...
    pub fn get(self: *const I16F64ListMultimap, key: i16) []const f64 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I16F64ListMultimap, key: i16) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I16F64ListMultimap, key: i16) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I16F64ListMultimap, key: i16) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I16F64ListMultimap, key: i16, value: f64) bool {
    ...
    pub fn keysCount(self: *const I16F64ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I16F64ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I16F64ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I16F64ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I16F64ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I16F64ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16F64ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16F64ListMultimap, f: *const fn (i16, f64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I16F64ListMultimap, predicate: *const fn (i16, f64) bool) I16F64ListMultimap {
        var result = I16F64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I16F64ListMultimap, predicate: *const fn (i16, f64) bool) I16F64ListMultimap {
        var result = I16F64ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I16F64ListMultimap, predicate: *const fn (i16, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I16F64ListMultimap, predicate: *const fn (i16, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I16F64ListMultimap, predicate: *const fn (i16, f64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I16F64ListMultimap, predicate: *const fn (i16, f64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I16F64ListMultimap, allocator: Allocator) []i16 {
    ...
    pub fn valuesToSlice(self: *const I16F64ListMultimap, allocator: Allocator) []f64 {
    ...
    pub fn withKeyValue(self: *I16F64ListMultimap, key: i16, value: f64) *I16F64ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I16F64ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I16F64ListMultimap, other: *const I16F64ListMultimap) bool {
...
test "I16F64ListMultimap: put and get" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
    m.put(2, 3.0);
...
test "I16F64ListMultimap: get returns values" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F64ListMultimap: get empty key" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I16F64ListMultimap: removeAll" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F64ListMultimap: containsKey and containsKeyValue" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F64ListMultimap: select and reject" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F64ListMultimap: anySatisfy noneSatisfy count" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(2, 2.0);
...
test "I16F64ListMultimap: uniqueKeys and valuesToSlice" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F64ListMultimap: fluent withKeyValue" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F64ListMultimap: eql" {
    var m1 = I16F64ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1.0);
    m1.put(1, 2.0);
    var m2 = I16F64ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1.0);
    m2.put(1, 2.0);
...
test "I16F64ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I16F64ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F64ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16F64ListMultimap.init(failing.allocator());
    defer m.deinit();
