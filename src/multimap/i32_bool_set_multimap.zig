...
pub const I32BoolSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I32BoolSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I32BoolSetMultimap) void {
    ...
    pub fn put(self: *I32BoolSetMultimap, key: i32, value: bool) void {
        const map_key = key;
    ...
    pub fn get(self: *const I32BoolSetMultimap, key: i32) []const bool {
        const map_key = key;
    ...
    pub fn getCount(self: *const I32BoolSetMultimap, key: i32) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I32BoolSetMultimap, key: i32) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I32BoolSetMultimap, key: i32) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I32BoolSetMultimap, key: i32, value: bool) bool {
    ...
    pub fn keysCount(self: *const I32BoolSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I32BoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I32BoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I32BoolSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I32BoolSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I32BoolSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32BoolSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32BoolSetMultimap, f: *const fn (i32, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I32BoolSetMultimap, predicate: *const fn (i32, bool) bool) I32BoolSetMultimap {
        var result = I32BoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I32BoolSetMultimap, predicate: *const fn (i32, bool) bool) I32BoolSetMultimap {
        var result = I32BoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I32BoolSetMultimap, predicate: *const fn (i32, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I32BoolSetMultimap, predicate: *const fn (i32, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I32BoolSetMultimap, predicate: *const fn (i32, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I32BoolSetMultimap, predicate: *const fn (i32, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I32BoolSetMultimap, allocator: Allocator) []i32 {
    ...
    pub fn valuesToSlice(self: *const I32BoolSetMultimap, allocator: Allocator) []bool {
    ...
    pub fn withKeyValue(self: *I32BoolSetMultimap, key: i32, value: bool) *I32BoolSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I32BoolSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I32BoolSetMultimap, other: *const I32BoolSetMultimap) bool {
...
test "I32BoolSetMultimap: put and get" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
    m.put(2, true);
...
test "I32BoolSetMultimap: get returns values" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I32BoolSetMultimap: get empty key" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I32BoolSetMultimap: removeAll" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I32BoolSetMultimap: containsKey and containsKeyValue" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I32BoolSetMultimap: select" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(2, false);
...
test "I32BoolSetMultimap: uniqueKeys and valuesToSlice" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I32BoolSetMultimap: fluent withKeyValue" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32BoolSetMultimap: eql" {
    var m1 = I32BoolSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, true);
    m1.put(1, false);
    var m2 = I32BoolSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, true);
    m2.put(1, false);
...
test "I32BoolSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I32BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32BoolSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32BoolSetMultimap.init(failing.allocator());
    defer m.deinit();
