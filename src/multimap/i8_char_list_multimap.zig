...
pub const I8CharListMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I8CharListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i8, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I8CharListMultimap) void {
    ...
    pub fn put(self: *I8CharListMultimap, key: i8, value: u21) void {
        const map_key = key;
    ...
    pub fn get(self: *const I8CharListMultimap, key: i8) []const u21 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I8CharListMultimap, key: i8) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I8CharListMultimap, key: i8) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I8CharListMultimap, key: i8) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I8CharListMultimap, key: i8, value: u21) bool {
    ...
    pub fn keysCount(self: *const I8CharListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I8CharListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I8CharListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I8CharListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I8CharListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I8CharListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8CharListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8CharListMultimap, f: *const fn (i8, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I8CharListMultimap, predicate: *const fn (i8, u21) bool) I8CharListMultimap {
        var result = I8CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I8CharListMultimap, predicate: *const fn (i8, u21) bool) I8CharListMultimap {
        var result = I8CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I8CharListMultimap, predicate: *const fn (i8, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I8CharListMultimap, predicate: *const fn (i8, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I8CharListMultimap, predicate: *const fn (i8, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I8CharListMultimap, predicate: *const fn (i8, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I8CharListMultimap, allocator: Allocator) []i8 {
    ...
    pub fn valuesToSlice(self: *const I8CharListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *I8CharListMultimap, key: i8, value: u21) *I8CharListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I8CharListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I8CharListMultimap, other: *const I8CharListMultimap) bool {
...
test "I8CharListMultimap: put and get" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
    m.put(2, 'c');
...
test "I8CharListMultimap: get returns values" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I8CharListMultimap: get empty key" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I8CharListMultimap: removeAll" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I8CharListMultimap: containsKey and containsKeyValue" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I8CharListMultimap: select and reject" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I8CharListMultimap: anySatisfy noneSatisfy count" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I8CharListMultimap: uniqueKeys and valuesToSlice" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I8CharListMultimap: fluent withKeyValue" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8CharListMultimap: eql" {
    var m1 = I8CharListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 'a');
    m1.put(1, 'b');
    var m2 = I8CharListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 'a');
    m2.put(1, 'b');
...
test "I8CharListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I8CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8CharListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I8CharListMultimap.init(failing.allocator());
    defer m.deinit();
