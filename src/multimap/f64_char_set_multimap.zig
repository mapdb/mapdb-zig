...
/// Set multimap from `f64` keys to `u21` values.
///
...
pub const F64CharSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) F64CharSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u64, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *F64CharSetMultimap) void {
    ...
    pub fn put(self: *F64CharSetMultimap, key: f64, value: u21) void {
    ...
    pub fn get(self: *const F64CharSetMultimap, key: f64) []const u21 {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn getCount(self: *const F64CharSetMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *F64CharSetMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn containsKey(self: *const F64CharSetMultimap, key: f64) bool {
        const map_key = @as(u64, @bitCast(key));
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const F64CharSetMultimap, key: f64, value: u21) bool {
    ...
    pub fn keysCount(self: *const F64CharSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const F64CharSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const F64CharSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const F64CharSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *F64CharSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *F64CharSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64CharSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64CharSetMultimap, f: *const fn (f64, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const F64CharSetMultimap, predicate: *const fn (f64, u21) bool) F64CharSetMultimap {
        var result = F64CharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const F64CharSetMultimap, predicate: *const fn (f64, u21) bool) F64CharSetMultimap {
        var result = F64CharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn anySatisfy(self: *const F64CharSetMultimap, predicate: *const fn (f64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const F64CharSetMultimap, predicate: *const fn (f64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const F64CharSetMultimap, predicate: *const fn (f64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const F64CharSetMultimap, predicate: *const fn (f64, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn uniqueKeys(self: *const F64CharSetMultimap, allocator: Allocator) []f64 {
    ...
    pub fn valuesToSlice(self: *const F64CharSetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *F64CharSetMultimap, key: f64, value: u21) *F64CharSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const F64CharSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const F64CharSetMultimap, other: *const F64CharSetMultimap) bool {
...
test "F64CharSetMultimap: put and get" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
    m.put(2.0, 'c');
...
test "F64CharSetMultimap: get returns values" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F64CharSetMultimap: get empty key" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64CharSetMultimap: removeAll" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F64CharSetMultimap: containsKey and containsKeyValue" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F64CharSetMultimap: select and reject" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F64CharSetMultimap: anySatisfy noneSatisfy count" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(2.0, 'b');
...
test "F64CharSetMultimap: uniqueKeys and valuesToSlice" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F64CharSetMultimap: fluent withKeyValue" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64CharSetMultimap: eql" {
    var m1 = F64CharSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1.0, 'a');
    m1.put(1.0, 'b');
    var m2 = F64CharSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1.0, 'a');
    m2.put(1.0, 'b');
...
test "F64CharSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = F64CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64CharSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64CharSetMultimap.init(failing.allocator());
    defer m.deinit();
