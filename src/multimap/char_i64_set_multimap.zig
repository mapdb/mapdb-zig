...
pub const CharI64SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) CharI64SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharI64SetMultimap) void {
    ...
    pub fn put(self: *CharI64SetMultimap, key: u21, value: i64) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharI64SetMultimap, key: u21) []const i64 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharI64SetMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharI64SetMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharI64SetMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharI64SetMultimap, key: u21, value: i64) bool {
    ...
    pub fn keysCount(self: *const CharI64SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharI64SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharI64SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharI64SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharI64SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharI64SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI64SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI64SetMultimap, f: *const fn (u21, i64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharI64SetMultimap, predicate: *const fn (u21, i64) bool) CharI64SetMultimap {
        var result = CharI64SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharI64SetMultimap, predicate: *const fn (u21, i64) bool) CharI64SetMultimap {
        var result = CharI64SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharI64SetMultimap, predicate: *const fn (u21, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharI64SetMultimap, predicate: *const fn (u21, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharI64SetMultimap, predicate: *const fn (u21, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharI64SetMultimap, predicate: *const fn (u21, i64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharI64SetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharI64SetMultimap, allocator: Allocator) []i64 {
    ...
    pub fn withKeyValue(self: *CharI64SetMultimap, key: u21, value: i64) *CharI64SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharI64SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharI64SetMultimap, other: *const CharI64SetMultimap) bool {
...
test "CharI64SetMultimap: put and get" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
    m.put('b', 3);
...
test "CharI64SetMultimap: get returns values" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64SetMultimap: get empty key" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharI64SetMultimap: removeAll" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64SetMultimap: containsKey and containsKeyValue" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64SetMultimap: select and reject" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64SetMultimap: anySatisfy noneSatisfy count" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI64SetMultimap: uniqueKeys and valuesToSlice" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI64SetMultimap: fluent withKeyValue" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI64SetMultimap: eql" {
    var m1 = CharI64SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1);
    m1.put('a', 2);
    var m2 = CharI64SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1);
    m2.put('a', 2);
...
test "CharI64SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI64SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI64SetMultimap.init(failing.allocator());
    defer m.deinit();
