...
/// List multimap from `i32` keys to `u21` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I32CharListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(u21)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I32CharListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i32, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I32CharListMultimap) void {
    ...
    pub fn put(self: *I32CharListMultimap, key: i32, value: u21) void {
        const map_key = key;
    ...
    pub fn get(self: *const I32CharListMultimap, key: i32) []const u21 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I32CharListMultimap, key: i32) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I32CharListMultimap, key: i32) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I32CharListMultimap, key: i32) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I32CharListMultimap, key: i32, value: u21) bool {
    ...
    pub fn keysCount(self: *const I32CharListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I32CharListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I32CharListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I32CharListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I32CharListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I32CharListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I32CharListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I32CharListMultimap, f: *const fn (i32, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I32CharListMultimap, predicate: *const fn (i32, u21) bool) I32CharListMultimap {
        var result = I32CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I32CharListMultimap, predicate: *const fn (i32, u21) bool) I32CharListMultimap {
        var result = I32CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I32CharListMultimap, predicate: *const fn (i32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I32CharListMultimap, predicate: *const fn (i32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I32CharListMultimap, predicate: *const fn (i32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I32CharListMultimap, predicate: *const fn (i32, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I32CharListMultimap, allocator: Allocator) []i32 {
    ...
    pub fn valuesToSlice(self: *const I32CharListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *I32CharListMultimap, key: i32, value: u21) *I32CharListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I32CharListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I32CharListMultimap, other: *const I32CharListMultimap) bool {
...
test "I32CharListMultimap: put and get" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
    m.put(2, 'c');
...
test "I32CharListMultimap: get returns values" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharListMultimap: get empty key" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I32CharListMultimap: removeAll" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharListMultimap: containsKey and containsKeyValue" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharListMultimap: clear" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I32CharListMultimap: select and reject" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharListMultimap: anySatisfy noneSatisfy count" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I32CharListMultimap: uniqueKeys and valuesToSlice" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I32CharListMultimap: fluent withKeyValue" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32CharListMultimap: eql" {
    var m1 = I32CharListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 'a');
    m1.put(1, 'b');
    var m2 = I32CharListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 'a');
    m2.put(1, 'b');
...
test "I32CharListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32CharListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32CharListMultimap.init(failing.allocator());
    defer m.deinit();
