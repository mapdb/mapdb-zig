...
pub const I8BoolSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I8BoolSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i8, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I8BoolSetMultimap) void {
    ...
    pub fn put(self: *I8BoolSetMultimap, key: i8, value: bool) void {
        const map_key = key;
    ...
    pub fn get(self: *const I8BoolSetMultimap, key: i8) []const bool {
        const map_key = key;
    ...
    pub fn getCount(self: *const I8BoolSetMultimap, key: i8) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I8BoolSetMultimap, key: i8) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I8BoolSetMultimap, key: i8) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I8BoolSetMultimap, key: i8, value: bool) bool {
    ...
    pub fn keysCount(self: *const I8BoolSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I8BoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I8BoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I8BoolSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I8BoolSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I8BoolSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I8BoolSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I8BoolSetMultimap, f: *const fn (i8, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I8BoolSetMultimap, predicate: *const fn (i8, bool) bool) I8BoolSetMultimap {
        var result = I8BoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I8BoolSetMultimap, predicate: *const fn (i8, bool) bool) I8BoolSetMultimap {
        var result = I8BoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I8BoolSetMultimap, predicate: *const fn (i8, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I8BoolSetMultimap, predicate: *const fn (i8, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I8BoolSetMultimap, predicate: *const fn (i8, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I8BoolSetMultimap, predicate: *const fn (i8, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I8BoolSetMultimap, allocator: Allocator) []i8 {
    ...
    pub fn valuesToSlice(self: *const I8BoolSetMultimap, allocator: Allocator) []bool {
    ...
    pub fn withKeyValue(self: *I8BoolSetMultimap, key: i8, value: bool) *I8BoolSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I8BoolSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I8BoolSetMultimap, other: *const I8BoolSetMultimap) bool {
...
test "I8BoolSetMultimap: put and get" {
    var m = I8BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
    m.put(2, true);
...
test "I8BoolSetMultimap: get returns values" {
    var m = I8BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I8BoolSetMultimap: get empty key" {
    var m = I8BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I8BoolSetMultimap: removeAll" {
    var m = I8BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I8BoolSetMultimap: containsKey and containsKeyValue" {
    var m = I8BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I8BoolSetMultimap: uniqueKeys and valuesToSlice" {
    var m = I8BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, true);
    m.put(1, false);
...
test "I8BoolSetMultimap: fluent withKeyValue" {
    var m = I8BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolSetMultimap: eql" {
    var m1 = I8BoolSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, true);
    m1.put(1, false);
    var m2 = I8BoolSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, true);
    m2.put(1, false);
...
test "I8BoolSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I8BoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I8BoolSetMultimap.init(failing.allocator());
    defer m.deinit();
