...
/// List multimap from `i64` keys to `u21` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I64CharListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i64, std.ArrayListUnmanaged(u21)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I64CharListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i64, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I64CharListMultimap) void {
    ...
    pub fn put(self: *I64CharListMultimap, key: i64, value: u21) void {
        const map_key = key;
    ...
    pub fn get(self: *const I64CharListMultimap, key: i64) []const u21 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I64CharListMultimap, key: i64) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I64CharListMultimap, key: i64) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I64CharListMultimap, key: i64) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I64CharListMultimap, key: i64, value: u21) bool {
    ...
    pub fn keysCount(self: *const I64CharListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I64CharListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I64CharListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I64CharListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I64CharListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I64CharListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I64CharListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I64CharListMultimap, f: *const fn (i64, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I64CharListMultimap, predicate: *const fn (i64, u21) bool) I64CharListMultimap {
        var result = I64CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I64CharListMultimap, predicate: *const fn (i64, u21) bool) I64CharListMultimap {
        var result = I64CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I64CharListMultimap, predicate: *const fn (i64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I64CharListMultimap, predicate: *const fn (i64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I64CharListMultimap, predicate: *const fn (i64, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I64CharListMultimap, predicate: *const fn (i64, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I64CharListMultimap, allocator: Allocator) []i64 {
    ...
    pub fn valuesToSlice(self: *const I64CharListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *I64CharListMultimap, key: i64, value: u21) *I64CharListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I64CharListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I64CharListMultimap, other: *const I64CharListMultimap) bool {
...
test "I64CharListMultimap: put and get" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
    m.put(2, 'c');
...
test "I64CharListMultimap: get returns values" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharListMultimap: get empty key" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I64CharListMultimap: removeAll" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharListMultimap: containsKey and containsKeyValue" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharListMultimap: clear" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I64CharListMultimap: select and reject" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharListMultimap: anySatisfy noneSatisfy count" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I64CharListMultimap: uniqueKeys and valuesToSlice" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I64CharListMultimap: fluent withKeyValue" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharListMultimap: eql" {
    var m1 = I64CharListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 'a');
    m1.put(1, 'b');
    var m2 = I64CharListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 'a');
    m2.put(1, 'b');
...
test "I64CharListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I64CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64CharListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I64CharListMultimap.init(failing.allocator());
    defer m.deinit();
