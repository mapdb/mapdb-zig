...
pub const BoolI8SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) BoolI8SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(bool, std.ArrayListUnmanaged(i8)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *BoolI8SetMultimap) void {
    ...
    pub fn put(self: *BoolI8SetMultimap, key: bool, value: i8) void {
        const map_key = key;
    ...
    pub fn get(self: *const BoolI8SetMultimap, key: bool) []const i8 {
        const map_key = key;
    ...
    pub fn getCount(self: *const BoolI8SetMultimap, key: bool) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *BoolI8SetMultimap, key: bool) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const BoolI8SetMultimap, key: bool) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const BoolI8SetMultimap, key: bool, value: i8) bool {
    ...
    pub fn keysCount(self: *const BoolI8SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const BoolI8SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const BoolI8SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const BoolI8SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *BoolI8SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *BoolI8SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolI8SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolI8SetMultimap, f: *const fn (bool, i8) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const BoolI8SetMultimap, predicate: *const fn (bool, i8) bool) BoolI8SetMultimap {
        var result = BoolI8SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const BoolI8SetMultimap, predicate: *const fn (bool, i8) bool) BoolI8SetMultimap {
        var result = BoolI8SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const BoolI8SetMultimap, predicate: *const fn (bool, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const BoolI8SetMultimap, predicate: *const fn (bool, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const BoolI8SetMultimap, predicate: *const fn (bool, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const BoolI8SetMultimap, predicate: *const fn (bool, i8) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const BoolI8SetMultimap, allocator: Allocator) []bool {
    ...
    pub fn valuesToSlice(self: *const BoolI8SetMultimap, allocator: Allocator) []i8 {
    ...
    pub fn withKeyValue(self: *BoolI8SetMultimap, key: bool, value: i8) *BoolI8SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const BoolI8SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const BoolI8SetMultimap, other: *const BoolI8SetMultimap) bool {
...
test "BoolI8SetMultimap: put and get" {
    var m = BoolI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
    m.put(false, 3);
...
test "BoolI8SetMultimap: get returns values" {
    var m = BoolI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI8SetMultimap: get empty key" {
    var m = BoolI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8SetMultimap: removeAll" {
    var m = BoolI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI8SetMultimap: containsKey and containsKeyValue" {
    var m = BoolI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI8SetMultimap: uniqueKeys and valuesToSlice" {
    var m = BoolI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI8SetMultimap: fluent withKeyValue" {
    var m = BoolI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8SetMultimap: eql" {
    var m1 = BoolI8SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(true, 1);
    m1.put(true, 2);
    var m2 = BoolI8SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(true, 1);
    m2.put(true, 2);
...
test "BoolI8SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = BoolI8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolI8SetMultimap.init(failing.allocator());
    defer m.deinit();
