...
pub const CharCharSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) CharCharSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharCharSetMultimap) void {
    ...
    pub fn put(self: *CharCharSetMultimap, key: u21, value: u21) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharCharSetMultimap, key: u21) []const u21 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharCharSetMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharCharSetMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharCharSetMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharCharSetMultimap, key: u21, value: u21) bool {
    ...
    pub fn keysCount(self: *const CharCharSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharCharSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharCharSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharCharSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharCharSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharCharSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharCharSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharCharSetMultimap, f: *const fn (u21, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharCharSetMultimap, predicate: *const fn (u21, u21) bool) CharCharSetMultimap {
        var result = CharCharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharCharSetMultimap, predicate: *const fn (u21, u21) bool) CharCharSetMultimap {
        var result = CharCharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharCharSetMultimap, predicate: *const fn (u21, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharCharSetMultimap, predicate: *const fn (u21, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharCharSetMultimap, predicate: *const fn (u21, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharCharSetMultimap, predicate: *const fn (u21, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharCharSetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharCharSetMultimap, allocator: Allocator) []u21 {
        var result = std.ArrayListUnmanaged(u21){};
    ...
    pub fn withKeyValue(self: *CharCharSetMultimap, key: u21, value: u21) *CharCharSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharCharSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharCharSetMultimap, other: *const CharCharSetMultimap) bool {
...
test "CharCharSetMultimap: put and get" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 'a');
    m.put('a', 'b');
    m.put('b', 'c');
...
test "CharCharSetMultimap: get returns values" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 'a');
    m.put('a', 'b');
...
test "CharCharSetMultimap: get empty key" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharCharSetMultimap: removeAll" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 'a');
    m.put('a', 'b');
...
test "CharCharSetMultimap: containsKey and containsKeyValue" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 'a');
    m.put('a', 'b');
...
test "CharCharSetMultimap: clear" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 'a');
    m.put('b', 'b');
...
test "CharCharSetMultimap: select and reject" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 'a');
    m.put('a', 'b');
...
test "CharCharSetMultimap: anySatisfy noneSatisfy count" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 'a');
    m.put('b', 'b');
...
test "CharCharSetMultimap: uniqueKeys and valuesToSlice" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 'a');
    m.put('a', 'b');
...
test "CharCharSetMultimap: fluent withKeyValue" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharCharSetMultimap: eql" {
    var m1 = CharCharSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 'a');
    m1.put('a', 'b');
    var m2 = CharCharSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 'a');
    m2.put('a', 'b');
...
test "CharCharSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharCharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharCharSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharCharSetMultimap.init(failing.allocator());
    defer m.deinit();
