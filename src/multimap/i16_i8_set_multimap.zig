...
pub const I16I8SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I16I8SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i16, std.ArrayListUnmanaged(i8)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I16I8SetMultimap) void {
    ...
    pub fn put(self: *I16I8SetMultimap, key: i16, value: i8) void {
        const map_key = key;
    ...
    pub fn get(self: *const I16I8SetMultimap, key: i16) []const i8 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I16I8SetMultimap, key: i16) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I16I8SetMultimap, key: i16) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I16I8SetMultimap, key: i16) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I16I8SetMultimap, key: i16, value: i8) bool {
    ...
    pub fn keysCount(self: *const I16I8SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I16I8SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I16I8SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I16I8SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I16I8SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I16I8SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16I8SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16I8SetMultimap, f: *const fn (i16, i8) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I16I8SetMultimap, predicate: *const fn (i16, i8) bool) I16I8SetMultimap {
        var result = I16I8SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I16I8SetMultimap, predicate: *const fn (i16, i8) bool) I16I8SetMultimap {
        var result = I16I8SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I16I8SetMultimap, predicate: *const fn (i16, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I16I8SetMultimap, predicate: *const fn (i16, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I16I8SetMultimap, predicate: *const fn (i16, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I16I8SetMultimap, predicate: *const fn (i16, i8) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I16I8SetMultimap, allocator: Allocator) []i16 {
    ...
    pub fn valuesToSlice(self: *const I16I8SetMultimap, allocator: Allocator) []i8 {
    ...
    pub fn withKeyValue(self: *I16I8SetMultimap, key: i16, value: i8) *I16I8SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I16I8SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I16I8SetMultimap, other: *const I16I8SetMultimap) bool {
...
test "I16I8SetMultimap: put and get" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
    m.put(2, 3);
...
test "I16I8SetMultimap: get returns values" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I8SetMultimap: get empty key" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I16I8SetMultimap: removeAll" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I8SetMultimap: containsKey and containsKeyValue" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I8SetMultimap: select and reject" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I8SetMultimap: anySatisfy noneSatisfy count" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(2, 2);
...
test "I16I8SetMultimap: uniqueKeys and valuesToSlice" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1);
    m.put(1, 2);
...
test "I16I8SetMultimap: fluent withKeyValue" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16I8SetMultimap: eql" {
    var m1 = I16I8SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1);
    m1.put(1, 2);
    var m2 = I16I8SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1);
    m2.put(1, 2);
...
test "I16I8SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I16I8SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16I8SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16I8SetMultimap.init(failing.allocator());
    defer m.deinit();
