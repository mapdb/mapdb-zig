...
pub const F64BoolListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(u64, std.ArrayListUnmanaged(bool)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F64BoolListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u64, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *F64BoolListMultimap) void {
    ...
    pub fn put(self: *F64BoolListMultimap, key: f64, value: bool) void {
    ...
    pub fn get(self: *const F64BoolListMultimap, key: f64) []const bool {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn getCount(self: *const F64BoolListMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *F64BoolListMultimap, key: f64) usize {
        const map_key = @as(u64, @bitCast(key));
    ...
    pub fn containsKey(self: *const F64BoolListMultimap, key: f64) bool {
        const map_key = @as(u64, @bitCast(key));
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const F64BoolListMultimap, key: f64, value: bool) bool {
    ...
    pub fn keysCount(self: *const F64BoolListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const F64BoolListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const F64BoolListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const F64BoolListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *F64BoolListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *F64BoolListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F64BoolListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F64BoolListMultimap, f: *const fn (f64, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const F64BoolListMultimap, predicate: *const fn (f64, bool) bool) F64BoolListMultimap {
        var result = F64BoolListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const F64BoolListMultimap, predicate: *const fn (f64, bool) bool) F64BoolListMultimap {
        var result = F64BoolListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn anySatisfy(self: *const F64BoolListMultimap, predicate: *const fn (f64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const F64BoolListMultimap, predicate: *const fn (f64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const F64BoolListMultimap, predicate: *const fn (f64, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const F64BoolListMultimap, predicate: *const fn (f64, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f64, @bitCast(entry.key_ptr.*));
    ...
    pub fn uniqueKeys(self: *const F64BoolListMultimap, allocator: Allocator) []f64 {
    ...
    pub fn valuesToSlice(self: *const F64BoolListMultimap, allocator: Allocator) []bool {
    ...
    pub fn withKeyValue(self: *F64BoolListMultimap, key: f64, value: bool) *F64BoolListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const F64BoolListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const F64BoolListMultimap, other: *const F64BoolListMultimap) bool {
...
test "F64BoolListMultimap: put and get" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
    m.put(2.0, true);
...
test "F64BoolListMultimap: get returns values" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
...
test "F64BoolListMultimap: get empty key" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolListMultimap: removeAll" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
...
test "F64BoolListMultimap: containsKey and containsKeyValue" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
...
test "F64BoolListMultimap: clear" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(2.0, false);
...
test "F64BoolListMultimap: select" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(2.0, false);
...
test "F64BoolListMultimap: uniqueKeys and valuesToSlice" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, true);
    m.put(1.0, false);
...
test "F64BoolListMultimap: fluent withKeyValue" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolListMultimap: eql" {
    var m1 = F64BoolListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1.0, true);
    m1.put(1.0, false);
    var m2 = F64BoolListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1.0, true);
    m2.put(1.0, false);
...
test "F64BoolListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = F64BoolListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64BoolListMultimap.init(failing.allocator());
    defer m.deinit();
