...
/// List multimap from `i16` keys to `u21` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const I16CharListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(i16, std.ArrayListUnmanaged(u21)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) I16CharListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i16, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I16CharListMultimap) void {
    ...
    pub fn put(self: *I16CharListMultimap, key: i16, value: u21) void {
        const map_key = key;
    ...
    pub fn get(self: *const I16CharListMultimap, key: i16) []const u21 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I16CharListMultimap, key: i16) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I16CharListMultimap, key: i16) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I16CharListMultimap, key: i16) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I16CharListMultimap, key: i16, value: u21) bool {
    ...
    pub fn keysCount(self: *const I16CharListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I16CharListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I16CharListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I16CharListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I16CharListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I16CharListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16CharListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16CharListMultimap, f: *const fn (i16, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I16CharListMultimap, predicate: *const fn (i16, u21) bool) I16CharListMultimap {
        var result = I16CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I16CharListMultimap, predicate: *const fn (i16, u21) bool) I16CharListMultimap {
        var result = I16CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I16CharListMultimap, predicate: *const fn (i16, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I16CharListMultimap, predicate: *const fn (i16, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I16CharListMultimap, predicate: *const fn (i16, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I16CharListMultimap, predicate: *const fn (i16, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I16CharListMultimap, allocator: Allocator) []i16 {
    ...
    pub fn valuesToSlice(self: *const I16CharListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *I16CharListMultimap, key: i16, value: u21) *I16CharListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I16CharListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I16CharListMultimap, other: *const I16CharListMultimap) bool {
...
test "I16CharListMultimap: put and get" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
    m.put(2, 'c');
...
test "I16CharListMultimap: get returns values" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I16CharListMultimap: get empty key" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I16CharListMultimap: removeAll" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I16CharListMultimap: containsKey and containsKeyValue" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I16CharListMultimap: clear" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I16CharListMultimap: select and reject" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I16CharListMultimap: anySatisfy noneSatisfy count" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(2, 'b');
...
test "I16CharListMultimap: uniqueKeys and valuesToSlice" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 'a');
    m.put(1, 'b');
...
test "I16CharListMultimap: fluent withKeyValue" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16CharListMultimap: eql" {
    var m1 = I16CharListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 'a');
    m1.put(1, 'b');
    var m2 = I16CharListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 'a');
    m2.put(1, 'b');
...
test "I16CharListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I16CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16CharListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16CharListMultimap.init(failing.allocator());
    defer m.deinit();
