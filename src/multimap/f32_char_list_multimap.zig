...
/// List multimap from `f32` keys to `u21` values.
///
...
pub const F32CharListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(u32, std.ArrayListUnmanaged(u21)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) F32CharListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u32, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *F32CharListMultimap) void {
    ...
    pub fn put(self: *F32CharListMultimap, key: f32, value: u21) void {
    ...
    pub fn get(self: *const F32CharListMultimap, key: f32) []const u21 {
        const map_key = @as(u32, @bitCast(key));
    ...
    pub fn getCount(self: *const F32CharListMultimap, key: f32) usize {
        const map_key = @as(u32, @bitCast(key));
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *F32CharListMultimap, key: f32) usize {
        const map_key = @as(u32, @bitCast(key));
    ...
    pub fn containsKey(self: *const F32CharListMultimap, key: f32) bool {
        const map_key = @as(u32, @bitCast(key));
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const F32CharListMultimap, key: f32, value: u21) bool {
    ...
    pub fn keysCount(self: *const F32CharListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const F32CharListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const F32CharListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const F32CharListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *F32CharListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *F32CharListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F32CharListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32CharListMultimap, f: *const fn (f32, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const F32CharListMultimap, predicate: *const fn (f32, u21) bool) F32CharListMultimap {
        var result = F32CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const F32CharListMultimap, predicate: *const fn (f32, u21) bool) F32CharListMultimap {
        var result = F32CharListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
    ...
    pub fn anySatisfy(self: *const F32CharListMultimap, predicate: *const fn (f32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const F32CharListMultimap, predicate: *const fn (f32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const F32CharListMultimap, predicate: *const fn (f32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const F32CharListMultimap, predicate: *const fn (f32, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
    ...
    pub fn uniqueKeys(self: *const F32CharListMultimap, allocator: Allocator) []f32 {
    ...
    pub fn valuesToSlice(self: *const F32CharListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *F32CharListMultimap, key: f32, value: u21) *F32CharListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const F32CharListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const F32CharListMultimap, other: *const F32CharListMultimap) bool {
...
test "F32CharListMultimap: put and get" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
    m.put(2.0, 'c');
...
test "F32CharListMultimap: get returns values" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharListMultimap: get empty key" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32CharListMultimap: removeAll" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharListMultimap: containsKey and containsKeyValue" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharListMultimap: clear" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(2.0, 'b');
...
test "F32CharListMultimap: select and reject" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharListMultimap: anySatisfy noneSatisfy count" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(2.0, 'b');
...
test "F32CharListMultimap: uniqueKeys and valuesToSlice" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharListMultimap: fluent withKeyValue" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32CharListMultimap: eql" {
    var m1 = F32CharListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1.0, 'a');
    m1.put(1.0, 'b');
    var m2 = F32CharListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1.0, 'a');
    m2.put(1.0, 'b');
...
test "F32CharListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = F32CharListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32CharListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F32CharListMultimap.init(failing.allocator());
    defer m.deinit();
