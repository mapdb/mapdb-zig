...
pub const I16F32ListMultimap = struct {
    ...
    pub fn init(allocator: Allocator) I16F32ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(i16, std.ArrayListUnmanaged(f32)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *I16F32ListMultimap) void {
    ...
    pub fn put(self: *I16F32ListMultimap, key: i16, value: f32) void {
        const map_key = key;
    ...
    pub fn get(self: *const I16F32ListMultimap, key: i16) []const f32 {
        const map_key = key;
    ...
    pub fn getCount(self: *const I16F32ListMultimap, key: i16) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *I16F32ListMultimap, key: i16) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const I16F32ListMultimap, key: i16) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const I16F32ListMultimap, key: i16, value: f32) bool {
    ...
    pub fn keysCount(self: *const I16F32ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const I16F32ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const I16F32ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const I16F32ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *I16F32ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *I16F32ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *I16F32ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const I16F32ListMultimap, f: *const fn (i16, f32) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const I16F32ListMultimap, predicate: *const fn (i16, f32) bool) I16F32ListMultimap {
        var result = I16F32ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const I16F32ListMultimap, predicate: *const fn (i16, f32) bool) I16F32ListMultimap {
        var result = I16F32ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const I16F32ListMultimap, predicate: *const fn (i16, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const I16F32ListMultimap, predicate: *const fn (i16, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const I16F32ListMultimap, predicate: *const fn (i16, f32) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const I16F32ListMultimap, predicate: *const fn (i16, f32) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const I16F32ListMultimap, allocator: Allocator) []i16 {
    ...
    pub fn valuesToSlice(self: *const I16F32ListMultimap, allocator: Allocator) []f32 {
    ...
    pub fn withKeyValue(self: *I16F32ListMultimap, key: i16, value: f32) *I16F32ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const I16F32ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const I16F32ListMultimap, other: *const I16F32ListMultimap) bool {
...
test "I16F32ListMultimap: put and get" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
    m.put(2, 3.0);
...
test "I16F32ListMultimap: get returns values" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F32ListMultimap: get empty key" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get(99);
...
test "I16F32ListMultimap: removeAll" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F32ListMultimap: containsKey and containsKeyValue" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F32ListMultimap: select and reject" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F32ListMultimap: anySatisfy noneSatisfy count" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(2, 2.0);
...
test "I16F32ListMultimap: uniqueKeys and valuesToSlice" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1, 1.0);
    m.put(1, 2.0);
...
test "I16F32ListMultimap: fluent withKeyValue" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F32ListMultimap: eql" {
    var m1 = I16F32ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1, 1.0);
    m1.put(1, 2.0);
    var m2 = I16F32ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1, 1.0);
    m2.put(1, 2.0);
...
test "I16F32ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = I16F32ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16F32ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16F32ListMultimap.init(failing.allocator());
    defer m.deinit();
