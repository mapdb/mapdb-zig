...
pub const BoolI64SetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) BoolI64SetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(bool, std.ArrayListUnmanaged(i64)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *BoolI64SetMultimap) void {
    ...
    pub fn put(self: *BoolI64SetMultimap, key: bool, value: i64) void {
        const map_key = key;
    ...
    pub fn get(self: *const BoolI64SetMultimap, key: bool) []const i64 {
        const map_key = key;
    ...
    pub fn getCount(self: *const BoolI64SetMultimap, key: bool) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *BoolI64SetMultimap, key: bool) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const BoolI64SetMultimap, key: bool) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const BoolI64SetMultimap, key: bool, value: i64) bool {
    ...
    pub fn keysCount(self: *const BoolI64SetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const BoolI64SetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const BoolI64SetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const BoolI64SetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *BoolI64SetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *BoolI64SetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolI64SetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolI64SetMultimap, f: *const fn (bool, i64) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const BoolI64SetMultimap, predicate: *const fn (bool, i64) bool) BoolI64SetMultimap {
        var result = BoolI64SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const BoolI64SetMultimap, predicate: *const fn (bool, i64) bool) BoolI64SetMultimap {
        var result = BoolI64SetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const BoolI64SetMultimap, predicate: *const fn (bool, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const BoolI64SetMultimap, predicate: *const fn (bool, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const BoolI64SetMultimap, predicate: *const fn (bool, i64) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const BoolI64SetMultimap, predicate: *const fn (bool, i64) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const BoolI64SetMultimap, allocator: Allocator) []bool {
    ...
    pub fn valuesToSlice(self: *const BoolI64SetMultimap, allocator: Allocator) []i64 {
    ...
    pub fn withKeyValue(self: *BoolI64SetMultimap, key: bool, value: i64) *BoolI64SetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const BoolI64SetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const BoolI64SetMultimap, other: *const BoolI64SetMultimap) bool {
...
test "BoolI64SetMultimap: put and get" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
    m.put(false, 3);
...
test "BoolI64SetMultimap: get returns values" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI64SetMultimap: get empty key" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI64SetMultimap: removeAll" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI64SetMultimap: containsKey and containsKeyValue" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI64SetMultimap: select" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(false, 2);
...
test "BoolI64SetMultimap: uniqueKeys and valuesToSlice" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, 1);
    m.put(true, 2);
...
test "BoolI64SetMultimap: fluent withKeyValue" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI64SetMultimap: eql" {
    var m1 = BoolI64SetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(true, 1);
    m1.put(true, 2);
    var m2 = BoolI64SetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(true, 1);
    m2.put(true, 2);
...
test "BoolI64SetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = BoolI64SetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI64SetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolI64SetMultimap.init(failing.allocator());
    defer m.deinit();
