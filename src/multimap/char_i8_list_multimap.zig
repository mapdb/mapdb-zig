...
/// List multimap from `u21` keys to `i8` values.
///
/// Each key maps to a list of values, preserving insertion order per key.
/// Backed by `AutoHashMapUnmanaged` for O(1) key lookup.
pub const CharI8ListMultimap = struct {
    inner: std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i8)),
    allocator: Allocator,
    total_size: usize,

    // ---- Construction / Destruction ----

    pub fn init(allocator: Allocator) CharI8ListMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u21, std.ArrayListUnmanaged(i8)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *CharI8ListMultimap) void {
    ...
    pub fn put(self: *CharI8ListMultimap, key: u21, value: i8) void {
        const map_key = key;
    ...
    pub fn get(self: *const CharI8ListMultimap, key: u21) []const i8 {
        const map_key = key;
    ...
    pub fn getCount(self: *const CharI8ListMultimap, key: u21) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *CharI8ListMultimap, key: u21) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const CharI8ListMultimap, key: u21) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const CharI8ListMultimap, key: u21, value: i8) bool {
    ...
    pub fn keysCount(self: *const CharI8ListMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const CharI8ListMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const CharI8ListMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const CharI8ListMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *CharI8ListMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *CharI8ListMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *CharI8ListMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const CharI8ListMultimap, f: *const fn (u21, i8) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const CharI8ListMultimap, predicate: *const fn (u21, i8) bool) CharI8ListMultimap {
        var result = CharI8ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const CharI8ListMultimap, predicate: *const fn (u21, i8) bool) CharI8ListMultimap {
        var result = CharI8ListMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const CharI8ListMultimap, predicate: *const fn (u21, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const CharI8ListMultimap, predicate: *const fn (u21, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const CharI8ListMultimap, predicate: *const fn (u21, i8) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const CharI8ListMultimap, predicate: *const fn (u21, i8) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const CharI8ListMultimap, allocator: Allocator) []u21 {
    ...
    pub fn valuesToSlice(self: *const CharI8ListMultimap, allocator: Allocator) []i8 {
    ...
    pub fn withKeyValue(self: *CharI8ListMultimap, key: u21, value: i8) *CharI8ListMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const CharI8ListMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const CharI8ListMultimap, other: *const CharI8ListMultimap) bool {
...
test "CharI8ListMultimap: put and get" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
    m.put('b', 3);
...
test "CharI8ListMultimap: get returns values" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8ListMultimap: get empty key" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    const vals = m.get('z');
...
test "CharI8ListMultimap: removeAll" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8ListMultimap: containsKey and containsKeyValue" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8ListMultimap: select and reject" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8ListMultimap: anySatisfy noneSatisfy count" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('b', 2);
...
test "CharI8ListMultimap: uniqueKeys and valuesToSlice" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put('a', 1);
    m.put('a', 2);
...
test "CharI8ListMultimap: fluent withKeyValue" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8ListMultimap: eql" {
    var m1 = CharI8ListMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put('a', 1);
    m1.put('a', 2);
    var m2 = CharI8ListMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put('a', 1);
    m2.put('a', 2);
...
test "CharI8ListMultimap: ensureUnusedCapacity reserves map slots" {
    var m = CharI8ListMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8ListMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI8ListMultimap.init(failing.allocator());
    defer m.deinit();
