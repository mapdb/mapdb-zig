...
pub const BoolBoolSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) BoolBoolSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(bool, std.ArrayListUnmanaged(bool)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *BoolBoolSetMultimap) void {
    ...
    pub fn put(self: *BoolBoolSetMultimap, key: bool, value: bool) void {
        const map_key = key;
    ...
    pub fn get(self: *const BoolBoolSetMultimap, key: bool) []const bool {
        const map_key = key;
    ...
    pub fn getCount(self: *const BoolBoolSetMultimap, key: bool) usize {
        const map_key = key;
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *BoolBoolSetMultimap, key: bool) usize {
        const map_key = key;
    ...
    pub fn containsKey(self: *const BoolBoolSetMultimap, key: bool) bool {
        const map_key = key;
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const BoolBoolSetMultimap, key: bool, value: bool) bool {
    ...
    pub fn keysCount(self: *const BoolBoolSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const BoolBoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const BoolBoolSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const BoolBoolSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *BoolBoolSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *BoolBoolSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *BoolBoolSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const BoolBoolSetMultimap, f: *const fn (bool, bool) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const BoolBoolSetMultimap, predicate: *const fn (bool, bool) bool) BoolBoolSetMultimap {
        var result = BoolBoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const BoolBoolSetMultimap, predicate: *const fn (bool, bool) bool) BoolBoolSetMultimap {
        var result = BoolBoolSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn anySatisfy(self: *const BoolBoolSetMultimap, predicate: *const fn (bool, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const BoolBoolSetMultimap, predicate: *const fn (bool, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const BoolBoolSetMultimap, predicate: *const fn (bool, bool) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const BoolBoolSetMultimap, predicate: *const fn (bool, bool) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = entry.key_ptr.*;
    ...
    pub fn uniqueKeys(self: *const BoolBoolSetMultimap, allocator: Allocator) []bool {
    ...
    pub fn valuesToSlice(self: *const BoolBoolSetMultimap, allocator: Allocator) []bool {
        var result = std.ArrayListUnmanaged(bool){};
    ...
    pub fn withKeyValue(self: *BoolBoolSetMultimap, key: bool, value: bool) *BoolBoolSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const BoolBoolSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const BoolBoolSetMultimap, other: *const BoolBoolSetMultimap) bool {
...
test "BoolBoolSetMultimap: put and get" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
    m.put(false, true);
...
test "BoolBoolSetMultimap: get returns values" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
...
test "BoolBoolSetMultimap: get empty key" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolBoolSetMultimap: removeAll" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
...
test "BoolBoolSetMultimap: containsKey and containsKeyValue" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
...
test "BoolBoolSetMultimap: clear" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(false, false);
...
test "BoolBoolSetMultimap: select" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(false, false);
...
test "BoolBoolSetMultimap: uniqueKeys and valuesToSlice" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(true, true);
    m.put(true, false);
...
test "BoolBoolSetMultimap: fluent withKeyValue" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolBoolSetMultimap: eql" {
    var m1 = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(true, true);
    m1.put(true, false);
    var m2 = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(true, true);
    m2.put(true, false);
...
test "BoolBoolSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = BoolBoolSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolBoolSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolBoolSetMultimap.init(failing.allocator());
    defer m.deinit();
