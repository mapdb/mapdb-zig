...
/// Set multimap from `f32` keys to `u21` values.
///
...
pub const F32CharSetMultimap = struct {
    ...
    pub fn init(allocator: Allocator) F32CharSetMultimap {
        return .{
            .inner = std.AutoHashMapUnmanaged(u32, std.ArrayListUnmanaged(u21)){},
            .allocator = allocator,
            .total_size = 0,
    ...
    pub fn deinit(self: *F32CharSetMultimap) void {
    ...
    pub fn put(self: *F32CharSetMultimap, key: f32, value: u21) void {
    ...
    pub fn get(self: *const F32CharSetMultimap, key: f32) []const u21 {
        const map_key = @as(u32, @bitCast(key));
    ...
    pub fn getCount(self: *const F32CharSetMultimap, key: f32) usize {
        const map_key = @as(u32, @bitCast(key));
        if (self.inner.getPtr(map_key)) |list_ptr| {
    ...
    pub fn removeAll(self: *F32CharSetMultimap, key: f32) usize {
        const map_key = @as(u32, @bitCast(key));
    ...
    pub fn containsKey(self: *const F32CharSetMultimap, key: f32) bool {
        const map_key = @as(u32, @bitCast(key));
        return self.inner.contains(map_key);
    }

    /// Returns true if the multimap contains the given key-value pair.
    pub fn containsKeyValue(self: *const F32CharSetMultimap, key: f32, value: u21) bool {
    ...
    pub fn keysCount(self: *const F32CharSetMultimap) usize {
        return self.inner.count();
    }

    /// Returns the total number of values across all keys.
    pub fn size(self: *const F32CharSetMultimap) usize {
        return self.total_size;
    }

    pub fn len(self: *const F32CharSetMultimap) usize {
        return self.total_size;
    }

    pub fn isEmpty(self: *const F32CharSetMultimap) bool {
        return self.total_size == 0;
    }

    pub fn clear(self: *F32CharSetMultimap) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            entry.value_ptr.deinit(self.allocator);
        }
    ...
    pub fn ensureUnusedCapacity(self: *F32CharSetMultimap, additional: usize) Allocator.Error!void {
    ...
    pub fn ensureTotalCapacity(self: *F32CharSetMultimap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn forEach(self: *const F32CharSetMultimap, f: *const fn (f32, u21) void) void {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn select(self: *const F32CharSetMultimap, predicate: *const fn (f32, u21) bool) F32CharSetMultimap {
        var result = F32CharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn reject(self: *const F32CharSetMultimap, predicate: *const fn (f32, u21) bool) F32CharSetMultimap {
        var result = F32CharSetMultimap.init(self.allocator);
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
    ...
    pub fn anySatisfy(self: *const F32CharSetMultimap, predicate: *const fn (f32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn allSatisfy(self: *const F32CharSetMultimap, predicate: *const fn (f32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn noneSatisfy(self: *const F32CharSetMultimap, predicate: *const fn (f32, u21) bool) bool {
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
            for (entry.value_ptr.items) |v| {
    ...
    pub fn count(self: *const F32CharSetMultimap, predicate: *const fn (f32, u21) bool) usize {
        var c: usize = 0;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
            const key = @as(f32, @bitCast(entry.key_ptr.*));
    ...
    pub fn uniqueKeys(self: *const F32CharSetMultimap, allocator: Allocator) []f32 {
    ...
    pub fn valuesToSlice(self: *const F32CharSetMultimap, allocator: Allocator) []u21 {
    ...
    pub fn withKeyValue(self: *F32CharSetMultimap, key: f32, value: u21) *F32CharSetMultimap {
        self.put(key, value);
        return self;
    }
    ...
    pub fn format(self: *const F32CharSetMultimap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
        var first = true;
        var it = self.inner.iterator();
        while (it.next()) |entry| {
    ...
    pub fn eql(self: *const F32CharSetMultimap, other: *const F32CharSetMultimap) bool {
...
test "F32CharSetMultimap: put and get" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
    m.put(2.0, 'c');
...
test "F32CharSetMultimap: get returns values" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharSetMultimap: get empty key" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32CharSetMultimap: removeAll" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharSetMultimap: containsKey and containsKeyValue" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharSetMultimap: select and reject" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharSetMultimap: anySatisfy noneSatisfy count" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(2.0, 'b');
...
test "F32CharSetMultimap: uniqueKeys and valuesToSlice" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
    m.put(1.0, 'a');
    m.put(1.0, 'b');
...
test "F32CharSetMultimap: fluent withKeyValue" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32CharSetMultimap: eql" {
    var m1 = F32CharSetMultimap.init(std.testing.allocator);
    defer m1.deinit();
    m1.put(1.0, 'a');
    m1.put(1.0, 'b');
    var m2 = F32CharSetMultimap.init(std.testing.allocator);
    defer m2.deinit();
    m2.put(1.0, 'a');
    m2.put(1.0, 'b');
...
test "F32CharSetMultimap: ensureUnusedCapacity reserves map slots" {
    var m = F32CharSetMultimap.init(std.testing.allocator);
    defer m.deinit();
...
test "F32CharSetMultimap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F32CharSetMultimap.init(failing.allocator());
    defer m.deinit();
