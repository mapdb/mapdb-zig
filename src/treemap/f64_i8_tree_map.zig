...
/// Sorted map from `f64` keys to `i8` values, backed by sorted ArrayLists.
pub const F64I8TreeMap = struct {
    keys: std.ArrayListUnmanaged(f64) = .empty,
    vals: std.ArrayListUnmanaged(i8) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) F64I8TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) F64I8TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *F64I8TreeMap) void {
    ...
    fn findIndex(self: *const F64I8TreeMap, key: f64) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *F64I8TreeMap, key: f64, value: i8) ?i8 {
    ...
    pub fn get(self: *const F64I8TreeMap, key: f64) ?i8 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *F64I8TreeMap, key: f64) ?i8 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const F64I8TreeMap, key: f64) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const F64I8TreeMap, key: f64, default_value: i8) i8 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const F64I8TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const F64I8TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const F64I8TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *F64I8TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *F64I8TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *F64I8TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const F64I8TreeMap) ?struct { key: f64, value: i8 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const F64I8TreeMap) ?struct { key: f64, value: i8 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const F64I8TreeMap) []const f64 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const F64I8TreeMap) []const i8 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const F64I8TreeMap, f: *const fn (f64, i8) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const F64I8TreeMap, f: *const fn (f64) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const F64I8TreeMap, f: *const fn (i8) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F64I8TreeMap, predicate: *const fn (f64, i8) bool) F64I8TreeMap {
    ...
    pub fn reject(self: *const F64I8TreeMap, predicate: *const fn (f64, i8) bool) F64I8TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const F64I8TreeMap, predicate: *const fn (f64, i8) bool) ?struct { key: f64, value: i8 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const F64I8TreeMap, predicate: *const fn (f64, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const F64I8TreeMap, predicate: *const fn (f64, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const F64I8TreeMap, predicate: *const fn (f64, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F64I8TreeMap, predicate: *const fn (f64, i8) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const F64I8TreeMap, key: f64) ?struct { key: f64, value: i8 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const F64I8TreeMap, key: f64) ?struct { key: f64, value: i8 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const F64I8TreeMap, from: f64, to: f64) []const f64 {
    ...
    pub fn format(self: *const F64I8TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *F64I8TreeMap, key: f64, value: i8) *F64I8TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F64I8TreeMap, key: f64) *F64I8TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const F64I8TreeMap, other: *const F64I8TreeMap) bool {
...
test "F64I8TreeMap: put and get" {
    var m = F64I8TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, 1);
    _ = m.put(2.0, 2);
    _ = m.put(3.0, 3);
...
test "F64I8TreeMap: eql" {
    var m1 = F64I8TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, 1);
    _ = m1.put(2.0, 2);

    var m2 = F64I8TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, 2);
    _ = m2.put(1.0, 1);
...
test "F64I8TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = F64I8TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64I8TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64I8TreeMap.init(failing.allocator());
    defer m.deinit();
