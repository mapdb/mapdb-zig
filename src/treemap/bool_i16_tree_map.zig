...
/// Sorted map from `bool` keys to `i16` values, backed by sorted ArrayLists.
pub const BoolI16TreeMap = struct {
    keys: std.ArrayListUnmanaged(bool) = .empty,
    vals: std.ArrayListUnmanaged(i16) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) BoolI16TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) BoolI16TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *BoolI16TreeMap) void {
    ...
    fn findIndex(self: *const BoolI16TreeMap, key: bool) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *BoolI16TreeMap, key: bool, value: i16) ?i16 {
    ...
    pub fn get(self: *const BoolI16TreeMap, key: bool) ?i16 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *BoolI16TreeMap, key: bool) ?i16 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const BoolI16TreeMap, key: bool) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const BoolI16TreeMap, key: bool, default_value: i16) i16 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const BoolI16TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const BoolI16TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const BoolI16TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *BoolI16TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *BoolI16TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *BoolI16TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const BoolI16TreeMap) ?struct { key: bool, value: i16 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const BoolI16TreeMap) ?struct { key: bool, value: i16 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const BoolI16TreeMap) []const bool {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const BoolI16TreeMap) []const i16 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const BoolI16TreeMap, f: *const fn (bool, i16) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const BoolI16TreeMap, f: *const fn (bool) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const BoolI16TreeMap, f: *const fn (i16) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const BoolI16TreeMap, predicate: *const fn (bool, i16) bool) BoolI16TreeMap {
    ...
    pub fn reject(self: *const BoolI16TreeMap, predicate: *const fn (bool, i16) bool) BoolI16TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const BoolI16TreeMap, predicate: *const fn (bool, i16) bool) ?struct { key: bool, value: i16 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const BoolI16TreeMap, predicate: *const fn (bool, i16) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const BoolI16TreeMap, predicate: *const fn (bool, i16) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const BoolI16TreeMap, predicate: *const fn (bool, i16) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const BoolI16TreeMap, predicate: *const fn (bool, i16) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const BoolI16TreeMap, key: bool) ?struct { key: bool, value: i16 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const BoolI16TreeMap, key: bool) ?struct { key: bool, value: i16 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const BoolI16TreeMap, from: bool, to: bool) []const bool {
    ...
    pub fn format(self: *const BoolI16TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *BoolI16TreeMap, key: bool, value: i16) *BoolI16TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *BoolI16TreeMap, key: bool) *BoolI16TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const BoolI16TreeMap, other: *const BoolI16TreeMap) bool {
...
test "BoolI16TreeMap: put and get" {
    var m = BoolI16TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI16TreeMap: sorted keys" {
    var m = BoolI16TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI16TreeMap: eql" {
    var m1 = BoolI16TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(true, 1);
    _ = m1.put(false, 2);

    var m2 = BoolI16TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(false, 2);
    _ = m2.put(true, 1);
...
test "BoolI16TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = BoolI16TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI16TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolI16TreeMap.init(failing.allocator());
    defer m.deinit();
