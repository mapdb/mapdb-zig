...
/// Sorted map from `i64` keys to `i8` values, backed by sorted ArrayLists.
pub const I64I8TreeMap = struct {
    keys: std.ArrayListUnmanaged(i64) = .empty,
    vals: std.ArrayListUnmanaged(i8) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) I64I8TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) I64I8TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *I64I8TreeMap) void {
    ...
    fn findIndex(self: *const I64I8TreeMap, key: i64) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *I64I8TreeMap, key: i64, value: i8) ?i8 {
    ...
    pub fn get(self: *const I64I8TreeMap, key: i64) ?i8 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *I64I8TreeMap, key: i64) ?i8 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const I64I8TreeMap, key: i64) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const I64I8TreeMap, key: i64, default_value: i8) i8 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const I64I8TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const I64I8TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const I64I8TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *I64I8TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *I64I8TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *I64I8TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const I64I8TreeMap) ?struct { key: i64, value: i8 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const I64I8TreeMap) ?struct { key: i64, value: i8 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const I64I8TreeMap) []const i64 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const I64I8TreeMap) []const i8 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const I64I8TreeMap, f: *const fn (i64, i8) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const I64I8TreeMap, f: *const fn (i64) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const I64I8TreeMap, f: *const fn (i8) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I64I8TreeMap, predicate: *const fn (i64, i8) bool) I64I8TreeMap {
    ...
    pub fn reject(self: *const I64I8TreeMap, predicate: *const fn (i64, i8) bool) I64I8TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const I64I8TreeMap, predicate: *const fn (i64, i8) bool) ?struct { key: i64, value: i8 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const I64I8TreeMap, predicate: *const fn (i64, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const I64I8TreeMap, predicate: *const fn (i64, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const I64I8TreeMap, predicate: *const fn (i64, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I64I8TreeMap, predicate: *const fn (i64, i8) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const I64I8TreeMap, key: i64) ?struct { key: i64, value: i8 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const I64I8TreeMap, key: i64) ?struct { key: i64, value: i8 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const I64I8TreeMap, from: i64, to: i64) []const i64 {
    ...
    pub fn format(self: *const I64I8TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *I64I8TreeMap, key: i64, value: i8) *I64I8TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I64I8TreeMap, key: i64) *I64I8TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const I64I8TreeMap, other: *const I64I8TreeMap) bool {
...
test "I64I8TreeMap: put and get" {
    var m = I64I8TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    _ = m.put(3, 3);
...
test "I64I8TreeMap: eql" {
    var m1 = I64I8TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);

    var m2 = I64I8TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I64I8TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = I64I8TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I64I8TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I64I8TreeMap.init(failing.allocator());
    defer m.deinit();
