...
/// Sorted map from `i16` keys to `u21` values, backed by sorted ArrayLists.
pub const I16CharTreeMap = struct {
    keys: std.ArrayListUnmanaged(i16) = .empty,
    vals: std.ArrayListUnmanaged(u21) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) I16CharTreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) I16CharTreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *I16CharTreeMap) void {
    ...
    fn findIndex(self: *const I16CharTreeMap, key: i16) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *I16CharTreeMap, key: i16, value: u21) ?u21 {
    ...
    pub fn get(self: *const I16CharTreeMap, key: i16) ?u21 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *I16CharTreeMap, key: i16) ?u21 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const I16CharTreeMap, key: i16) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const I16CharTreeMap, key: i16, default_value: u21) u21 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const I16CharTreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const I16CharTreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const I16CharTreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *I16CharTreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *I16CharTreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *I16CharTreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const I16CharTreeMap) ?struct { key: i16, value: u21 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const I16CharTreeMap) ?struct { key: i16, value: u21 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const I16CharTreeMap) []const i16 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const I16CharTreeMap) []const u21 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const I16CharTreeMap, f: *const fn (i16, u21) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const I16CharTreeMap, f: *const fn (i16) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const I16CharTreeMap, f: *const fn (u21) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I16CharTreeMap, predicate: *const fn (i16, u21) bool) I16CharTreeMap {
    ...
    pub fn reject(self: *const I16CharTreeMap, predicate: *const fn (i16, u21) bool) I16CharTreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const I16CharTreeMap, predicate: *const fn (i16, u21) bool) ?struct { key: i16, value: u21 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const I16CharTreeMap, predicate: *const fn (i16, u21) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const I16CharTreeMap, predicate: *const fn (i16, u21) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const I16CharTreeMap, predicate: *const fn (i16, u21) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I16CharTreeMap, predicate: *const fn (i16, u21) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const I16CharTreeMap, key: i16) ?struct { key: i16, value: u21 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const I16CharTreeMap, key: i16) ?struct { key: i16, value: u21 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const I16CharTreeMap, from: i16, to: i16) []const i16 {
    ...
    pub fn format(self: *const I16CharTreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *I16CharTreeMap, key: i16, value: u21) *I16CharTreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I16CharTreeMap, key: i16) *I16CharTreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const I16CharTreeMap, other: *const I16CharTreeMap) bool {
...
test "I16CharTreeMap: put and get" {
    var m = I16CharTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 'a');
    _ = m.put(2, 'b');
    _ = m.put(3, 'c');
...
test "I16CharTreeMap: sorted keys" {
    var m = I16CharTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(3, 'c');
    _ = m.put(1, 'a');
...
test "I16CharTreeMap: eql" {
    var m1 = I16CharTreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 'a');
    _ = m1.put(2, 'b');

    var m2 = I16CharTreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 'b');
    _ = m2.put(1, 'a');
...
test "I16CharTreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = I16CharTreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16CharTreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16CharTreeMap.init(failing.allocator());
    defer m.deinit();
