...
/// Sorted map from `u21` keys to `i8` values, backed by sorted ArrayLists.
pub const CharI8TreeMap = struct {
    keys: std.ArrayListUnmanaged(u21) = .empty,
    vals: std.ArrayListUnmanaged(i8) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) CharI8TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) CharI8TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *CharI8TreeMap) void {
    ...
    fn findIndex(self: *const CharI8TreeMap, key: u21) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *CharI8TreeMap, key: u21, value: i8) ?i8 {
    ...
    pub fn get(self: *const CharI8TreeMap, key: u21) ?i8 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *CharI8TreeMap, key: u21) ?i8 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const CharI8TreeMap, key: u21) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const CharI8TreeMap, key: u21, default_value: i8) i8 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const CharI8TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const CharI8TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const CharI8TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *CharI8TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *CharI8TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *CharI8TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const CharI8TreeMap) ?struct { key: u21, value: i8 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const CharI8TreeMap) ?struct { key: u21, value: i8 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const CharI8TreeMap) []const u21 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const CharI8TreeMap) []const i8 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const CharI8TreeMap, f: *const fn (u21, i8) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const CharI8TreeMap, f: *const fn (u21) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const CharI8TreeMap, f: *const fn (i8) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const CharI8TreeMap, predicate: *const fn (u21, i8) bool) CharI8TreeMap {
    ...
    pub fn reject(self: *const CharI8TreeMap, predicate: *const fn (u21, i8) bool) CharI8TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const CharI8TreeMap, predicate: *const fn (u21, i8) bool) ?struct { key: u21, value: i8 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const CharI8TreeMap, predicate: *const fn (u21, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const CharI8TreeMap, predicate: *const fn (u21, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const CharI8TreeMap, predicate: *const fn (u21, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const CharI8TreeMap, predicate: *const fn (u21, i8) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const CharI8TreeMap, key: u21) ?struct { key: u21, value: i8 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const CharI8TreeMap, key: u21) ?struct { key: u21, value: i8 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const CharI8TreeMap, from: u21, to: u21) []const u21 {
    ...
    pub fn format(self: *const CharI8TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *CharI8TreeMap, key: u21, value: i8) *CharI8TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *CharI8TreeMap, key: u21) *CharI8TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const CharI8TreeMap, other: *const CharI8TreeMap) bool {
...
test "CharI8TreeMap: put and get" {
    var m = CharI8TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1);
    _ = m.put('b', 2);
    _ = m.put('c', 3);
...
test "CharI8TreeMap: sorted keys" {
    var m = CharI8TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('c', 3);
    _ = m.put('a', 1);
...
test "CharI8TreeMap: eql" {
    var m1 = CharI8TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1);
    _ = m1.put('b', 2);

    var m2 = CharI8TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2);
    _ = m2.put('a', 1);
...
test "CharI8TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = CharI8TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharI8TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharI8TreeMap.init(failing.allocator());
    defer m.deinit();
