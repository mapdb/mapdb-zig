...
/// Sorted map from `u21` keys to `u21` values, backed by sorted ArrayLists.
pub const CharCharTreeMap = struct {
    keys: std.ArrayListUnmanaged(u21) = .empty,
    vals: std.ArrayListUnmanaged(u21) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) CharCharTreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) CharCharTreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *CharCharTreeMap) void {
    ...
    fn findIndex(self: *const CharCharTreeMap, key: u21) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *CharCharTreeMap, key: u21, value: u21) ?u21 {
    ...
    pub fn get(self: *const CharCharTreeMap, key: u21) ?u21 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *CharCharTreeMap, key: u21) ?u21 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const CharCharTreeMap, key: u21) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const CharCharTreeMap, key: u21, default_value: u21) u21 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const CharCharTreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const CharCharTreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const CharCharTreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *CharCharTreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *CharCharTreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *CharCharTreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const CharCharTreeMap) ?struct { key: u21, value: u21 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const CharCharTreeMap) ?struct { key: u21, value: u21 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const CharCharTreeMap) []const u21 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const CharCharTreeMap) []const u21 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const CharCharTreeMap, f: *const fn (u21, u21) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const CharCharTreeMap, f: *const fn (u21) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const CharCharTreeMap, f: *const fn (u21) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const CharCharTreeMap, predicate: *const fn (u21, u21) bool) CharCharTreeMap {
    ...
    pub fn reject(self: *const CharCharTreeMap, predicate: *const fn (u21, u21) bool) CharCharTreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const CharCharTreeMap, predicate: *const fn (u21, u21) bool) ?struct { key: u21, value: u21 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const CharCharTreeMap, predicate: *const fn (u21, u21) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const CharCharTreeMap, predicate: *const fn (u21, u21) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const CharCharTreeMap, predicate: *const fn (u21, u21) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const CharCharTreeMap, predicate: *const fn (u21, u21) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const CharCharTreeMap, key: u21) ?struct { key: u21, value: u21 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const CharCharTreeMap, key: u21) ?struct { key: u21, value: u21 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const CharCharTreeMap, from: u21, to: u21) []const u21 {
    ...
    pub fn format(self: *const CharCharTreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *CharCharTreeMap, key: u21, value: u21) *CharCharTreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *CharCharTreeMap, key: u21) *CharCharTreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const CharCharTreeMap, other: *const CharCharTreeMap) bool {
...
test "CharCharTreeMap: put and get" {
    var m = CharCharTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 'a');
    _ = m.put('b', 'b');
    _ = m.put('c', 'c');
...
test "CharCharTreeMap: sorted keys" {
    var m = CharCharTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('c', 'c');
    _ = m.put('a', 'a');
...
test "CharCharTreeMap: eql" {
    var m1 = CharCharTreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 'a');
    _ = m1.put('b', 'b');

    var m2 = CharCharTreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 'b');
    _ = m2.put('a', 'a');
...
test "CharCharTreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = CharCharTreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharCharTreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharCharTreeMap.init(failing.allocator());
    defer m.deinit();
