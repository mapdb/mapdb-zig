...
/// Sorted map from `i16` keys to `i16` values, backed by sorted ArrayLists.
pub const I16I16TreeMap = struct {
    keys: std.ArrayListUnmanaged(i16) = .empty,
    vals: std.ArrayListUnmanaged(i16) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) I16I16TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) I16I16TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *I16I16TreeMap) void {
    ...
    fn findIndex(self: *const I16I16TreeMap, key: i16) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *I16I16TreeMap, key: i16, value: i16) ?i16 {
    ...
    pub fn get(self: *const I16I16TreeMap, key: i16) ?i16 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *I16I16TreeMap, key: i16) ?i16 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const I16I16TreeMap, key: i16) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const I16I16TreeMap, key: i16, default_value: i16) i16 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const I16I16TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const I16I16TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const I16I16TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *I16I16TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *I16I16TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *I16I16TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const I16I16TreeMap) ?struct { key: i16, value: i16 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const I16I16TreeMap) ?struct { key: i16, value: i16 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const I16I16TreeMap) []const i16 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const I16I16TreeMap) []const i16 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const I16I16TreeMap, f: *const fn (i16, i16) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const I16I16TreeMap, f: *const fn (i16) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const I16I16TreeMap, f: *const fn (i16) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I16I16TreeMap, predicate: *const fn (i16, i16) bool) I16I16TreeMap {
    ...
    pub fn reject(self: *const I16I16TreeMap, predicate: *const fn (i16, i16) bool) I16I16TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const I16I16TreeMap, predicate: *const fn (i16, i16) bool) ?struct { key: i16, value: i16 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const I16I16TreeMap, predicate: *const fn (i16, i16) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const I16I16TreeMap, predicate: *const fn (i16, i16) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const I16I16TreeMap, predicate: *const fn (i16, i16) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I16I16TreeMap, predicate: *const fn (i16, i16) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const I16I16TreeMap, key: i16) ?struct { key: i16, value: i16 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const I16I16TreeMap, key: i16) ?struct { key: i16, value: i16 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const I16I16TreeMap, from: i16, to: i16) []const i16 {
    ...
    pub fn format(self: *const I16I16TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *I16I16TreeMap, key: i16, value: i16) *I16I16TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I16I16TreeMap, key: i16) *I16I16TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const I16I16TreeMap, other: *const I16I16TreeMap) bool {
...
test "I16I16TreeMap: put and get" {
    var m = I16I16TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1);
    _ = m.put(2, 2);
    _ = m.put(3, 3);
...
test "I16I16TreeMap: sorted keys" {
    var m = I16I16TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(3, 3);
    _ = m.put(1, 1);
...
test "I16I16TreeMap: eql" {
    var m1 = I16I16TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1);
    _ = m1.put(2, 2);

    var m2 = I16I16TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2);
    _ = m2.put(1, 1);
...
test "I16I16TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = I16I16TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I16I16TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I16I16TreeMap.init(failing.allocator());
    defer m.deinit();
