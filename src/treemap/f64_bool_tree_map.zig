...
/// Sorted map from `f64` keys to `bool` values, backed by sorted ArrayLists.
pub const F64BoolTreeMap = struct {
    keys: std.ArrayListUnmanaged(f64) = .empty,
    vals: std.ArrayListUnmanaged(bool) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) F64BoolTreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) F64BoolTreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *F64BoolTreeMap) void {
    ...
    fn findIndex(self: *const F64BoolTreeMap, key: f64) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *F64BoolTreeMap, key: f64, value: bool) ?bool {
    ...
    pub fn get(self: *const F64BoolTreeMap, key: f64) ?bool {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *F64BoolTreeMap, key: f64) ?bool {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const F64BoolTreeMap, key: f64) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const F64BoolTreeMap, key: f64, default_value: bool) bool {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const F64BoolTreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const F64BoolTreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const F64BoolTreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *F64BoolTreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *F64BoolTreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *F64BoolTreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const F64BoolTreeMap) ?struct { key: f64, value: bool } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const F64BoolTreeMap) ?struct { key: f64, value: bool } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const F64BoolTreeMap) []const f64 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const F64BoolTreeMap) []const bool {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const F64BoolTreeMap, f: *const fn (f64, bool) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const F64BoolTreeMap, f: *const fn (f64) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const F64BoolTreeMap, f: *const fn (bool) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const F64BoolTreeMap, predicate: *const fn (f64, bool) bool) F64BoolTreeMap {
    ...
    pub fn reject(self: *const F64BoolTreeMap, predicate: *const fn (f64, bool) bool) F64BoolTreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const F64BoolTreeMap, predicate: *const fn (f64, bool) bool) ?struct { key: f64, value: bool } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const F64BoolTreeMap, predicate: *const fn (f64, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const F64BoolTreeMap, predicate: *const fn (f64, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const F64BoolTreeMap, predicate: *const fn (f64, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const F64BoolTreeMap, predicate: *const fn (f64, bool) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const F64BoolTreeMap, key: f64) ?struct { key: f64, value: bool } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const F64BoolTreeMap, key: f64) ?struct { key: f64, value: bool } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const F64BoolTreeMap, from: f64, to: f64) []const f64 {
    ...
    pub fn format(self: *const F64BoolTreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *F64BoolTreeMap, key: f64, value: bool) *F64BoolTreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *F64BoolTreeMap, key: f64) *F64BoolTreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const F64BoolTreeMap, other: *const F64BoolTreeMap) bool {
...
test "F64BoolTreeMap: put and get" {
    var m = F64BoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1.0, true);
    _ = m.put(2.0, false);
    _ = m.put(3.0, true);
...
test "F64BoolTreeMap: sorted keys" {
    var m = F64BoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(3.0, true);
    _ = m.put(1.0, true);
...
test "F64BoolTreeMap: eql" {
    var m1 = F64BoolTreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1.0, true);
    _ = m1.put(2.0, false);

    var m2 = F64BoolTreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2.0, false);
    _ = m2.put(1.0, true);
...
test "F64BoolTreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = F64BoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "F64BoolTreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = F64BoolTreeMap.init(failing.allocator());
    defer m.deinit();
