...
/// Sorted map from `i8` keys to `bool` values, backed by sorted ArrayLists.
pub const I8BoolTreeMap = struct {
    keys: std.ArrayListUnmanaged(i8) = .empty,
    vals: std.ArrayListUnmanaged(bool) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) I8BoolTreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) I8BoolTreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *I8BoolTreeMap) void {
    ...
    fn findIndex(self: *const I8BoolTreeMap, key: i8) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *I8BoolTreeMap, key: i8, value: bool) ?bool {
    ...
    pub fn get(self: *const I8BoolTreeMap, key: i8) ?bool {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *I8BoolTreeMap, key: i8) ?bool {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const I8BoolTreeMap, key: i8) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const I8BoolTreeMap, key: i8, default_value: bool) bool {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const I8BoolTreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const I8BoolTreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const I8BoolTreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *I8BoolTreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *I8BoolTreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *I8BoolTreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const I8BoolTreeMap) ?struct { key: i8, value: bool } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const I8BoolTreeMap) ?struct { key: i8, value: bool } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const I8BoolTreeMap) []const i8 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const I8BoolTreeMap) []const bool {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const I8BoolTreeMap, f: *const fn (i8, bool) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const I8BoolTreeMap, f: *const fn (i8) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const I8BoolTreeMap, f: *const fn (bool) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I8BoolTreeMap, predicate: *const fn (i8, bool) bool) I8BoolTreeMap {
    ...
    pub fn reject(self: *const I8BoolTreeMap, predicate: *const fn (i8, bool) bool) I8BoolTreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const I8BoolTreeMap, predicate: *const fn (i8, bool) bool) ?struct { key: i8, value: bool } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const I8BoolTreeMap, predicate: *const fn (i8, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const I8BoolTreeMap, predicate: *const fn (i8, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const I8BoolTreeMap, predicate: *const fn (i8, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I8BoolTreeMap, predicate: *const fn (i8, bool) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const I8BoolTreeMap, key: i8) ?struct { key: i8, value: bool } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const I8BoolTreeMap, key: i8) ?struct { key: i8, value: bool } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const I8BoolTreeMap, from: i8, to: i8) []const i8 {
    ...
    pub fn format(self: *const I8BoolTreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *I8BoolTreeMap, key: i8, value: bool) *I8BoolTreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I8BoolTreeMap, key: i8) *I8BoolTreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const I8BoolTreeMap, other: *const I8BoolTreeMap) bool {
...
test "I8BoolTreeMap: put and get" {
    var m = I8BoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, true);
    _ = m.put(2, false);
    _ = m.put(3, true);
...
test "I8BoolTreeMap: sorted keys" {
    var m = I8BoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(3, true);
    _ = m.put(1, true);
...
test "I8BoolTreeMap: eql" {
    var m1 = I8BoolTreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, true);
    _ = m1.put(2, false);

    var m2 = I8BoolTreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, false);
    _ = m2.put(1, true);
...
test "I8BoolTreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = I8BoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I8BoolTreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I8BoolTreeMap.init(failing.allocator());
    defer m.deinit();
