...
/// Sorted map from `u21` keys to `f64` values, backed by sorted ArrayLists.
pub const CharF64TreeMap = struct {
    keys: std.ArrayListUnmanaged(u21) = .empty,
    vals: std.ArrayListUnmanaged(f64) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) CharF64TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) CharF64TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *CharF64TreeMap) void {
    ...
    fn findIndex(self: *const CharF64TreeMap, key: u21) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *CharF64TreeMap, key: u21, value: f64) ?f64 {
    ...
    pub fn get(self: *const CharF64TreeMap, key: u21) ?f64 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *CharF64TreeMap, key: u21) ?f64 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const CharF64TreeMap, key: u21) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const CharF64TreeMap, key: u21, default_value: f64) f64 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const CharF64TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const CharF64TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const CharF64TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *CharF64TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *CharF64TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *CharF64TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const CharF64TreeMap) ?struct { key: u21, value: f64 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const CharF64TreeMap) ?struct { key: u21, value: f64 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const CharF64TreeMap) []const u21 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const CharF64TreeMap) []const f64 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const CharF64TreeMap, f: *const fn (u21, f64) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const CharF64TreeMap, f: *const fn (u21) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const CharF64TreeMap, f: *const fn (f64) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const CharF64TreeMap, predicate: *const fn (u21, f64) bool) CharF64TreeMap {
    ...
    pub fn reject(self: *const CharF64TreeMap, predicate: *const fn (u21, f64) bool) CharF64TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const CharF64TreeMap, predicate: *const fn (u21, f64) bool) ?struct { key: u21, value: f64 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const CharF64TreeMap, predicate: *const fn (u21, f64) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const CharF64TreeMap, predicate: *const fn (u21, f64) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const CharF64TreeMap, predicate: *const fn (u21, f64) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const CharF64TreeMap, predicate: *const fn (u21, f64) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const CharF64TreeMap, key: u21) ?struct { key: u21, value: f64 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const CharF64TreeMap, key: u21) ?struct { key: u21, value: f64 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const CharF64TreeMap, from: u21, to: u21) []const u21 {
    ...
    pub fn format(self: *const CharF64TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *CharF64TreeMap, key: u21, value: f64) *CharF64TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *CharF64TreeMap, key: u21) *CharF64TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const CharF64TreeMap, other: *const CharF64TreeMap) bool {
...
test "CharF64TreeMap: put and get" {
    var m = CharF64TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', 1.0);
    _ = m.put('b', 2.0);
    _ = m.put('c', 3.0);
...
test "CharF64TreeMap: sorted keys" {
    var m = CharF64TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('c', 3.0);
    _ = m.put('a', 1.0);
...
test "CharF64TreeMap: eql" {
    var m1 = CharF64TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', 1.0);
    _ = m1.put('b', 2.0);

    var m2 = CharF64TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', 2.0);
    _ = m2.put('a', 1.0);
...
test "CharF64TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = CharF64TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharF64TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharF64TreeMap.init(failing.allocator());
    defer m.deinit();
