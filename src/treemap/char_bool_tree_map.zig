...
/// Sorted map from `u21` keys to `bool` values, backed by sorted ArrayLists.
pub const CharBoolTreeMap = struct {
    keys: std.ArrayListUnmanaged(u21) = .empty,
    vals: std.ArrayListUnmanaged(bool) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) CharBoolTreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) CharBoolTreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *CharBoolTreeMap) void {
    ...
    fn findIndex(self: *const CharBoolTreeMap, key: u21) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *CharBoolTreeMap, key: u21, value: bool) ?bool {
    ...
    pub fn get(self: *const CharBoolTreeMap, key: u21) ?bool {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *CharBoolTreeMap, key: u21) ?bool {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const CharBoolTreeMap, key: u21) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const CharBoolTreeMap, key: u21, default_value: bool) bool {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const CharBoolTreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const CharBoolTreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const CharBoolTreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *CharBoolTreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *CharBoolTreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *CharBoolTreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const CharBoolTreeMap) ?struct { key: u21, value: bool } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const CharBoolTreeMap) ?struct { key: u21, value: bool } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const CharBoolTreeMap) []const u21 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const CharBoolTreeMap) []const bool {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const CharBoolTreeMap, f: *const fn (u21, bool) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const CharBoolTreeMap, f: *const fn (u21) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const CharBoolTreeMap, f: *const fn (bool) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const CharBoolTreeMap, predicate: *const fn (u21, bool) bool) CharBoolTreeMap {
    ...
    pub fn reject(self: *const CharBoolTreeMap, predicate: *const fn (u21, bool) bool) CharBoolTreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const CharBoolTreeMap, predicate: *const fn (u21, bool) bool) ?struct { key: u21, value: bool } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const CharBoolTreeMap, predicate: *const fn (u21, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const CharBoolTreeMap, predicate: *const fn (u21, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const CharBoolTreeMap, predicate: *const fn (u21, bool) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const CharBoolTreeMap, predicate: *const fn (u21, bool) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const CharBoolTreeMap, key: u21) ?struct { key: u21, value: bool } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const CharBoolTreeMap, key: u21) ?struct { key: u21, value: bool } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const CharBoolTreeMap, from: u21, to: u21) []const u21 {
    ...
    pub fn format(self: *const CharBoolTreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *CharBoolTreeMap, key: u21, value: bool) *CharBoolTreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *CharBoolTreeMap, key: u21) *CharBoolTreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const CharBoolTreeMap, other: *const CharBoolTreeMap) bool {
...
test "CharBoolTreeMap: put and get" {
    var m = CharBoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('a', true);
    _ = m.put('b', false);
    _ = m.put('c', true);
...
test "CharBoolTreeMap: sorted keys" {
    var m = CharBoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put('c', true);
    _ = m.put('a', true);
...
test "CharBoolTreeMap: eql" {
    var m1 = CharBoolTreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put('a', true);
    _ = m1.put('b', false);

    var m2 = CharBoolTreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put('b', false);
    _ = m2.put('a', true);
...
test "CharBoolTreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = CharBoolTreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "CharBoolTreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = CharBoolTreeMap.init(failing.allocator());
    defer m.deinit();
