...
/// Sorted map from `bool` keys to `f32` values, backed by sorted ArrayLists.
pub const BoolF32TreeMap = struct {
    keys: std.ArrayListUnmanaged(bool) = .empty,
    vals: std.ArrayListUnmanaged(f32) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) BoolF32TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) BoolF32TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *BoolF32TreeMap) void {
    ...
    fn findIndex(self: *const BoolF32TreeMap, key: bool) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *BoolF32TreeMap, key: bool, value: f32) ?f32 {
    ...
    pub fn get(self: *const BoolF32TreeMap, key: bool) ?f32 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *BoolF32TreeMap, key: bool) ?f32 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const BoolF32TreeMap, key: bool) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const BoolF32TreeMap, key: bool, default_value: f32) f32 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const BoolF32TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const BoolF32TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const BoolF32TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *BoolF32TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *BoolF32TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *BoolF32TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const BoolF32TreeMap) ?struct { key: bool, value: f32 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const BoolF32TreeMap) ?struct { key: bool, value: f32 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const BoolF32TreeMap) []const bool {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const BoolF32TreeMap) []const f32 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const BoolF32TreeMap, f: *const fn (bool, f32) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const BoolF32TreeMap, f: *const fn (bool) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const BoolF32TreeMap, f: *const fn (f32) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const BoolF32TreeMap, predicate: *const fn (bool, f32) bool) BoolF32TreeMap {
    ...
    pub fn reject(self: *const BoolF32TreeMap, predicate: *const fn (bool, f32) bool) BoolF32TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const BoolF32TreeMap, predicate: *const fn (bool, f32) bool) ?struct { key: bool, value: f32 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const BoolF32TreeMap, predicate: *const fn (bool, f32) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const BoolF32TreeMap, predicate: *const fn (bool, f32) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const BoolF32TreeMap, predicate: *const fn (bool, f32) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const BoolF32TreeMap, predicate: *const fn (bool, f32) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const BoolF32TreeMap, key: bool) ?struct { key: bool, value: f32 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const BoolF32TreeMap, key: bool) ?struct { key: bool, value: f32 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const BoolF32TreeMap, from: bool, to: bool) []const bool {
    ...
    pub fn format(self: *const BoolF32TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *BoolF32TreeMap, key: bool, value: f32) *BoolF32TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *BoolF32TreeMap, key: bool) *BoolF32TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const BoolF32TreeMap, other: *const BoolF32TreeMap) bool {
...
test "BoolF32TreeMap: put and get" {
    var m = BoolF32TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
    _ = m.put(false, 2.0);
...
test "BoolF32TreeMap: sorted keys" {
    var m = BoolF32TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1.0);
    _ = m.put(false, 2.0);
...
test "BoolF32TreeMap: eql" {
    var m1 = BoolF32TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(true, 1.0);
    _ = m1.put(false, 2.0);

    var m2 = BoolF32TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(false, 2.0);
    _ = m2.put(true, 1.0);
...
test "BoolF32TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = BoolF32TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolF32TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolF32TreeMap.init(failing.allocator());
    defer m.deinit();
