...
/// Sorted map from `i32` keys to `f64` values, backed by sorted ArrayLists.
pub const I32F64TreeMap = struct {
    keys: std.ArrayListUnmanaged(i32) = .empty,
    vals: std.ArrayListUnmanaged(f64) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) I32F64TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) I32F64TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *I32F64TreeMap) void {
    ...
    fn findIndex(self: *const I32F64TreeMap, key: i32) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *I32F64TreeMap, key: i32, value: f64) ?f64 {
    ...
    pub fn get(self: *const I32F64TreeMap, key: i32) ?f64 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *I32F64TreeMap, key: i32) ?f64 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const I32F64TreeMap, key: i32) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const I32F64TreeMap, key: i32, default_value: f64) f64 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const I32F64TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const I32F64TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const I32F64TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *I32F64TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *I32F64TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *I32F64TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const I32F64TreeMap) ?struct { key: i32, value: f64 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const I32F64TreeMap) ?struct { key: i32, value: f64 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const I32F64TreeMap) []const i32 {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const I32F64TreeMap) []const f64 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const I32F64TreeMap, f: *const fn (i32, f64) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const I32F64TreeMap, f: *const fn (i32) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const I32F64TreeMap, f: *const fn (f64) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const I32F64TreeMap, predicate: *const fn (i32, f64) bool) I32F64TreeMap {
    ...
    pub fn reject(self: *const I32F64TreeMap, predicate: *const fn (i32, f64) bool) I32F64TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const I32F64TreeMap, predicate: *const fn (i32, f64) bool) ?struct { key: i32, value: f64 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const I32F64TreeMap, predicate: *const fn (i32, f64) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const I32F64TreeMap, predicate: *const fn (i32, f64) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const I32F64TreeMap, predicate: *const fn (i32, f64) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const I32F64TreeMap, predicate: *const fn (i32, f64) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const I32F64TreeMap, key: i32) ?struct { key: i32, value: f64 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const I32F64TreeMap, key: i32) ?struct { key: i32, value: f64 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const I32F64TreeMap, from: i32, to: i32) []const i32 {
    ...
    pub fn format(self: *const I32F64TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *I32F64TreeMap, key: i32, value: f64) *I32F64TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *I32F64TreeMap, key: i32) *I32F64TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const I32F64TreeMap, other: *const I32F64TreeMap) bool {
...
test "I32F64TreeMap: put and get" {
    var m = I32F64TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(1, 1.0);
    _ = m.put(2, 2.0);
    _ = m.put(3, 3.0);
...
test "I32F64TreeMap: sorted keys" {
    var m = I32F64TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(3, 3.0);
    _ = m.put(1, 1.0);
...
test "I32F64TreeMap: eql" {
    var m1 = I32F64TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(1, 1.0);
    _ = m1.put(2, 2.0);

    var m2 = I32F64TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(2, 2.0);
    _ = m2.put(1, 1.0);
...
test "I32F64TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = I32F64TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "I32F64TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = I32F64TreeMap.init(failing.allocator());
    defer m.deinit();
