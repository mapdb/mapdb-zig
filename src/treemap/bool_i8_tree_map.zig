...
/// Sorted map from `bool` keys to `i8` values, backed by sorted ArrayLists.
pub const BoolI8TreeMap = struct {
    keys: std.ArrayListUnmanaged(bool) = .empty,
    vals: std.ArrayListUnmanaged(i8) = .empty,
    config: AllocatorConfig,

    pub fn init(allocator: Allocator) BoolI8TreeMap {
        return .{ .config = AllocatorConfig.init(allocator) };
    }

    pub fn initWithConfig(config: AllocatorConfig) BoolI8TreeMap {
        return .{ .config = config };
    }

    pub fn deinit(self: *BoolI8TreeMap) void {
    ...
    fn findIndex(self: *const BoolI8TreeMap, key: bool) struct { index: usize, found: bool } {
        var lo: usize = 0;
    ...
    pub fn put(self: *BoolI8TreeMap, key: bool, value: i8) ?i8 {
    ...
    pub fn get(self: *const BoolI8TreeMap, key: bool) ?i8 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        return self.vals.items[result.index];
    }

    pub fn remove(self: *BoolI8TreeMap, key: bool) ?i8 {
        const result = self.findIndex(key);
        if (!result.found) return null;
        const old = self.vals.items[result.index];
    ...
    pub fn containsKey(self: *const BoolI8TreeMap, key: bool) bool {
        return self.findIndex(key).found;
    }

    pub fn getOrDefault(self: *const BoolI8TreeMap, key: bool, default_value: i8) i8 {
        return self.get(key) orelse default_value;
    }

    pub fn len(self: *const BoolI8TreeMap) usize {
        return self.keys.items.len;
    }

    /// Alias for len() — matches Go/Java naming.
    pub fn size(self: *const BoolI8TreeMap) usize {
        return self.len();
    }

    pub fn isEmpty(self: *const BoolI8TreeMap) bool {
        return self.keys.items.len == 0;
    }

    pub fn clear(self: *BoolI8TreeMap) void {
    ...
    /// allocator fails.
    pub fn ensureUnusedCapacity(self: *BoolI8TreeMap, additional: usize) Allocator.Error!void {
    ...
    /// `new_capacity`.
    pub fn ensureTotalCapacity(self: *BoolI8TreeMap, new_capacity: usize) Allocator.Error!void {
    ...
    pub fn min(self: *const BoolI8TreeMap) ?struct { key: bool, value: i8 } {
        if (self.keys.items.len == 0) return null;
        return .{ .key = self.keys.items[0], .value = self.vals.items[0] };
    }

    pub fn max(self: *const BoolI8TreeMap) ?struct { key: bool, value: i8 } {
        if (self.keys.items.len == 0) return null;
        const last = self.keys.items.len - 1;
        return .{ .key = self.keys.items[last], .value = self.vals.items[last] };
    }

    pub fn keysSlice(self: *const BoolI8TreeMap) []const bool {
        return self.keys.items;
    }

    pub fn valuesSlice(self: *const BoolI8TreeMap) []const i8 {
        return self.vals.items;
    }

    // ---- Iteration ----

    pub fn forEach(self: *const BoolI8TreeMap, f: *const fn (bool, i8) void) void {
        for (self.keys.items, self.vals.items) |k, val| f(k, val);
    }

    pub fn forEachKey(self: *const BoolI8TreeMap, f: *const fn (bool) void) void {
        for (self.keys.items) |k| f(k);
    }

    pub fn forEachValue(self: *const BoolI8TreeMap, f: *const fn (i8) void) void {
        for (self.vals.items) |val| f(val);
    }

    // ---- Functional Operations ----

    pub fn select(self: *const BoolI8TreeMap, predicate: *const fn (bool, i8) bool) BoolI8TreeMap {
    ...
    pub fn reject(self: *const BoolI8TreeMap, predicate: *const fn (bool, i8) bool) BoolI8TreeMap {
        var result = init(self.config.base);
        for (self.keys.items, self.vals.items) |k, val| {
            if (!predicate(k, val)) _ = result.put(k, val);
        }
    ...
    pub fn detect(self: *const BoolI8TreeMap, predicate: *const fn (bool, i8) bool) ?struct { key: bool, value: i8 } {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn anySatisfy(self: *const BoolI8TreeMap, predicate: *const fn (bool, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn allSatisfy(self: *const BoolI8TreeMap, predicate: *const fn (bool, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn noneSatisfy(self: *const BoolI8TreeMap, predicate: *const fn (bool, i8) bool) bool {
        for (self.keys.items, self.vals.items) |k, val| {
            if (predicate(k, val)) return false;
        }
        return true;
    ...
    pub fn count(self: *const BoolI8TreeMap, predicate: *const fn (bool, i8) bool) usize {
        var c: usize = 0;
        for (self.keys.items, self.vals.items) |k, val| {
    ...
    pub fn ceiling(self: *const BoolI8TreeMap, key: bool) ?struct { key: bool, value: i8 } {
        const result = self.findIndex(key);
    ...
    pub fn floor(self: *const BoolI8TreeMap, key: bool) ?struct { key: bool, value: i8 } {
        const result = self.findIndex(key);
        if (result.found) return .{ .key = self.keys.items[result.index], .value = self.vals.items[result.index] };
    ...
    pub fn rangeKeys(self: *const BoolI8TreeMap, from: bool, to: bool) []const bool {
    ...
    pub fn format(self: *const BoolI8TreeMap, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        try writer.writeAll("{");
    ...
    pub fn withKeyValue(self: *BoolI8TreeMap, key: bool, value: i8) *BoolI8TreeMap {
        _ = self.put(key, value);
        return self;
    }

    pub fn withoutKey(self: *BoolI8TreeMap, key: bool) *BoolI8TreeMap {
        _ = self.remove(key);
        return self;
    }
    ...
    pub fn eql(self: *const BoolI8TreeMap, other: *const BoolI8TreeMap) bool {
...
test "BoolI8TreeMap: put and get" {
    var m = BoolI8TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI8TreeMap: sorted keys" {
    var m = BoolI8TreeMap.init(std.testing.allocator);
    defer m.deinit();
    _ = m.put(true, 1);
    _ = m.put(false, 2);
...
test "BoolI8TreeMap: eql" {
    var m1 = BoolI8TreeMap.init(std.testing.allocator);
    defer m1.deinit();
    _ = m1.put(true, 1);
    _ = m1.put(false, 2);

    var m2 = BoolI8TreeMap.init(std.testing.allocator);
    defer m2.deinit();
    _ = m2.put(false, 2);
    _ = m2.put(true, 1);
...
test "BoolI8TreeMap: ensureUnusedCapacity reserves both buffers" {
    var m = BoolI8TreeMap.init(std.testing.allocator);
    defer m.deinit();
...
test "BoolI8TreeMap: ensureUnusedCapacity propagates allocator error" {
    ...
    var m = BoolI8TreeMap.init(failing.allocator());
    defer m.deinit();
