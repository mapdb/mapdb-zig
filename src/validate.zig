...
// (forward-compat). Direct translation of the Rust runner's run_count_min /
// run_space_saving (mapdb-rust/src/bin/validate.rs).

/// Parse an optional `count` operand: a u64 DECIMAL STRING (or bare JSON number
/// for small values) parsed straight to u64, never via f64. Omitted (`.null`)
/// => 1 (the add_one shape). Returns null when the value is present but does
/// not parse as a `0..=maxInt(u64)` integer (negative / non-numeric / too big)
/// — the caller SKIPs the scenario (same wide-integer discipline as i64 keys).
fn parseCountOpt(v: ?std.json.Value) ?u64 {
    const val = v orelse return 1; // omitted => 1
    return switch (val) {
        .null => 1,
...
/// Recognise an `<prefix><v>` assertion whose `<v>` is a SIGNED base-10 i32
/// (full range incl. negatives; a leading `+` is rejected). Used for
/// `estimate_<v>` / `count_<v>` / `error_<v>`.
...
/// Compute the canonical string for a CountMin assertion key, or
/// "UNKNOWN_ASSERTION:<key>" to skip. `counters` is row-major with each u64
/// QUOTED as a decimal string; `total`/`depth`/`width`/`estimate_<v>` are bare.
...
/// Render an expected CountMin JSON assertion value into the canonical string
/// `evalCountMin` produces. `counters` is a JSON array of decimal-string
/// elements (rendered QUOTED, in order); scalar string/number values render
/// bare (matching the unquoted u64 scalars).
...
fn runCountMin(
    ...
    // Authoring rule: exactly ONE leading `with_params` op (zero or multiple =>
    // malformed => SKIP). Subsequent ops are `add`.
...
/// Render a triple slice as `[[item,"count","error"],…]` (item signed int,
/// count/error u64 decimal strings) — the canonical computed form.
...
/// Compute the canonical string for a SpaceSaving assertion key, or
/// "UNKNOWN_ASSERTION:<key>" to skip.
...
/// Render an expected SpaceSaving JSON value into the canonical string
/// `evalSpaceSaving` produces. `monitored_set`/`top_k_<k>` are JSON arrays of
/// `[item, count, error]` triples (count/error are decimal strings or numbers,
/// rendered QUOTED); scalar string/number values render bare.
    ...
    switch (v) {
        .array => |arr| {
            ...
            for (arr.items, 0..) |triple, i| {
                ...
                // [item, count, error]
                try writer.writeAll("[");
                try renderSSField(t.items[0], false, writer); // item: bare signed int
                try writer.writeAll(",");
                try renderSSField(t.items[1], true, writer); // count: quoted decimal
                try writer.writeAll(",");
                try renderSSField(t.items[2], true, writer); // error: quoted decimal
                try writer.writeAll("]");
            }
            try writer.writeAll("]");
        },
...
/// Render one triple field: `quoted` selects a quoted decimal-string u64
/// (count/error) vs a bare signed int (item).
...
fn runSpaceSaving(
    ...
    // Authoring rule: exactly ONE leading `with_capacity` op (zero or multiple
    // => malformed => SKIP). Subsequent `add` ops are applied IN LISTED ORDER
    // (Space-Saving is order-dependent — a runner MUST NOT reorder).
