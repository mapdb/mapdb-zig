...
pub fn build(b: *std.Build) void {
    ...
    // catch `@panic`, so the always-on traps (CMS `w == 0`, `optimal`
    // epsilon/delta, SpaceSaving `m == 0`) are verified out of process: with no
    // args this exe re-execs itself once per trap case and asserts each child
    // terminates non-cleanly. Made a `test` dependency so `zig build test`
    // (incl. `-Doptimize=ReleaseFast`) exercises it.
